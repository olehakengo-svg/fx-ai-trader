...
v7.3変更:
  ...
  - 強トレンドバイパス: ADX>=30 かつ DI gap>=15 → EMA21プルバック不要
    (極端なモメンタム相場でEMA21到達を待つことの機会損失を解消)
...
class GoldTrendMomentum(StrategyBase):
    ...
    def evaluate(self, ctx: SignalContext) -> Optional[Candidate]:
        ...
        # ── 強トレンドバイパス ──
        # ADX>=30 かつ DI gap>=15 の極端なモメンタム相場では
        # EMA21プルバック要件を免除 → トレンド継続エントリーに移行
        # 根拠: Wilder(1978) ADX>=30 = 非常に強いトレンド。
        #       金の強気相場(2026-04-) はEMA21に届かないまま継続上昇するケースが多発。
        #       プルバック待ちは機会損失 → 強トレンド中は現在のモメンタムで参入。
        _extreme_momentum = ctx.adx >= 30 and abs(_di_gap) >= 15
