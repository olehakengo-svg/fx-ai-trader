#!/usr/bin/env python3
"""A/B BT filter for ema200_trend_reversal V2 USD_JPY routing + per-bar dedup."""
...
def _compute_ema200_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    if cand is None:
        return {
            ...
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["ema200_trend_reversal no signal"],
            "atr": atr,
            "mode": "daytrade",
        }
    return {
        ...
        "reasons": ["✅ ema200_trend_reversal strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
        "regime": {},
        "indicators": {"rsi": ctx.rsi, "adx": ctx.adx, "ema200": ctx.ema200},
        "shadow_emit_signals": [],
    }
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_ema200_only_signal
