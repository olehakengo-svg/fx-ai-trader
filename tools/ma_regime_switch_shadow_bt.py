#!/usr/bin/env python3
"""A/B BT light filter for ma_regime_switch V2 M15 ATR regime selector."""
...
FLAG = "MA_REGIME_SWITCH_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "MA_REGIME_SWITCH_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "ma_regime_switch-shadow-redesign-v2-2026-05-05.json"
_M15_FEATURE_CACHE = {}


def _m15_features(df):
    import pandas as pd
    ...
    if INTERVAL == "15m":
        row = df.iloc[-1]
        ema21 = float(row.get("ema21", 0.0) or 0.0)
        ema21_3ago = float(df["ema21"].iloc[-4]) if "ema21" in df.columns and len(df) >= 4 else ema21
        atr = float(row.get("atr", 0.0) or 0.0)
        atr_pct = 50.0
        if "atr" in df.columns and atr > 0:
            atr_window = df["atr"].iloc[-50:].dropna()
            if len(atr_window) >= 20:
                atr_pct = float((atr_window < atr).sum()) / float(len(atr_window)) * 100.0
        return {
            "close": float(row["Close"]),
            "adx": float(row.get("adx", 0.0) or 0.0),
            "ema9": float(row.get("ema9", 0.0) or 0.0),
            "ema21": ema21,
            "ema50": float(row.get("ema50", 0.0) or 0.0),
            "ema_slope": ema21 - ema21_3ago,
            "atr": atr,
            "atr_pct": atr_pct,
        }
    ...
    last_ts = pd.Timestamp(ohlc.index[-1])
    closed_before = last_ts.floor("15min")
    closed_ohlc = ohlc.loc[ohlc.index < closed_before]
    if len(closed_ohlc) < 150:
        return {}
    cache_key = closed_ohlc.index[-1]
    cached = _M15_FEATURE_CACHE.get(cache_key)
    if cached is not None:
        return cached
    if len(_M15_FEATURE_CACHE) > 256:
        _M15_FEATURE_CACHE.clear()

    m15 = closed_ohlc.iloc[-900:].resample("15min").agg(
    ...
    atr_pct = 50.0
    atr_window = valid_atr.iloc[-50:]
    if atr_val > 0 and len(atr_window) >= 20:
        atr_pct = float((atr_window < atr_val).sum()) / float(len(atr_window)) * 100.0
    ema21_now = float(ema21.iloc[-1])
    ema21_3ago = float(ema21.iloc[-4]) if len(ema21) >= 4 else ema21_now
    payload = {
    ...
    _M15_FEATURE_CACHE[cache_key] = payload
    return payload


def _compute_ma_regime_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    if "bb_width" in df.columns and len(df) >= 50:
        bw_series = df["bb_width"].iloc[-50:]
        bb_width_pct = float((bw_series < bb_width).sum()) / 50.0
    else:
        bb_width_pct = 0.5

    htf = {
        "m15": _m15_features(df),
        "m5": {
            "close": float(row["Close"]),
            "prev_close": float(prev["Close"]),
            "ema21": float(row.get("ema21", 0.0)),
            "bbpb": float(row.get("bb_pband", 0.5)),
            "rsi14": float(row.get("rsi", 50.0)),
    ...
    cand = MaRegimeSwitch().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["ma_regime_switch no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ ma_regime_switch strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
        "regime": {},
        "indicators": {"rsi": ctx.rsi, "adx": ctx.adx, "bb_pband": ctx.bbpb},
        "shadow_emit_signals": [],
    }
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_ma_regime_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "m15_atr_percentile_regime_v2",
        "flag": FLAG,
        "shadow_worker_flag": SHADOW_PROMOTE_FLAG,
        "lookback_days": LOOKBACK_DAYS,
        "minimum_days": MINIMUM_DAYS,
        "interval": INTERVAL,
        "runner": "app.run_daytrade_backtest (production daytrade path; strategy-filter compute_daytrade_signal patch)",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy is absent from ScalperEngine unless MA_REGIME_SWITCH_REDESIGN_V2=1; shadow loser emit also needs MA_REGIME_SWITCH_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered M15 ATR percentile regime replacement is evaluated.",
