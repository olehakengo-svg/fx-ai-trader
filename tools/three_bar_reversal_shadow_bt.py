#!/usr/bin/env python3
"""A/B BT filter for three_bar_reversal V2 shadow redesign."""
...
FLAG = "THREE_BAR_REVERSAL_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "THREE_BAR_REVERSAL_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "three_bar_reversal-shadow-redesign-v2-2026-05-05.json"


def _compute_three_bar_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = ThreeBarReversal().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["three_bar_reversal no signal"],
            "atr": atr,
            "mode": "daytrade",
        }
    return {
        ...
        "reasons": ["✅ three_bar_reversal strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_three_bar_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "prev_open_reclaim_relaxed_extension_v2",
        "flag": FLAG,
        "shadow_worker_flag": SHADOW_PROMOTE_FLAG,
        "lookback_days": LOOKBACK_DAYS,
        "minimum_days": MINIMUM_DAYS,
        "interval": INTERVAL,
        "runner": "app.run_daytrade_backtest(backtest_mode=True; production daytrade path; three_bar_reversal strategy-only compute patch)",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless THREE_BAR_REVERSAL_REDESIGN_V2=1; shadow emit also requires THREE_BAR_REVERSAL_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered prev-open reclaim/ break V2 variant is evaluated.",
            "bt_source_guard": "PASS: BT_MODE=1 and BT_REQUIRE_MASSIVE_CACHE=1 are set before importing app.",
            "runner_note": "PASS: run_daytrade_backtest(backtest_mode=True) is used with a strategy-only compute patch so production execution/friction/data handling remains active.",
        },
    }
