#!/usr/bin/env python3
"""A/B BT filter for dt_sr_channel_reversal V2 closed-bar + MR geometry."""
...
FLAG = "DT_SR_CHANNEL_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "DT_SR_CHANNEL_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "dt_sr_channel-shadow-redesign-v2-2026-05-05.json"
...
def _compute_dt_sr_channel_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    if cand is None:
        return {
            ...
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["dt_sr_channel_reversal no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ dt_sr_channel_reversal strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    result = {
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless DT_SR_CHANNEL_REDESIGN_V2=1; shadow emit also requires DT_SR_CHANNEL_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed-bar + boundary SL + mean target V2 variant is evaluated.",
