#!/usr/bin/env python3
"""A/B BT light filter for bb_rsi_reversion V2 JPY high-ADX tail split."""
...
FLAG = "BB_RSI_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "BB_RSI_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "bb_rsi-shadow-redesign-v2-2026-05-05.json"


def _compute_bb_rsi_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = BBRsiReversion().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["bb_rsi_reversion no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ bb_rsi_reversion strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_bb_rsi_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "jpy_high_adx_tail_penalty_bypass_v2",
        "flag": FLAG,
        "shadow_worker_flag": SHADOW_PROMOTE_FLAG,
        "lookback_days": days,
        "minimum_days": MINIMUM_DAYS,
        "interval": INTERVAL,
        "runner": "app.run_daytrade_backtest (production daytrade path; bb_rsi strategy-only compute patch)",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless BB_RSI_REDESIGN_V2=1; shadow emit also requires BB_RSI_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered JPY high-ADX penalty-bypass variant is evaluated.",
