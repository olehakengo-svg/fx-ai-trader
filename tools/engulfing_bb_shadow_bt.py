#!/usr/bin/env python3
"""A/B BT filter for engulfing_bb closed-bar V2 shadow redesign."""
...
FLAG = "ENGULFING_BB_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "ENGULFING_BB_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "engulfing_bb-shadow-redesign-v2-2026-05-05.json"


def _compute_engulfing_bb_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = EngulfingBB().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["engulfing_bb no signal"],
            "atr": atr,
            "mode": "daytrade",
        }
    return {
        ...
        "reasons": ["✅ engulfing_bb strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_engulfing_bb_only_signal
    ...
    result = {
        ...
        "interval": INTERVAL,
        "runner": "app.run_daytrade_backtest(backtest_mode=True; production daytrade path; engulfing_bb strategy-only compute patch)",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless ENGULFING_BB_REDESIGN_V2=1; shadow emit also requires ENGULFING_BB_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed-bar variant is evaluated.",
