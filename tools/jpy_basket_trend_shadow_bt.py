#!/usr/bin/env python3
"""A/B BT filter for jpy_basket_trend V2 real-basket closed-bar redesign."""
...
TARGETS = [("USD_JPY", "USDJPY=X"), ("EUR_JPY", "EURJPY=X")]
LOOKBACK_DAYS = 365
MINIMUM_DAYS = 365
INTERVAL = "15m"
STRATEGY = "jpy_basket_trend"
FLAG = "JPY_BASKET_TREND_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "JPY_BASKET_TREND_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "jpy_basket_trend-shadow-redesign-v2-2026-05-05.json"
_BASKET_DFS = {}
_BASKET_SNAPSHOTS = {}


def _symbol_key(symbol: str) -> str:
    return symbol.upper().replace("=X", "").replace("_", "").replace("/", "")
...
def _snapshot_from_row(row) -> dict:
    return {
        "ema9": float(row.get("ema9")),
        "ema21": float(row.get("ema21")),
        "ema50": float(row.get("ema50")),
    }


def _counterpart_snapshot(symbol: str, signal_bar_time):
    counterpart = "EURJPY=X" if _symbol_key(symbol) == "USDJPY" else "USDJPY=X"
    snapshots = _BASKET_SNAPSHOTS.get(_symbol_key(counterpart))
    if not snapshots:
        return None
    times, rows = snapshots
    pos = bisect_right(times, signal_bar_time) - 1
    if pos < 0:
        return None
    return rows[pos]


def _compute_jpy_basket_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    basket = {
        _symbol_key(symbol): _snapshot_from_row(signal_bar),
    }
    other = _counterpart_snapshot(symbol, signal_bar_time)
    if other is not None:
        basket["EURJPY" if _symbol_key(symbol) == "USDJPY" else "USDJPY"] = other
    htf = {**htf, "jpy_basket": basket}
    if bar_time is None:
        bar_time = df.index[-1]
    ...
    ctx = SignalContext(
        ...
        ema200_bull=entry > float(row.get("ema200", row.get("ema50", entry))),
        symbol=symbol,
        tf=tf,
        is_jpy=is_jpy,
    ...
    cand = JpyBasketTrend().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["jpy_basket_trend no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ jpy_basket_trend strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def _criteria(current: dict, proposed: dict) -> dict:
    ...
    proposed_err = proposed.get("bt_error")
    if proposed_err:
        return {
            "verdict": "REJECT",
            "reason": "bt_error",
            "bt_error": proposed_err,
    ...
    current_insufficient = (
        current.get("bt_error")
        and "20トレード未満" in str(current.get("bt_error"))
        and current.get("N", 0) < 20
    )
    if current_insufficient:
        floor = proposed["wilson_lo"] >= 0.20 and proposed_pf >= 0.85
        verdict = "PASS" if floor else "REJECT"
        return {
            "reason": "current baseline returned <20-trade BT error; catastrophic regression is not established, so v2 filter applies proposed sanity floor only",
            "current_baseline_evidence": "INSUFFICIENT_BT_EVIDENCE",
            "n_change_pct": round(_change_pct(proposed["N"], current["N"]), 4),
            "catastrophic_check": "SKIPPED_BASELINE_INSUFFICIENT",
            "sanity_floor": floor,
            "verdict": verdict,
...
def main() -> int:
    ...
    if missing:
        ...
        for _pair, _symbol in TARGETS:
            _df = app.fetch_ohlcv(_symbol, period=f"{LOOKBACK_DAYS}d", interval=INTERVAL)
            _df = app.add_indicators(_df).dropna()
            _BASKET_DFS[_symbol_key(_symbol)] = _df
            _BASKET_SNAPSHOTS[_symbol_key(_symbol)] = (
                list(_df.index),
                [_snapshot_from_row(row) for _, row in _df.iterrows()],
            )

        app.compute_daytrade_signal = _compute_jpy_basket_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "real_basket_po_closed_bar_dedup_v2",
        "flag": FLAG,
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless JPY_BASKET_TREND_REDESIGN_V2=1; shadow emit also requires JPY_BASKET_TREND_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered real_basket_po_closed_bar_dedup_v2 variant is evaluated.",
            "bt_source_guard": "PASS: BT_REQUIRE_MASSIVE_CACHE=1 prevents Yahoo fallback for 15m price data; required MASSIVE cache files are checked before running.",
        },
    }
