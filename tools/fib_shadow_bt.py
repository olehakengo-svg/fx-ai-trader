#!/usr/bin/env python3
"""A/B BT filter for fib/dt_fib_reversal V2 closed-bar + wider fib SL."""
...
MINIMUM_DAYS = 365
INTERVAL = "15m"
STRATEGY = "dt_fib_reversal"
AUDIT_STRATEGY = "fib_reversal"
FLAG = "FIB_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "FIB_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "fib-shadow-redesign-v2-2026-05-05.json"


def _compute_fib_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = DtFibReversal().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["dt_fib_reversal no signal"],
            "atr": atr,
            "mode": "daytrade",
            "indicators": {"rsi": ctx.rsi, "bb_mid": ctx.bb_mid},
        }
    return {
        "signal": cand.signal,
        "entry": entry,
        ...
        "reasons": ["✅ dt_fib_reversal strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_fib_only_signal
    ...
    result = {
        ...
        "variant": "closed_bar_dedup_wide_fib_sl_v2",
        "flag": FLAG,
        ...
        "target_scope_note": "USD_JPY narrow cell: audits/edge_design/fib.md marks USDJPY as the only fit/narrow positive cell; XAUUSD MASSIVE 15m cache is unavailable in this workspace.",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless FIB_REDESIGN_V2=1; shadow emit also requires FIB_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed-bar + dedup + wider fib SL V2 variant is evaluated.",
