#!/usr/bin/env python3
"""A/B BT filter for mtf_regime_trend_cascade_scalp redesign V2."""
...
FLAG = "MTF_REGIME_TREND_CASCADE_SCALP_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "MTF_REGIME_TREND_CASCADE_SCALP_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "mtf_regime_trend_cascade_scalp-shadow-redesign-v2-2026-05-05.json"
...
def _build_ctx(df, tf, sr_levels, symbol, backtest_mode, bar_time):
    ...
    slope_window = min(4, len(df))
    ema_slope = ema21 - float(df["ema21"].iloc[-slope_window]) if "ema21" in df.columns else 0.0
    m5_close = entry
    m5_prev_close = float(prev["Close"])
    htf = {
        "m15": {
            "adx": adx,
            "ema_slope": ema_slope,
...
def _compute_strategy_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = MtfRegimeTrendCascadeScalp().evaluate(ctx)
...
def main() -> int:
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "closed_prior_bar_signal_v2",
        "flag": FLAG,
        ...
        "self_review": {
            "catastrophic_only": "PASS: no positive-direction or Kelly threshold is applied.",
            "insufficient_bt_evidence": "PASS: proposed N<20 returns INSUFFICIENT_BT_EVIDENCE and recommends shadow.",
            "production_live_safety": "PASS: redesign and shadow worker behavior are default-off env flags.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed_prior_bar_signal_v2 variant is evaluated.",
