#!/usr/bin/env python3
"""A/B BT filter for ma_mr_hybrid V2 M15-bias softening."""
...
FLAG = "MA_MR_HYBRID_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "MA_MR_HYBRID_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "ma_mr_hybrid-shadow-redesign-v2-2026-05-05.json"
...
def _load_m5_features(pair: str):
    if pair in _M5_CACHE:
        return _M5_CACHE[pair]
    import pandas as pd
    import app

    df = pd.read_parquet(_cache_path(pair, M5_INTERVAL))
    rename = {
        "open": "Open",
        "high": "High",
        "low": "Low",
    ...
    df = app.add_indicators(df).dropna()
    _M5_CACHE[pair] = df
    return df


def _latest_m5_row(pair: str, bar_time):
    m5 = _load_m5_features(pair)
    if bar_time is None:
        return None
    rows = m5.loc[:bar_time]
    if rows.empty:
        return None
    return rows.iloc[-1]


def _compute_ma_mr_only_signal(
    df,
    tf,
    sr_levels,
    symbol="USDJPY=X",
    ...
    row = df.iloc[-1]
    prev = df.iloc[-2]
    m5_row = _latest_m5_row(pair, bar_time)
    if m5_row is None:
        return {"signal": "WAIT", "entry_type": "wait", "reasons": ["missing m5 massive row"]}
    ...
    ctx = SignalContext(
        ...
        macdh_prev2=float(df.iloc[-3].get("macd_hist", 0.0)),
        ...
        htf={
            ...
            "m5": {
                "bbpb": float(m5_row.get("bb_pband", 0.5)),
                "rsi14": float(m5_row.get("rsi", 50.0)),
                "stoch_k": float(m5_row.get("stoch_k", 50.0)),
                "stoch_d": float(m5_row.get("stoch_d", 50.0)),
            },
        },
        symbol=symbol,
        tf=tf,
    ...
    cand = MaMrHybrid().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["ma_mr_hybrid no signal"],
            "atr": atr,
            "mode": "daytrade",
        }
    return {
        ...
        "reasons": ["ma_mr_hybrid strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    for pair, _ in TARGETS:
        for interval in (INTERVAL, M5_INTERVAL):
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_ma_mr_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "m15_bias_soft_feature_v2",
        "flag": FLAG,
        ...
        "data_source_required": "data/cache/massive/USD_JPY_15m.parquet and USD_JPY_5m.parquet with BT_MODE=1 and BT_REQUIRE_MASSIVE_CACHE=1",
        "data_source_note": "M15 and M5 features are loaded from native MASSIVE parquet files; no resampling or Yahoo fallback.",
        ...
        "self_review": {
            ...
            "production_live_safety": f"PASS: default behavior is unchanged unless {FLAG}=1; shadow emit also requires {SHADOW_PROMOTE_FLAG}=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered m15_bias_soft_feature_v2 axis is evaluated.",
            "bt_source_guard": "PASS: BT_REQUIRE_MASSIVE_CACHE=1 prevents Yahoo/network fallback; M5 helper also requires native MASSIVE cache.",
        },
    }
