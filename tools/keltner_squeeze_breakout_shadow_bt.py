#!/usr/bin/env python3
"""A/B BT filter for keltner_squeeze_breakout V2 closed-bar hardening."""
...
FLAG = "KELTNER_SQUEEZE_BREAKOUT_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "KELTNER_SQUEEZE_BREAKOUT_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "keltner_squeeze_breakout-shadow-redesign-v2-2026-05-05.json"


def _compute_ksb_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = KeltnerSqueezeBreakout().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["keltner_squeeze_breakout no signal"],
            "atr": atr,
            "mode": "hourly",
    ...
    return {
        ...
        "reasons": ["ksb strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "hourly",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        for pair, _symbol in TARGETS:
            cache_err = f"{err}: {_cache_path(pair).relative_to(ROOT)}"
            cells[pair] = {
                "current": {"N": 0, "wins": 0, "WR": 0.0, "EV": 0.0, "PnL": 0.0,
                            "PF": 0.0, "wilson_lo": 0.0, "bt_error": cache_err},
        ...
        app.compute_daytrade_signal = _compute_ksb_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "closed_bar_next_entry_dedup_v2",
        "flag": FLAG,
        ...
        "data_source_note": "run_daytrade_backtest omits data_source/bars_fetched when it returns the <20 trades error; console output showed massive-parquet/1h for each target.",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless KELTNER_SQUEEZE_BREAKOUT_REDESIGN_V2=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed_bar_next_entry_dedup_v2 variant is evaluated.",
