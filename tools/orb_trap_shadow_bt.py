#!/usr/bin/env python3
"""A/B BT light filter for orb_trap V2 fixed OR-edge TP geometry."""
...
INTERVAL = "15m"
STRATEGY = "orb_trap"
FLAG = "ORB_TRAP_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "ORB_TRAP_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "orb_trap-shadow-redesign-v2-2026-05-05.json"


def _compute_orb_trap_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = OrbTrap().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["orb_trap no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["orb_trap strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_orb_trap_only_signal
    ...
    result = {
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless ORB_TRAP_REDESIGN_V2=1; shadow emit also requires ORB_TRAP_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered fixed_or_edge_tp_v2 variant is evaluated.",
            "bt_source_guard": "PASS: BT_REQUIRE_MASSIVE_CACHE=1 prevents Yahoo fallback for 15m price data; required MASSIVE caches are verified before BT.",
        },
    }
