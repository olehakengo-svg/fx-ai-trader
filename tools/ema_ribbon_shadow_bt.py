#!/usr/bin/env python3
"""A/B BT filter for ema_ribbon V2 closed-bar + R-floor redesign."""
...
MINIMUM_DAYS = 365
INTERVAL = "15m"
STRATEGY = "ema_ribbon_ride"
FLAG = "EMA_RIBBON_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "EMA_RIBBON_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "ema_ribbon-shadow-redesign-v2-2026-05-05.json"


def _compute_ema_ribbon_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = EmaRibbonRide().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["ema_ribbon no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ ema_ribbon strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_ema_ribbon_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "closed_bar_r_floor_dedup_v2",
        "flag": FLAG,
        ...
        "target_scope_note": "MASSIVE 15m caches exist for USD_JPY/EUR_USD/EUR_JPY; XAUUSD is implementation-enabled but omitted because no XAU_USD_15m MASSIVE cache is present.",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless EMA_RIBBON_REDESIGN_V2=1; shadow emit also requires EMA_RIBBON_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed_bar_r_floor_dedup_v2 variant is evaluated.",
            "bt_source_guard": "PASS: BT_REQUIRE_MASSIVE_CACHE=1 prevents Yahoo/network fallback for 15m price data.",
            "runner_note": "PASS: user-required run_daytrade_backtest path is used; this remains a filter BT, not a live-promotion gate.",
        },
    }
