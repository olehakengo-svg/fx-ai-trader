#!/usr/bin/env python3
"""A/B BT filter for macdh_reversal V2 Tier1 MR geometry redesign."""
...
FLAG = "MACDH_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "MACDH_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "macdh-shadow-redesign-v2-2026-05-05.json"


def _compute_macdh_only_signal(
    df,
    tf,
    sr_levels,
    symbol="EURUSD=X",
    ...
    cand = MacdhReversal().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["macdh_reversal no signal"],
            "atr": atr,
            "mode": "daytrade",
            "indicators": {"adx": ctx.adx, "rsi5": ctx.rsi5, "bb_pband": ctx.bbpb},
        }
    return {
        "signal": cand.signal,
        "entry": entry,
        ...
        "reasons": ["macdh_reversal strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
        "regime": {},
        "indicators": {"adx": ctx.adx, "rsi5": ctx.rsi5, "bb_pband": ctx.bbpb, "macdh": ctx.macdh},
        "shadow_emit_signals": [],
    }
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_macdh_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "tier1_extreme_wide_mr_geometry_v2",
        "flag": FLAG,
        ...
        "target_scope_note": "W4 audit target is macdh_reversal ALL; EUR_GBP is included but remains disabled by existing strategy policy.",
        ...
        "self_review": {
            ...
            "post_hoc_adjustment": "PASS: only the pre-registered Tier1 extreme + wide MR geometry variant is evaluated.",
