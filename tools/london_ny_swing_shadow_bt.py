#!/usr/bin/env python3
"""A/B BT filter for london_ny_swing V2 timing hardening."""
...
FLAG = "LONDON_NY_SWING_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "LONDON_NY_SWING_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "london_ny_swing-shadow-redesign-v2-2026-05-05.json"


class _LondonNyOnlyDaytradeEngine:
    SHADOW_ALWAYS_STRATEGIES = frozenset()

    def __init__(self):
        from strategies.daytrade.london_ny_swing import LondonNySwing
        self.strategies = [LondonNySwing()]
...
def _compute_london_ny_only_signal(df, tf, sr_levels, symbol="EURUSD=X",
    ...
    cand = LondonNySwing().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["london_ny_swing no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ london_ny_swing strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
...
def main() -> int:
    ...
    if missing:
        ...
        daytrade_mod.DaytradeEngine = _LondonNyOnlyDaytradeEngine
        app.compute_daytrade_signal = _compute_london_ny_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "closed_signal_bar_next_execution_v2",
        "flag": FLAG,
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless LONDON_NY_SWING_REDESIGN_V2=1; shadow emit also needs a second promote flag.",
            "post_hoc_adjustment": "PASS: only the pre-registered timing hardening variant is evaluated.",
