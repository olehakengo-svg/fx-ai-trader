#!/usr/bin/env python3
"""A/B BT filter for Tokyo Range Breakout V2 shadow redesign."""
...
MINIMUM_DAYS = 365
INTERVAL = "15m"
STRATEGY = "tokyo_range_breakout_up"
FLAG = "TOKYO_RANGE_BREAKOUT_REDESIGN_V2"
SHADOW_FLAG = "TOKYO_RANGE_BREAKOUT_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "tokyo_range_breakout-shadow-redesign-v2-2026-05-05.json"


def _compute_tokyo_range_breakout_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = TokyoRangeBreakout().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["tokyo_range_breakout no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ tokyo_range_breakout strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def _warnings(current: dict, proposed: dict) -> dict:
...
def _criteria(current: dict, proposed: dict) -> dict:
    ...
    if err and "missing MASSIVE parquet cache" in str(err):
        return {
    ...
    if proposed["N"] < 20:
        return {
            ...
            "warnings": _warnings(current, proposed),
        }
...
def main() -> int:
    ...
    if missing:
        for pair, _symbol in TARGETS:
            ...
            stats = {"N": 0, "wins": 0, "WR": 0.0, "EV": 0.0, "PnL": 0.0,
                     "PF": 0.0, "wilson_lo": 0.0, "bt_error": err}
            cells[pair] = {
                "current": stats,
                "proposed": stats,
                "lock_criteria": _criteria(stats, stats),
            }
    else:
        import app  # noqa: E402
        from modules import data as data_mod  # noqa: E402

        app.compute_daytrade_signal = _compute_tokyo_range_breakout_only_signal
    ...
    if any(v == "REJECT" for v in verdicts):
        overall = "REJECT"
    elif any(v == "BLOCKED_DATA" for v in verdicts):
        overall = "BLOCKED_DATA"
    elif any(v == "INSUFFICIENT_BT_EVIDENCE" for v in verdicts):
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "bar_close_buffered_compression_invalidation_exit_v2",
        "flag": FLAG,
        ...
        "lock_spec": {
            ...
            "warn_only": ["PF change", "Wilson lower change", "N change pct"],
        ...
        "shadow_promote_recommendation": (
            "RECOMMEND_SHADOW" if overall in {"PASS", "INSUFFICIENT_BT_EVIDENCE"} else
            "IMPLEMENT_ONLY_DEFAULT_OFF" if overall == "BLOCKED_DATA" else "REJECT"
        ),
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless TOKYO_RANGE_BREAKOUT_REDESIGN_V2=1; shadow emit also requires TOKYO_RANGE_BREAKOUT_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered bar-close/buffer/compression/invalidation V2 variant is evaluated.",
            "bt_source_guard": "PASS: BT_REQUIRE_MASSIVE_CACHE=1 prevents Yahoo fallback for 15m price data.",
            "absolute_kelly": "PASS: no absolute Kelly threshold is used.",
        },
    }
