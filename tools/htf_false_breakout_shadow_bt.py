#!/usr/bin/env python3
"""A/B BT light filter for htf_false_breakout V2 trigger/timing redesign."""
...
FLAG = "HTF_FALSE_BREAKOUT_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "HTF_FALSE_BREAKOUT_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "htf_false_breakout-shadow-redesign-v2-2026-05-05.json"


class _HtfFalseBreakoutOnlyDaytradeEngine:
    """BT strategy filter equivalent to strategies=['htf_false_breakout']."""

    SHADOW_ALWAYS_STRATEGIES = frozenset()

    def __init__(self):
        from strategies.daytrade.htf_false_breakout import HtfFalseBreakout
        self.strategies = [HtfFalseBreakout()]
...
def _compute_hfb_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    ctx = SignalContext(
        ...
        prev_close=float(prev.get("Close", entry)),
        prev_open=float(prev.get("Open", entry)),
        symbol=symbol,
        tf=tf,
        is_jpy=is_jpy,
    ...
    cand = HtfFalseBreakout().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["htf_false_breakout no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ htf_false_breakout strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        daytrade_mod.DaytradeEngine = _HtfFalseBreakoutOnlyDaytradeEngine
        app.compute_daytrade_signal = _compute_hfb_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "explicit_h1_breakout_first_15m_reentry_v2",
        "flag": FLAG,
        "shadow_worker_flag": SHADOW_PROMOTE_FLAG,
        "lookback_days": days,
        "minimum_days": MINIMUM_DAYS,
        "interval": INTERVAL,
        "runner": "app.run_daytrade_backtest (production daytrade path; strategy-only DaytradeEngine patch)",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless HTF_FALSE_BREAKOUT_REDESIGN_V2=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered explicit_h1_breakout_first_15m_reentry_v2 variant is evaluated.",
