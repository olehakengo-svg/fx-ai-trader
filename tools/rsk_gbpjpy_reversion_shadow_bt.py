#!/usr/bin/env python3
"""A/B BT filter for rsk_gbpjpy_reversion V2 MR exit geometry."""
...
TARGETS = [("GBP_JPY", "GBPJPY=X")]
LOOKBACK_DAYS = 365
MINIMUM_DAYS = 365
INTERVAL = "15m"
STRATEGY = "rsk_gbpjpy_reversion"
FLAG = "RSK_GBPJPY_REVERSION_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "RSK_GBPJPY_REVERSION_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "rsk_gbpjpy_reversion-shadow-redesign-v2-2026-05-05.json"


def _compute_rsk_only_signal(df, tf, sr_levels, symbol="GBPJPY=X",
    ...
    if cand is None:
        return {
            ...
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["rsk_gbpjpy_reversion no signal"],
            "atr": atr,
            "mode": "daytrade",
            "indicators": {},
        }
    return {
        ...
        "reasons": ["✅ rsk_gbpjpy_reversion strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_rsk_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "mr_exit_geometry_v2_sl_1p8atr_tp_0p8atr_no_min_rr",
        "flag": FLAG,
        "shadow_worker_flag": SHADOW_PROMOTE_FLAG,
        "target_scope": "GBP_JPY only; trigger and pair gate unchanged",
        "lookback_days": LOOKBACK_DAYS,
        ...
        "shadow_registration_note": (
            "base rsk_gbpjpy_reversion is already in DaytradeEngine.SHADOW_ALWAYS_STRATEGIES; "
            "V2 live/shadow behavior remains default-off unless RSK_GBPJPY_REVERSION_REDESIGN_V2=1"
        ),
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy V2 geometry is default-off unless RSK_GBPJPY_REVERSION_REDESIGN_V2=1; app BT cache and SL/TP preserve are flag-aware.",
            "post_hoc_adjustment": "PASS: only the pre-registered MR exit geometry variant is evaluated.",
