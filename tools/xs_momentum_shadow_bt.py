#!/usr/bin/env python3
"""A/B BT light filter for xs_momentum V2 timing hardening."""
...
def _compute_xs_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = XsMomentum().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["xs_momentum no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ xs_momentum strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
...
def main() -> int:
    ...
    if missing:
        ...
        daytrade_mod.DaytradeEngine = _XsOnlyDaytradeEngine
        app.compute_daytrade_signal = _compute_xs_only_signal
