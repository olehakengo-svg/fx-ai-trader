#!/usr/bin/env python3
"""A/B BT filter for lin_reg_channel V2 closed-bar + midline geometry."""
...
TARGETS = [("EUR_USD", "EURUSD=X")]
LOOKBACK_DAYS = 365
MINIMUM_DAYS = 365
INTERVAL = "15m"
STRATEGY = "lin_reg_channel"
FLAG = "LIN_REG_CHANNEL_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "LIN_REG_CHANNEL_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "lin_reg_channel-shadow-redesign-v2-2026-05-05.json"


def _compute_lin_reg_channel_only_signal(df, tf, sr_levels, symbol="EURUSD=X",
    ...
    cand = LinRegChannel().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["lin_reg_channel no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["lin_reg_channel strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_lin_reg_channel_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "closed_bar_signal_next_bar_entry_midline_tp_v2",
        "flag": FLAG,
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless LIN_REG_CHANNEL_REDESIGN_V2=1; shadow emit also requires LIN_REG_CHANNEL_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed-bar + next-bar entry + midline TP V2 variant is evaluated.",
