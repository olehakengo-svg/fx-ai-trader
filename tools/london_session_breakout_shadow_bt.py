#!/usr/bin/env python3
"""A/B BT filter for london_session_breakout V2 shadow redesign."""
...
TARGETS = [("EUR_USD", "EURUSD=X"), ("GBP_USD", "GBPUSD=X")]
LOOKBACK_DAYS = 365
MINIMUM_DAYS = 365
INTERVAL = "15m"
STRATEGY = "london_session_breakout"
FLAG = "LONDON_SESSION_BREAKOUT_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "LONDON_SESSION_BREAKOUT_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "london_session_breakout-shadow-redesign-v2-2026-05-05.json"


class _LondonSessionBreakoutOnlyDaytradeEngine:
    SHADOW_ALWAYS_STRATEGIES = frozenset()

    def __init__(self):
        from strategies.daytrade.london_session_breakout import LondonSessionBreakout
        self.strategies = [LondonSessionBreakout()]
...
def _compute_lsb_only_signal(df, tf, sr_levels, symbol="EURUSD=X",
    ...
    cand = LondonSessionBreakout().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["london_session_breakout no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ london_session_breakout strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
...
def main() -> int:
    ...
    if missing:
        ...
        daytrade_mod.DaytradeEngine = _LondonSessionBreakoutOnlyDaytradeEngine
        app.compute_daytrade_signal = _compute_lsb_only_signal
        app.get_master_bias = lambda _symbol: {"direction": "neutral", "label": "BT lsb-only", "score": 0}
        app._compute_bt_htf_bias = lambda _df, _idx, mode="daytrade": {"agreement": "mixed"}
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "closed_07utc_london_pair_breakout_v2",
        "flag": FLAG,
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless LONDON_SESSION_BREAKOUT_REDESIGN_V2=1; shadow emit also needs a second promote flag.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed-07UTC London-pair breakout variant is evaluated.",
