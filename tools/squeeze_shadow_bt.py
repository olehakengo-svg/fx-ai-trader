#!/usr/bin/env python3
"""A/B BT filter for bb_squeeze_breakout V2 actual-breakout trigger."""
...
FLAG = "SQUEEZE_REDESIGN_V2"
SHADOW_FLAG = "SQUEEZE_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "squeeze-shadow-redesign-v2-2026-05-05.json"


def _compute_squeeze_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = BBSqueezeBreakout().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["bb_squeeze_breakout no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ squeeze strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_squeeze_only_signal
        app.find_sr_levels_weighted = lambda *args, **kwargs: []
        app.get_master_bias = lambda *args, **kwargs: {"direction": "neutral", "label": "BT_PATCH", "score": 0}
        app._compute_bt_htf_bias = lambda *args, **kwargs: {"agreement": "mixed"}
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "closed_bar_actual_breakout_v2",
        "flag": FLAG,
        "shadow_worker_flag": SHADOW_FLAG,
        "lookback_days": LOOKBACK_DAYS,
        "minimum_days": MINIMUM_DAYS,
        "interval": INTERVAL,
        "runner": "app.run_daytrade_backtest(backtest_mode=True; production daytrade execution/data path; strategy-only compute patch)",
        "runner_note": "The strategy is normally registered in the scalp engine; this BT uses the user-required daytrade runner with a strategy-only signal patch.",
        ...
        "bt_error_data_source_note": "run_daytrade_backtest omits data_source/bars_fetched when it returns the <20 trades error; console output should show massive-parquet/15m and missing_caches must be empty.",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless SQUEEZE_REDESIGN_V2=1; shadow emit also requires SQUEEZE_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed_bar_actual_breakout_v2 variant is evaluated.",
            "bt_source_guard": "PASS: BT_REQUIRE_MASSIVE_CACHE=1 prevents Yahoo/network fallback for this report.",
            "runner_note": "PASS: user-required run_daytrade_backtest path is used with a strategy-only compute patch.",
        },
    }
