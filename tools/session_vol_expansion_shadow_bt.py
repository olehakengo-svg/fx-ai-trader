#!/usr/bin/env python3
"""A/B BT filter for session_vol_expansion V2 range redesign."""
...
FLAG = "SESSION_VOL_EXPANSION_REDESIGN_V2"
SHADOW_FLAG = "SESSION_VOL_EXPANSION_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results/session_vol_expansion-shadow-redesign-v2-2026-05-05.json"


def _compute_sve_only_signal(df, tf, sr_levels, symbol="EURUSD=X",
    ...
    cand = SessionVolExpansion().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["session_vol_expansion no signal"],
            "atr": atr7,
            "mode": "scalp",
    ...
    return {
        ...
        "reasons": ["✅ session_vol_expansion strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr7,
        "mode": "scalp",
        "layer_status": {},
...
class _SveOnlyScalperEngine:
    SHADOW_ALWAYS_STRATEGIES = frozenset()

    def __init__(self):
        from strategies.scalp.session_vol_expansion import SessionVolExpansion
        self.strategies = [SessionVolExpansion()]
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_scalp_signal = _compute_sve_only_signal
        scalp_mod.ScalperEngine = _SveOnlyScalperEngine
        app.ScalperEngine = _SveOnlyScalperEngine
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "closed_prior_range_breakout_v2",
        "flag": FLAG,
        ...
        "runner_note": "session_vol_expansion is registered in the scalp engine; run_daytrade_backtest does not evaluate this strategy.",
        ...
        "bt_error_data_source_note": "run_scalp_backtest may omit data_source/bars_fetched when it returns the <20 trades error; missing_caches must be empty.",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless SESSION_VOL_EXPANSION_REDESIGN_V2=1; shadow emit also requires SESSION_VOL_EXPANSION_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed_prior_range_breakout_v2 variant is evaluated.",
            "bt_source_guard": "PASS: BT_REQUIRE_MASSIVE_CACHE=1 prevents Yahoo/network fallback for this report.",
            "runner_note": "PASS: production scalp runner is used because this is a scalp-engine strategy; daytrade runner is not an executable evaluator for session_vol_expansion.",
        },
    }
