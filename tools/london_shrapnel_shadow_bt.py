#!/usr/bin/env python3
"""A/B BT filter for london_shrapnel V2 sweep/reclaim redesign."""
...
FLAG = "LONDON_SHRAPNEL_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "LONDON_SHRAPNEL_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "london_shrapnel-shadow-redesign-v2-2026-05-05.json"


def _compute_london_shrapnel_signal(df, tf, sr_levels, symbol="EURUSD=X",
    ...
    cand = LondonShrapnel().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["london_shrapnel no signal"],
            "atr": atr7,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ london_shrapnel strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr7,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_london_shrapnel_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "closed_bar_sweep_reclaim_bounded_target_v2",
        "flag": FLAG,
        "shadow_worker_flag": SHADOW_PROMOTE_FLAG,
        "lookback_days": LOOKBACK_DAYS,
        "minimum_days": MINIMUM_DAYS,
        "interval": INTERVAL,
        "runner": "app.run_daytrade_backtest(backtest_mode=True; production daytrade path; london_shrapnel strategy-filter compute patch)",
        "runner_note": "london_shrapnel is a scalp-engine strategy; the required daytrade runner is used with a strategy-filter compute patch so production BT execution, friction, cooldown, and exit path are exercised.",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless LONDON_SHRAPNEL_REDESIGN_V2=1; shadow emit also requires LONDON_SHRAPNEL_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered sweep/reclaim V2 variant is evaluated.",
