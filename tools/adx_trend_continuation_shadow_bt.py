#!/usr/bin/env python3
"""A/B BT filter for adx_trend_continuation V2 timing hardening."""
...
FLAG = "ADX_TREND_CONTINUATION_REDESIGN_V2"
OUTFILE = ROOT / "bt-results" / "adx_trend_continuation-shadow-redesign-v2-2026-05-05.json"


class _AdxOnlyDaytradeEngine:
    SHADOW_ALWAYS_STRATEGIES = frozenset()

    def __init__(self):
        from strategies.daytrade.adx_trend_continuation import AdxTrendContinuation
        self.strategies = [AdxTrendContinuation()]
...
def _compute_adx_only_signal(df, tf, sr_levels, symbol="EURUSD=X",
    ...
    cand = AdxTrendContinuation().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["adx_trend_continuation no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ adx_trend_continuation strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
...
def main() -> int:
    ...
    if missing:
        ...
        daytrade_mod.DaytradeEngine = _AdxOnlyDaytradeEngine
        app.compute_daytrade_signal = _compute_adx_only_signal
    ...
    result = {
        ...
        "flag": FLAG,
        "shadow_worker_flag": "ADX_TREND_CONTINUATION_REDESIGN_V2_SHADOW_PROMOTE",
        "lookback_days": LOOKBACK_DAYS,
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless ADX_TREND_CONTINUATION_REDESIGN_V2=1.",
