#!/usr/bin/env python3
"""A/B BT filter for pullback_to_liquidity_v1 V2 fixed-pip touch redesign."""
...
FLAG = "PULLBACK_TO_LIQUIDITY_V1_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "PULLBACK_TO_LIQUIDITY_V1_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "pullback_to_liquidity_v1-shadow-redesign-v2-2026-05-05.json"


def _compute_p2l_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    if bar_time is None:
        bar_time = df.index[-1]
    is_jpy_scale = "JPY" in symbol.upper() or "XAU" in symbol.upper()
    ...
    ctx = SignalContext(
        ...
        pip_mult=100 if is_jpy_scale else 10000,
        df=df,
        sr_levels=sr_levels,
    ...
    cand = PullbackToLiquidityV1().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["pullback_to_liquidity_v1 no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["pullback_to_liquidity_v1 strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_p2l_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "fixed_pip_liquidity_touch_v2",
        "flag": FLAG,
        ...
        "target_scope_note": "pre-reg Phase 3.A pair set: USD_JPY, EUR_USD, GBP_USD",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy redesign is default-off unless PULLBACK_TO_LIQUIDITY_V1_REDESIGN_V2=1; shadow emit also requires PULLBACK_TO_LIQUIDITY_V1_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-specified fixed_pip_liquidity_touch_v2 variant is evaluated.",
