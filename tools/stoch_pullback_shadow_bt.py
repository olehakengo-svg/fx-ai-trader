#!/usr/bin/env python3
"""A/B BT filter for stoch_pullback V2 closed-bar confirmation redesign."""
...
FLAG = "STOCH_PULLBACK_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "STOCH_PULLBACK_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "stoch_pullback-shadow-redesign-v2-2026-05-05.json"


def _compute_stoch_pullback_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = StochTrendPullback().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["stoch_pullback no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ stoch_pullback strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def _criteria(current: dict, proposed: dict) -> dict:
    if proposed["N"] < 20:
        return {
            ...
            "reason": "proposed BT trades < 20; v2.1 skips catastrophic check and recommends shadow measurement",
    ...
    if err:
        return {
            ...
            "catastrophic_check": "SKIPPED",
            "sanity_floor": "REMOVED",
            "shadow_promote_recommendation": "IMPLEMENT_FLAG_ONLY_DEFAULT_OFF",
        }
    ...
    return {
        **checks,
        ...
        "sanity_floor": "REMOVED",
        "warn_only": {
            "pf_change": checks["pf_change"],
            "wilson_lo_change": checks["wilson_lo_change"],
            "n_change_pct": checks["n_change_pct"],
        },
        "verdict": verdict,
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_stoch_pullback_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "closed_bar_confirmation_dedup_v2",
        "flag": FLAG,
        ...
        "target_scope_note": "strategy-filter BT targets USD_JPY first because the audit identifies it as the only N>=30 pair bucket with weak positive fit before closed-bar redesign; cross-pair shadow observation remains out of scope.",
        "missing_caches": missing,
        "lock_spec": {
            "bt_evidence_threshold": "if proposed N < 20 => INSUFFICIENT_BT_EVIDENCE, skip catastrophic, recommend shadow",
            "catastrophic_check": {
                "pnl_sign_preserved_only": "baseline PnL > 0 and proposed PnL < 0 is the only REJECT trigger",
            },
        ...
        "self_review": {
            "catastrophic_only": "PASS: verdict ignores PF, Wilson, N drop, positive-direction and Kelly; applies only pnl_sign_preserved when N>=20.",
            "insufficient_bt_evidence": "PASS: proposed N<20 becomes INSUFFICIENT_BT_EVIDENCE and shadow recommendation.",
            "production_live_safety": "PASS: strategy behavior is default-off unless STOCH_PULLBACK_REDESIGN_V2=1; shadow emit also requires STOCH_PULLBACK_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed_bar_confirmation_dedup_v2 variant is evaluated.",
