#!/usr/bin/env python3
"""A/B BT filter for mtf_counter_trend_scalp redesign V2.1."""
...
FLAG = "MTF_COUNTER_TREND_SCALP_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "MTF_COUNTER_TREND_SCALP_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "mtf_counter_trend_scalp-shadow-redesign-v2-2026-05-05.json"


def _cache_path(pair: str, interval: str = INTERVAL) -> Path:
...
def _stats(result: dict) -> dict:
    ...
    return {
        ...
        "EV": round(sum(pnls) / n, 4) if n else 0.0,
...
def _build_ctx(df, tf, sr_levels, symbol, backtest_mode, bar_time, htf_cache):
    ...
    htf = htf_cache.get("mtf_counter_trend_scalp_htf", {}) if htf_cache else {}
...
def _compute_strategy_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = MtfCounterTrendScalp().evaluate(ctx)
...
def main() -> int:
    ...
    missing = [
        str(_cache_path(pair, tf).relative_to(ROOT))
        for pair, tf in required
        if not _cache_path(pair, tf).exists()
    ]

    if missing:
        for pair, _symbol in TARGETS:
            err = "missing MASSIVE parquet cache: " + ", ".join(missing)
            cells[pair] = {
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "closed_m1_signal_bar_next_bar_execution_v2",
        "flag": FLAG,
        ...
        "data_source_required": "data/cache/massive/{PAIR}_{TF}.parquet for 1m/5m/15m with BT_MODE=1 and BT_REQUIRE_MASSIVE_CACHE=1",
        ...
        "shadow_promote_recommendation": (
            "RECOMMEND_SHADOW" if overall in {"PASS", "INSUFFICIENT_BT_EVIDENCE"}
            else "IMPLEMENTATION_ONLY_DEFAULT_OFF" if overall == "BLOCKED_DATA"
            else overall
        ),
        ...
        "self_review": {
            ...
            "post_hoc_adjustment": "PASS: only the pre-registered timing/data-contract variant is evaluated.",
