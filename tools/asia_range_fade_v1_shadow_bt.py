#!/usr/bin/env python3
"""A/B BT light filter for asia_range_fade_v1 V2 timing hardening."""
...
FLAG = "ASIA_RANGE_FADE_V1_REDESIGN_V2"
OUTFILE = ROOT / "knowledge-base/raw/bt-results/asia_range_fade_v1-shadow-bt-2026-05-05.json"


class _AsiaRangeOnlyDaytradeEngine:
    """BT strategy filter equivalent to strategies=['asia_range_fade_v1']."""

    SHADOW_ALWAYS_STRATEGIES = frozenset()

    def __init__(self):
        from strategies.daytrade.asia_range_fade_v1 import AsiaRangeFadeV1
        self.strategies = [AsiaRangeFadeV1()]
...
def _compute_asia_range_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = AsiaRangeFadeV1().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["asia_range_fade_v1 no signal"],
            "atr": atr,
            "mode": "daytrade",
            "indicators": {"rsi": ctx.rsi},
        }
    return {
        "signal": cand.signal,
        "entry": entry,
        ...
        "reasons": ["✅ asia_range_fade_v1 strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        daytrade_mod.DaytradeEngine = _AsiaRangeOnlyDaytradeEngine
        app.compute_daytrade_signal = _compute_asia_range_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "closed_prior_range_v2",
        "flag": FLAG,
        "lookback_days": days,
        "lookback_note": "90d used because available 15m MASSIVE caches are below full 365d coverage; this satisfies the pre-registered minimum.",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless ASIA_RANGE_FADE_V1_REDESIGN_V2=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed_prior_range_v2 variant is evaluated.",
