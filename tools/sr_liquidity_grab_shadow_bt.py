#!/usr/bin/env python3
"""A/B BT filter for sr_liquidity_grab V2 closed-bar timing contract."""
...
FLAG = "SR_LIQUIDITY_GRAB_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "SR_LIQUIDITY_GRAB_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "sr_liquidity_grab-shadow-redesign-v2-2026-05-05.json"


def _compute_sr_liquidity_grab_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = SrLiquidityGrab().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["sr_liquidity_grab no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ sr_liquidity_grab strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_sr_liquidity_grab_only_signal
    ...
    result = {
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless SR_LIQUIDITY_GRAB_REDESIGN_V2=1; shadow emit also requires SR_LIQUIDITY_GRAB_REDESIGN_V2_SHADOW_PROMOTE=1.",
