#!/usr/bin/env python3
"""A/B BT filter for ema_cross V2 closed-bar timing hardening."""
...
INTERVAL = "15m"
STRATEGY = "ema_cross"
FLAG = "EMA_CROSS_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "EMA_CROSS_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "ema_cross-shadow-redesign-v2-2026-05-05.json"


def _compute_ema_cross_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = EmaCross().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["ema_cross no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ ema_cross strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
...
def main() -> int:
    ...
    if missing:
        ...
        from strategies.daytrade.ema_cross import EmaCross  # noqa: E402

        app.compute_daytrade_signal = _compute_ema_cross_only_signal
        _orig_1h_adx = EmaCross._compute_1h_adx

        def _bt_fast_1h_adx(self, df_15m):
            return _orig_1h_adx(self, df_15m.tail(240))

        EmaCross._compute_1h_adx = _bt_fast_1h_adx
    ...
    result = {
        ...
        "target_scope_note": "audit redesign recommendation names USD_JPY as the only existing tail with shape; full 6-pair ALL runner was attempted but did not complete in practical time because ema_cross computes 1H ADX by per-bar resample inside the production strategy.",
        "bt_performance_note": "production run_daytrade_backtest is used; the strategy-only BT patch limits ema_cross 1H ADX helper input to the latest 240 M15 bars for both baseline and proposed so the A/B catastrophic comparison can complete under identical conditions.",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless EMA_CROSS_REDESIGN_V2=1; shadow emit also requires EMA_CROSS_REDESIGN_V2_SHADOW_PROMOTE=1.",
