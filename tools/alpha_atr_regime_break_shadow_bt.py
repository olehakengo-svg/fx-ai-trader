#!/usr/bin/env python3
"""A/B BT filter for alpha ATR regime break V2 shadow redesign."""
...
MINIMUM_DAYS = 365
INTERVAL = "15m"
STRATEGY = "atr_regime_break"
FLAG = "ALPHA_ATR_REGIME_BREAK_REDESIGN_V2"
SHADOW_FLAG = "ALPHA_ATR_REGIME_BREAK_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "alpha_atr_regime_break-shadow-redesign-v2-2026-05-05.json"


def _compute_atr_regime_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = AtrRegimeBreak().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["atr_regime_break no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ atr_regime_break strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_atr_regime_only_signal
    ...
    result = {
        ...
        "data_source_note": "run_daytrade_backtest omits data_source/bars_fetched when it returns the <20 trades error; console output showed massive-parquet/15m for each target.",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless ALPHA_ATR_REGIME_BREAK_REDESIGN_V2=1; shadow emit also requires the separate shadow flag.",
