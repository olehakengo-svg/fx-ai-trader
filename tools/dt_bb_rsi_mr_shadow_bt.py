#!/usr/bin/env python3
"""A/B BT light filter for dt_bb_rsi_mr V2 closed-bar timing hardening."""
...
FLAG = "DT_BB_RSI_MR_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "DT_BB_RSI_MR_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "dt_bb_rsi_mr-shadow-redesign-v2-2026-05-05.json"


def _compute_dt_bb_rsi_mr_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = DtBbRsiMR().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["dt_bb_rsi_mr no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ dt_bb_rsi_mr strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_dt_bb_rsi_mr_only_signal
    ...
    result = {
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless DT_BB_RSI_MR_REDESIGN_V2=1; shadow emit also requires DT_BB_RSI_MR_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the pre-registered closed_bar_signal_v2 variant is evaluated.",
            "bt_source_guard": "PASS: BT_REQUIRE_MASSIVE_CACHE=1 prevents Yahoo fallback for 15m price data; each target reports data_source=massive-parquet.",
        },
    }
