#!/usr/bin/env python3
"""A/B BT filter for ema_trend_scalp V2 moderate-trend redesign."""
...
FLAG = "EMA_TREND_SCALP_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "EMA_TREND_SCALP_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "ema_trend_scalp-shadow-redesign-v2-2026-05-05.json"


def _compute_ema_trend_scalp_only_signal(
    df,
    tf,
    sr_levels,
    symbol="GBPJPY=X",
    ...
    if cand is None:
        return {
            ...
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["ema_trend_scalp no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["ema_trend_scalp strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_ema_trend_scalp_only_signal
    ...
    result = {
        ...
        "strategy": STRATEGY,
        "variant": "moderate_trend_gate_no_strong_adx_bonus_v2",
        "flag": FLAG,
        ...
        "target_scope_note": "W4 audit identifies GBP_JPY as the consistent positive redesign candidate; this filter BT does not attempt ALL-strategy revival.",
        ...
        "self_review": {
            ...
            "post_hoc_adjustment": "PASS: only the pre-registered moderate_trend_gate_no_strong_adx_bonus_v2 variant is evaluated.",
