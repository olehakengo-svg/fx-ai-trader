...
mkdir -p .ai/tasks/queue

STAMP="$(date +%Y%m%d-%H%M)"
ISO="$(date +%Y-%m-%dT%H:%M:%S%z)"
TASK=".ai/tasks/queue/${STAMP}-gate0-production-safety-audit.md"

if find .ai/tasks/queue -maxdepth 1 -type f -name '*.md' ! -name '.gitkeep' | grep -q .; then
  echo "A queued task already exists:" >&2
  find .ai/tasks/queue -maxdepth 1 -type f -name '*.md' ! -name '.gitkeep' -print | sort >&2
  exit 0
fi

cat > "$TASK" <<EOF
---
id: ${STAMP}-gate0-production-safety-audit
...
echo "Created bootstrap task: $TASK"
echo "Run it with: ./tools/ai_run_codex.sh"
