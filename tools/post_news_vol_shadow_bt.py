#!/usr/bin/env python3
"""A/B BT filter for post_news_vol V2 shadow redesign."""
...
FLAG = "POST_NEWS_VOL_REDESIGN_V2"
SHADOW_FLAG = "POST_NEWS_VOL_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "post_news_vol-shadow-redesign-v2-2026-05-05.json"


def _compute_post_news_only_signal(df, tf, sr_levels, symbol="USDJPY=X",
    ...
    cand = PostNewsVol().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["post_news_vol no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ post_news_vol strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {"layer1": ctx.layer1},
        "regime": {},
        "indicators": {"adx": ctx.adx},
        "shadow_emit_signals": [],
    }
...
def main() -> int:
    ...
    if missing:
        ...
        app.compute_daytrade_signal = _compute_post_news_only_signal
    ...
    result = {
        ...
        "strategy": "post_news_vol",
        "entry_type": STRATEGY,
        "variant": "event_window_closed_bar_continuation_wider_management_target_v2",
        "flag": FLAG,
        ...
        "event_calendar_note": "MASSIVE OHLCV cache has no economic-calendar event feed; proposed V2 therefore requires runtime ctx.session high-impact events and produced zero BT events where unavailable.",
        "bt_mode": os.environ.get("BT_MODE"),
        "massive_cache_verified": not missing,
        "targets": [pair for pair, _ in TARGETS],
        "targets_note": "Allowed post_news_vol pairs with available 15m MASSIVE cache; XAUUSD omitted because no XAUUSD_15m MASSIVE cache is present.",
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless POST_NEWS_VOL_REDESIGN_V2=1; shadow emit also requires the separate shadow flag.",
            "post_hoc_adjustment": "PASS: only the pre-registered event-window closed-bar V2 variant is evaluated.",
