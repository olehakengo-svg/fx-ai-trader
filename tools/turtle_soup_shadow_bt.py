#!/usr/bin/env python3
"""A/B BT filter for turtle_soup V2 closed-bar timing contract."""
...
FLAG = "TURTLE_SOUP_REDESIGN_V2"
SHADOW_PROMOTE_FLAG = "TURTLE_SOUP_REDESIGN_V2_SHADOW_PROMOTE"
OUTFILE = ROOT / "bt-results" / "turtle_soup-shadow-redesign-v2-2026-05-05.json"


def _compute_turtle_soup_only_signal(
    df, tf, sr_levels, symbol="GBPUSD=X", backtest_mode=False, bar_time=None, htf_cache=None
):
    ...
    ctx = SignalContext(
        ...
        bar_time=bar_time,
        hour_utc=hour_utc,
        is_friday=bar_time.weekday() == 4 if hasattr(bar_time, "weekday") else False,
    )
    cand = TurtleSoup().evaluate(ctx)
    if cand is None:
        return {
            "signal": "WAIT",
            "entry": entry,
            "entry_type": "wait",
            "reasons": ["turtle_soup no signal"],
            "atr": atr,
            "mode": "daytrade",
    ...
    return {
        ...
        "reasons": ["✅ turtle_soup strategy-filter BT"] + list(cand.reasons or []),
        "score": cand.score,
        "atr": atr,
        "mode": "daytrade",
        "layer_status": {},
...
def _criteria(current: dict, proposed: dict) -> dict:
    if proposed["N"] < 20:
        n_change = _change_pct(proposed["N"], current["N"])
        return {
            ...
            "warnings_only": {
                "n_change_pct": round(n_change, 4) if math.isfinite(n_change) else str(n_change),
                "pf_change": "WARN_ONLY",
...
def _blocked_xau_cell() -> dict:
    ...
    return {
        ...
        "lock_criteria": {
            ...
            "reason": "XAU_USD MASSIVE cache absent; implementation remains default-off until opt-in data exists",
...
def main() -> int:
    ...
    missing = [
        ...
        for pair, _ in RUNNABLE_TARGETS + [XAU_TARGET]
    ...
    app.compute_daytrade_signal = _compute_turtle_soup_only_signal

    for pair, symbol in RUNNABLE_TARGETS:
        if not _cache_path(pair).exists():
            err = f"missing MASSIVE parquet cache: {_cache_path(pair).relative_to(ROOT)}"
    ...
    cells[XAU_TARGET[0]] = _blocked_xau_cell()

    aggregate_current = _aggregate([cells[pair]["current"] for pair, _ in RUNNABLE_TARGETS])
    aggregate_proposed = _aggregate([cells[pair]["proposed"] for pair, _ in RUNNABLE_TARGETS])
    ...
    result = {
        ...
        "targets": [pair for pair, _ in RUNNABLE_TARGETS + [XAU_TARGET]],
        ...
        "self_review": {
            ...
            "production_live_safety": "PASS: strategy behavior is default-off unless TURTLE_SOUP_REDESIGN_V2=1; shadow emit also requires TURTLE_SOUP_REDESIGN_V2_SHADOW_PROMOTE=1.",
            "post_hoc_adjustment": "PASS: only the audit-specified closed-bar timing + live dedup V2 variant is evaluated.",
