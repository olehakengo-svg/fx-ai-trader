...
## Proposal B — 730d Health Audit: ⏸ deferred

**状態**: PID 11016 / 2h 24m 経過時点で OANDA 15m data fetch phase (page 10/~20 × 1/6 pair)。完了には 5h+ 追加必要と判定し、本セッションでは **deferred**。

### 代替カバレッジ（B が狙った検証は E-w90 で概ね達成）
- ELITE_LIVE/PAIR_PROMOTED の "2x 期間" 安定性 → E の **w90 (4 windows / 長期平滑)** で近似カバー
- session_time_bias × USD_JPY: w60 で stable 昇格（w90 で `session_time_bias × EUR_USD` が stable 入り、USD_JPY は未掲載）
- wick_imbalance_reversion × USD_JPY: w90 stable に含まれず → **強度は borderline 止まり** と推定
- london_fix_reversal × GBP_USD: w90 結果に登場せず → **要再検証（B 完了時に優先チェック）**

### 次セッション着手時
- `walkforward-730d-2026-04-22.md` 生成確認 → 上記 3 targets を優先抽出
- 730d で stable → ELITE_LIVE 維持判定の根拠強化
- 730d で unstable → PAIR_PROMOTED 降格候補フラグ（ただし実装判断は独立監査経由）

想定対象: `session_time_bias × GBP_USD`, `london_fix_reversal × GBP_USD`, `wick_imbalance_reversion × USD_JPY`, ELITE_LIVE 全体の 2x データ安定性確認。

---

## Consolidated Insights (A+C+D+E)
...
- 次のアクション: window-invariant subset (w30∩w60∩w90) の 5 戦略を Live N≥30 で継続監視。KSFT × GBP_JPY は pair-specific filter 候補として Shadow 追跡。B (730d) は deferred → 次セッションで再実行。
...
- `raw/bt-results/walkforward-w7-2026-04-22.md`
- `raw/bt-results/walkforward-w60-2026-04-22.md`
- `raw/bt-results/walkforward-w90-2026-04-22.md`
