...
| **F** | Session-decomposition BT (Tokyo/London/NY) — 4th dimension scan | ✅ | `raw/bt-results/session-zoo-2026-04-23.md` |
...
## Proposal F — Session-Decomposition BT (Tokyo / London / NY)

**実行結果**: `raw/bt-results/session-zoo-2026-04-23.md` 生成完了（2026-04-23 03:56 UTC）

- **目的**: 前セッションでユーザーが指摘した「Tokyo 時間の OANDA 稼働が低い」問題に対し、session 別 edge の有無を定量化する。
- **手法**: `app.run_daytrade_backtest` の trade_log を entry_time の UTC hour で post-hoc 分類。Tokyo=0-7 / London=7-13 / NY=13-21 / Off=21-24。

### 🌐 Portfolio Session Aggregate（全戦略 × 全 pair）
...
**発見**: Tokyo session は N share が最低（17.5%）にも関わらず **weighted EV は London と同等以上 (+0.273 vs +0.256)**。NY は最悪（+0.123）。
→ Tokyo 稼働低下は **edge 欠如ではなく、戦略カバレッジ不足**。

### 🗼 Tokyo-Superior Edges（Tokyo EV > London & NY）
| Pair | Strategy | Tokyo N | Tokyo EV | London EV | NY EV | 備考 |
|------|----------|--------:|---------:|----------:|------:|------|
| **USD_JPY** | **vwap_mean_reversion** | 30 | **+1.71** (WR=80%) | +0.44 | +1.37 | 最高 EV cell |
| **USD_JPY** | **streak_reversal** | 104 | **+1.55** | +1.18 | +1.32 | 全 session 黒字、Tokyo 最良 |
| **GBP_USD** | **gbp_deep_pullback (ELITE_LIVE)** | 25 | **+1.08** | +0.23 | +0.36 | **Tokyo で 3-5× edge** |
| **EUR_JPY** | **vwap_mean_reversion** | 54 | **+1.03** | +0.53 | +0.69 | Tokyo 最良 |
| GBP_JPY | intraday_seasonality | 11 | +0.86 | -0.03 | +0.01 | 小 N |
| GBP_JPY | dt_fib_reversal | 16 | +0.70 | -0.01 | +0.31 | 小 N |
| USD_JPY | intraday_seasonality | 16 | +0.56 | -0.41 | +0.21 | 小 N |

### 🟥 Tokyo-Only Edges（London/NY で EV ≤ 0、Tokyo のみ正）
| Pair | Strategy | Tokyo N | Tokyo EV | London EV | NY EV |
|------|----------|--------:|---------:|----------:|------:|
| **EUR_JPY** | **sr_break_retest** | 41 | **+0.37** | -0.07 | -0.23 |
| **GBP_USD** | doji_breakout | 10 | +0.45 | -0.53 | -0.07 |
| **GBP_JPY** | dt_sr_channel_reversal | 46 | +0.14 | -0.12 | -0.32 |
| USD_JPY | vol_spike_mr | 17 | +0.09 | -0.40 | -0.17 |
| **USD_JPY** | **session_time_bias** | 342 | +0.17 | — | — | Tokyo 専用設計 |

### 判断
- **Tokyo silence の根本原因は戦略設計問題**: Share 17.5% は portfolio の Tokyo coverage 不足を定量的に裏付ける。edge が無いからではなく、London/NY 偏重の戦略群しかロードされていない。
- **gbp_deep_pullback × GBP_USD の Tokyo 3-5× edge** は ELITE_LIVE の既存評価を補強する新事実。ただし N=25 なので別期間検証必須。
- **EUR_JPY × sr_break_retest の Tokyo-only edge** (N=41, EV=+0.37) は現在 PAIR_PROMOTED にも入っていない。Shadow 監視候補。
- **vwap_mean_reversion の USD_JPY/EUR_JPY JPY-pair cell で Tokyo EV が突出** → JPY pair の Tokyo session 特性（東京中銀関与＋低ノイズ）と整合的。
- **session_time_bias × USD_JPY**: 730d では unstable だが、本スキャンで設計通り Tokyo のみ発火を確認（設計の意図は保たれている）。

### 次ステップ
- 別期間 (w60 walk-forward + 730d) で session-decomposition を再検証（本スキャンは 365d 一発 → lesson-reactive-changes）。
- Tokyo 稼働向上施策: 既存戦略の Tokyo cell の Shadow N≥30 監視、ないし Tokyo 専用戦略の設計検討（roadmap-v2.1 の scalp 枝 1m/5m と合わせて）。
- EUR_JPY × sr_break_retest の Tokyo-only edge を独立 N=30 で再検証。

---

## Consolidated Insights (A+B+C+D+E+F 全完了)
...
8. **🗼 Tokyo session は portfolio で under-weighted だが per-trade edge は最高水準** (F) — N share 17.5%、Weighted EV=+0.273（London +0.256 / NY +0.123 を上回る）。Tokyo 稼働低下は edge 欠如ではなく戦略カバレッジ不足。
9. **gbp_deep_pullback × GBP_USD (ELITE_LIVE) の Tokyo EV は +1.08、London/NY の 3-5 倍** (F) — 既存 ELITE_LIVE 評価を補強する新事実。Tokyo 時間の限定リスク管理で効率 2-3× 可能性。
10. **EUR_JPY × sr_break_retest の Tokyo-only edge (N=41 EV=+0.37)** (F) — 現在 Tier 未登録、London -0.07 / NY -0.23 で負。Shadow 監視候補。
...
- `tools/bt_session_zoo.py` — 新規 (Proposal F, session-decomposition scanner)
...
- `raw/bt-results/walkforward-730d-2026-04-22.md` (B)
- `raw/bt-results/session-zoo-2026-04-23.md` (F) ← 本セッション完了
