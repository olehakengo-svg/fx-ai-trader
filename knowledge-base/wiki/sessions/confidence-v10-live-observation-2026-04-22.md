...
### v11.1 追加タグ (2026-04-22 08:15 UTC+)

観測中に発見した未タグ戦略を追加：

| 戦略 | 場所 | strategy_type | 根拠 |
|---|---|---|---|
| vwap_mean_reversion (dt) | app.py L3173-3229 | MR | 命名・設計とも MR (VWAP±2σ回帰) |
| vwap_mean_reversion (sc) | app.py L8257-8308 | MR | 同上、scalp variant |
| wick_imbalance_reversion | daytrade/alpha_wick_imbalance.py | MR | Osler 2003 ヒゲ偏り反発 |
| sr_break_retest | daytrade/sr_break_retest.py | pullback | docstring 明記「ブレイク→リテスト→継続」 |

`app.py` にインラインで実装されていた vwap_mean_reversion に対して `_v2_apply_penalty()` 経由で MR penalty 適用。ペナルティ発火時は `🔧 [v2] MR anti-trend: ADX=X.X>25 → conf A→B` marker を出力。
