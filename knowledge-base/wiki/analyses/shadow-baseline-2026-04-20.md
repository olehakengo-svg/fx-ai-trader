...
## ⚠️ 警告マーカー (2026-04-20 Phase 4 追記)

以下の "Phase 2 追記" セクションの `edge`, `Kelly`, `Strategy-level Kelly` などの数値は **all-time data** から算出された可能性が高く、`_FIDELITY_CUTOFF=2026-04-16` 適用後の真の Kelly 基準データではありません。真の post-cutoff Live pool は **N=14** しかなく (2026-04-20 時点)、Kelly 算出に必要な N≥20 に未達です。

詳細は [[lesson-all-time-vs-post-cutoff-confusion]] 参照。以下の "negative edge narrative" は不確か、参考情報として残すが意思決定根拠には使わないこと。
