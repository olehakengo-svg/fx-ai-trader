...
|----------|------|---|-----|-----------|-----------------------|------|
| ema_trend_scalp | USD_JPY | 125 | 20.8% | 0.00 | FORCE_DEMOTED + PAIR_DEMOTED | Level A 該当済 |
...
| bb_squeeze_breakout | USD_JPY | 35 | 31.4% | 0.02 | **PAIR_PROMOTED** (Tier 1 Live) | **例外** — Live と shadow 並存。下記 §2.1 |
...
全 11 cell が Bayesian posterior で **「BEV を超える確率 < 10%」**。

**内訳**:
- 9 cell: 既に FORCE_DEMOTED + ペア別 PD 済で Level A 追加アクション不要
- 2 cell（bb_rsi_reversion × USD_JPY / EUR_USD）: SCALP_SENTINEL 意図的 shadow、継続
- 1 cell（bb_squeeze_breakout × USD_JPY）: **例外ケース** — 次節参照

### 2.1 bb_squeeze_breakout × USD_JPY の例外扱い

現 tier: **PAIR_PROMOTED (Tier 1 Live)**。しかし Bayesian 出力（is_shadow=1, post-cutoff）で N=35 WR=31.4% と failing。

- `pair-promoted-candidates-2026-04-20.md` より、同戦略は cutoff 別で shadow 主体に流れる傾向
  - 2026-04-14 cutoff: (shadow/live)=(40/6) EV=+0.77pip PF=1.35
  - 2026-04-17 cutoff: (shadow/live)=(21/0) +84.7pip PF=3.38（Live N=0）
- **矛盾点**: 同じ shadow データで別分析 EV=+4.0 ↔ 本ツール WR=31% FD_FLAG
- **原因候補**:
  1. mode 混在（scalp 5m / daytrade 15m 別経路）
  2. PAIR_PROMOTED の Live 実装経路と観察 shadow 経路の分離
  3. 集計ウィンドウ差（cutoff 日付差）
- **処遇**: 止血ルール適用 **保留**。調査タスクとして残す
- **調査項目**: `tools/bayesian_edge_check.py` に mode 別 breakdown を追加 →
  Live と shadow の cell 相関を確認
...
- [DECISION 2026-04-21]: 本日の 11 FD_FLAG 内訳:
  - 9 cell は既に FORCE_DEMOTED + ペア別 PAIR_DEMOTED 済 → Level A 該当済、追加アクション不要
  - 2 cell（bb_rsi_reversion × USD_JPY/EUR_USD）は SCALP_SENTINEL 意図的 shadow → 継続
  - 1 cell（bb_squeeze_breakout × USD_JPY）は PAIR_PROMOTED と shadow 観察の **並存矛盾** → 止血適用保留、mode 別調査タスク化
- [DECISION 2026-04-21]: Level B (shadow 全停止) 判定は **未実施**。3週連続の posterior 確認が必要
- [UNRESOLVED 2026-04-21]: `tools/bayesian_edge_check.py` に mode 別 breakdown を追加し、PAIR_PROMOTED Live 経路と shadow 経路の分離を可視化する
