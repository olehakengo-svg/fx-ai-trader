...
- **非 claim の記述所見** (事後スライス、昇格根拠に使用禁止): 保有 <1h (N=115) は −81k・WR19%、
  ...
  **forward 明細の継続提供 + 観測前凍結プロトコルの新 family** のみが正規経路 (月次スキャンで再評価)。**再入場の起点 = user が chat に明細 (CSV) を添付した時点** — 受領したセッションがその場で新 family を起票し観測前凍結してから測定する (user へは月 1 回の提供を依頼済み)
