...
訂正は旧文を置き換え、「2026-09-22 訂正、旧記述は誤り」を併記した (撤回は旧文の横ではなく旧文を置き換える — [[lesson-unscoped-global-replace-2026-09-18]])。L3 以外で触ったのは「Live 実績」節の見出し (09-22 更新の追記) と上表の broker 関連 3 行 + 直下の注記 2 件のみ (下記)。カード上表の「broker 実測 N=7 / 未集計」行は **同 PR (レビュー P2 対応) で 14/14 値 (+¥793 = +79.3p、EV +¥56.6 = +5.66p) に置換**し、broker-corrected 推定行 (+78.4p) には superseded 表示を付け (数値は歴史記録として残置)、09-17 注記「7/11 のみ突合・demo 3 行は broker 未観測」には superseded 前置きを付けた。上表直下に 09-22 の置換注記 (decided WR の scratch 定義差を含む) を追加。レビュー 2 巡目 (Codex P2 ×2) で、同カードの歴史ブロックにも superseded を明記した: 09-07 log (「11 本全て未突合」)、確認済み fill 列挙 (11 本 → #549260 / #573986 / #677396 を補完し 14 本)、09-11 節 (「13 本全件未実施 / +46.3 未監査」)、09-16 §(4) (「TP は ~65p 固定・SL 側だけ置換」→ §5 の QUICK_HARVEST ×0.85 機構で TP も置換と明記)、09-16 §(6) (「7/11・−41.1p 符号逆・+78.4 未監査」→ 未特定 4 本 + 未列挙 3 本の broker 値と 14/14 合計を併記)。旧文は削除せず「09-16 時点の記録」として残置。3 巡目 (Codex P2 ×1) で残っていた現在形の 3 箇条 (判定保留の「broker 基準 WR 50.0%」/「+78.4 へ回復」/ 09-09 positive control の「全件未実施・未監査」) にも superseded を付け、14/14 の値 (WR 57.1% 符号 / 40.0% decided、+¥793 = +79.3p、正値 4 本 +¥1,633 / 残 10 本 −¥840) を併記した。
