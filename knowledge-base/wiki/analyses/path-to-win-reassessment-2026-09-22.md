...
| 段 | 定義 | 今日の答え |
|---|---|---|
| (a) M2 到達 | +0.5%/月 NAV 実現 (rederivation L44)、L0→L1 昇格を含む | **30 日で勝てる手は無い**。time-to-M2 は現行定義 × 現 NAV × $0 施策で ∞ (§2)。有限化の前提は user 決裁 U3 入金 (D3 (i)、NAV ≥ ¥300k で L1 算数が閉じる) で、D1 会計定義は有限化後のサイズ (A 5〜8 本 / B 2〜4 本 @¥300k) を変えるだけ (訂正 PR #279 P2: D3 ∧ D1 の conjunction ではない)。有限化後の時計も G3 (live N≥30) が binding で 2028 年台 |
...
15. 制約 4.4 / 4.5 (証拠金) の多 leg 同時評価 — D3 の ¥300k vs ¥1,245,600 (旧 ¥900k、PR #279 P1 訂正) は 4.2 のみの計算、4.4/4.5 側は未計算。
