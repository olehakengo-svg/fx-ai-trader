...
### 原則0: 言ったことをやる (Say→Do→Verify)

```
自分で「Xする」と言った場合:
  1. 即座にXを実行する（次のタスクに移らない）
  2. Xが実行されたことを検証する
  3. 検証結果をログに書く
  ※ 「0.5秒でやる」と言って5分にするのは原則0違反
  ※ 「即時アクション」と言って数時間放置するのは原則0違反
```

**自動検知**: セッション内で「〜する」「〜に変更」と宣言した後、
次のコミットまでにその変更が含まれていなければ、
session logの未解決事項に自動追加すべき。
（現在は手動。フロー化の余地あり）
