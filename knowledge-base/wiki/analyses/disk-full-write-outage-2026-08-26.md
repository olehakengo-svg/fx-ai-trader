...
## 7. 追補 — フォローアップ (2026-08-26 同日、follow-up PR)

**§6 の「disk 増設は当面不要」は前提の見落としがあった。** あの判断は
reclaim 直後 (`backups: []`) の 23.61% を見て下したが、`keep_last=3` は
日次バックアップを 3 本まで積み直す。定常状態は main 204 + WAL 37 +
3×204 = **852 MB = 85.3%** で、**2026-08-29 の 3 本目作成時点から warn
75% を恒常的に超える**ことが計算上確定していた。容量を食う主因が
テーブル成長ではなくバックアップである以上 (§5.5: 解放 763 MB の 100%
がバックアップ由来)、是正もバックアップ側で行う:

1. **`keep_last` 3 → 2** (daily_review / `/api/db/backup` / デフォルト値)。
   定常 649 MB = 64.9% となり warn 帯に入らない。Render 自体が日次で
   ディスク全体のスナップショットを取る (7 日保持) ため、同一ディスク
   3 本目の DR 価値は無い
2. **`db_write_failed` 検知を新設 (write_probe)** — `/api/admin/disk_status`
   が応答時に実 INSERT (1 行 upsert) を実行し、その成否をそのまま返す。
   §5 の教訓「生存監視は永続化層に到達した最新レコードの時刻で行う」の
   実装。当初は main/WAL/SHM の mtime で staleness を測る案だったが、
   レビューの再現実験で二重に反証された: (a) 再起動時の checkpoint と
   WAL-index 再構築が mtime をリセットする (デプロイ実測 ~1.7 回/日)、
   (b) ENOSPC 下でも失敗する commit のリトライが確保済み WAL ブロックを
   上書きして mtime を前進させ続ける — つまり 08-21 事故では鳴らない。
   commit 成功の事実だけを信頼する方式なら検知遅延は 6h でなくポーリング
   間隔の 15 分。未使用だった `STALE_WRITE_HOURS` は削除した (staleness
   閾値そのものが不要になったため)
3. **`live_n_stagnation` の週末誤検知を修正** — 24h 閾値を実時間でなく
   市場オープン時間 (金 21:00 → 日 21:00 UTC の閉場を除外) で数える。
   実時間のままなら金曜夕方の最終取引が毎週末必ず誤発火していた
4. **Discord 通知に時間バケット抑制** — warn は 6h 毎 (≤4 通/日)、
   critical/db_write_failed/backup_blocked は 1h 毎。cron は状態を持てない
   ので「レベル変化時のみ」ではなく決定的バケットで上限を張る。
   **全イベントは毎回 stdout に 1 行 JSON で出す** — Render cron は
   クローンごと破棄されるため JSONL は本番に残らず、cron の stdout ログ
   が事実上の恒久記録になる (レビューで発覚: 当初「JSONL に記録済み」と
   案内していたが本番では虚偽だった)。既知の限界: バケット先頭の run が
   落ちるとそのバケットは無通知 (cron 失敗自体は Render notifyOnFail が
   通知する)。デプロイ直後の cron/web バージョン不一致による
   `write_probe_missing` は記録のみで Discord に流さない
5. dead code 除去 (`demo_db.backup_database` の到達不能な重複 except)

**残る user 決裁**: disk 1 GB → 5 GB 増設 (+$1.00/月、無停止、ダッシュ
ボード操作)。keep_last=2 でも main の正味成長 (~0.5〜2 MB/日) で
2〜5 ヶ月後には warn 帯に再接近するため、増設が恒久解。縮小不可なので
5 GB で止める (10 GB は不要)。
