...
- ✅ **供給スロットリング → 是正執行 (2026-08-11、user 決裁済み)**: HourlyEngine の winner-take-all × score 非対称 (DMB 5.0+ vs ps 1.0、crash bar で構造的共起) により family 供給が design の **~31%** (eur_aud/usd_cad は 0%) だった問題を、**(a) 席優先 select + (c) live feed MASSIVE 統一** で修正 ([[price-shock-seat-supply-audit-2026-07-29]] §9 執行記録)。displaced DMB/KSB の shadow 系列は shadow_always 経由で継続。30d 後に供給率再計測 (目標 capture ≥80%)。**是正以前の「発火数ゼロ/過少」を根拠にした席評価は引用前に再検討** (選抜層抑制のため)。WEEKEND_CLOSE は同 §10 で免除非推奨が確定 (初実射 14900 で +64.5p 保護的)
