...
- 🔴🔑 **2026-09-25 確定 (rule:R3、PR #299 review P1 4100250627): live は宣言 max hold 480 bars (~120h) を構造的に再現できない — 2 つの決定論的上限**
  ...
  - **処置分岐 (registry `kalman-d7-live-exit-spec-mismatch-disposition`、期日 2026-10-08)**: (a) **制約付き BT** — TV Pine (eval canon) に live exit スタック 6 経路 (8h cap / 金曜 21:45Z / ATR BE / ATR trail / 4h 含み損 TIME_DECAY / SIGNAL_REVERSE) を累積で入れて同期間で再計測 (Codex queue `20260925-0300-kalman-d7-live-constrained-bt`)。full stack の摩擦調整 EV ≤ 0 なら **R2 で shadow へ降格** (live は BT に無い戦略を走らせているのと同じ)。EV > 0 なら現状維持 + 参照 BT を差し替え — **ただし overlay を一部しか再現できない不完全なシミュレーションで「維持」は取れない** (不完全な結果が正当化できるのは保守側のみ)。(b) 宣言通りの保持 (override 120h + 週末保持) を live に導入するのは**新 live 挙動 = Rule 1** (週末ギャップ露出・1,000u でも SL 外決済リスク) で autopilot 執行禁止、user 決裁項目
