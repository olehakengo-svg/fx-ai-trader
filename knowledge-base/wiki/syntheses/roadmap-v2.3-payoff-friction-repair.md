...
> **📌 目標更新 (2026-07-10 user 承認)**: 目標は段階目標 M1→M2→M3 へ移行、21.6% は aspirational anchor に格下げ ([[monthly-target-rederivation-2026-07-10]] / [[shortest-path-decision-memo-2026-07-10]])。本ロードマップの WS 構成は不変 — 最短経路はトラックA (WS3 stage-2) / B (探索供給ライン = [[ws3-round2-explore-prereg-2026-07-10]]) / C (資本配管: agg-Kelly carve-out + 防御解除ラダー) の3並行。
