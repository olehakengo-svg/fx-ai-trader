...
| T6 | ⬛ **sweep_reversion_eurgbp_late (EUR_GBP) — 退役クローズ** (v2.2 T8 繰越) | **判定終了。新規の N 蓄積・EV 判定・期日監視はいずれも無効** (旧: HTF-rescued shadow N≥10 → EV 判定 / 2026-09-30 に N<5 → retire — 後者は N 単調増加のため到達不能だった) | ⬛ **2026-09-17: user 決裁 Option C = retire を採択** ([[ps1a-option-c-retire-2026-09-17]]、PR #263)。経緯 = 09-14 に unique N=10 到達 → 判定器 `OPTION_B_EXECUTE` (gross spaced +2.92p) だったが、凍結閾値 +6.22p が研究の **net-of-spread** 由来で shadow EV は mid fill の **gross** = estimand 不一致。同 estimand に揃えると **net spaced −3.33p (符号反転)**、実測 entry spread 5.4-16.6p で pre-reg 前提 (≤3.5p) 充足 **0/10**、**cap 救済集合も空** ([[ps1a-trigger-estimand-audit-2026-09-16]])。→ registry `t8-sweep-defer-decision` resolved / 判定器は `OPTION_C_RETIRED_USER` を恒久返却 / scheduled task 無効化 / shadow rescue のみ残置 (4原則#3)。⚠️ **旧「トリガ成立日に runbook §3 を直接執行」は falsified = 引用禁止**。再挑戦は net estimand の新規 pre-reg (Rule 1) のみ。**order 層 12-bar min-spacing 実装は前提から外れた** |
...
- **multi-bar cooldown の order 層実装** — ⬛ **不要化 (2026-09-17)**。唯一の要求元だった sweep (T6) が user 決裁で退役したため。現状 code pin OFF のまま。再挑戦の新規 pre-reg が要求したら再起票
