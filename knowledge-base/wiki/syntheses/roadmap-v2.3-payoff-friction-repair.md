...
| T6 | **sweep_reversion_eurgbp_late (EUR_GBP) DEFER 決定点** (v2.2 T8 繰越) | HTF-rescued shadow N≥10 → EV 判定 (R1) / 2026-09-30 に N<5 → retire (R2)。**復帰の追加前提 = order 層 12-bar min-spacing 実装** (検証 estimand との一致、[[t8-week1-gate-breach-2026-07-06]] forensic #3) | 🔴 **2026-09-16: トリガ成立 → 執行停止 (estimand 監査)**。09-14 に unique N=10 到達し判定器は `OPTION_B_EXECUTE` (gross spaced +2.92p) を返したが、凍結閾値 +6.22p は研究の **net-of-spread** 由来・shadow EV は mid fill の **gross** で estimand 不一致 → 同 estimand に揃えると **net spaced −3.33p (符号反転)**、摩擦 convention 4 通り中 3 通りで負。実測 entry spread 5.4-16.6p で **pre-reg 前提 (≤3.5p) 充足 0/10**、**cap 救済集合も空** (≤5.0p で N=0)。判定器は `USER_REDECISION_ESTIMAND` を返すよう修正し scheduled task の自動 live 昇格を停止。⚠️ N 単調増加のため 10-28 の `N<5 → retire` は**到達不能** = 期日自動終結なし。残る選択 = (a) Option C retire [推奨] / (b) 執行形態の再設計 (新規 pre-reg) = **user 決裁**。[[ps1a-trigger-estimand-audit-2026-09-16]] |
