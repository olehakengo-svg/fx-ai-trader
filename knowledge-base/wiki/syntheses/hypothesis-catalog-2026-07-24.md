...
| 21 | commodity_cross_range_mr (wave-6 EA-a) | **G0 完了 2026-08-03** — [[ea-landscape-sweep-2026-07-31]] §4.1 (GO score 63/56)。[[commodity-cross-g0-rt-freeze-2026-08-03]] (🔒 凍結コミット 981ae119 後に測定、fwd/シグナル非接触) | ✅ **G0 PASS 3/3** — stressed_RT_primary (p75+1.0p): AUD_NZD **3.80p** / AUD_CAD **3.70p** / NZD_CAD **3.90p** (全て ≤5.0p、conservative p90+2.0p でも 5.0-5.2p)。60 営業日 M5 BA candles n≈17.8k/ペア、アンカー妥当性 OK (USD_JPY p50 1.60p ≤ 限度 1.75p — 注: 全時間帯 bar-close 測定は friction table の典型値より系統的に広い、アンカー相対では整合)。**執行設計への副次発見 = 21:00 UTC 帯は毒 (p75 12-18p、D1 close/rollover 窓) だが 23:00 UTC 以降 2.7-2.9p に正常化 → explore pre-reg は執行 23:00 UTC 凍結を必須とする**。次 = explore pre-reg 起案 (12y MASSIVE、per-position 単発、純価格アンカー、M5/15m 禁止)。設計拘束の継続: per-position 単発のみ (grid/averaging 評価対象外)、v1 純価格アンカー限定 (金利差調整変種は E20 隣接で禁止)、M5/15m 形態禁止、session-mr-cross-wave1 (05-11 BLOCKED_DATA) を supersede、負の prior (「方向は合うが弱い」死型家系 + RBA/RBNZ デシンク窓 2014-15/2022-24 を OOS 敵対窓に必須組込)。`reports/commodity-cross-rt-g0-2026-08-03.md` **並行整備 (同日、8a2b135f): `tools/commodity_cross_g0_rt.py` + Render cron fx-ai-cc-g0-rt (日次 06:00 UTC) = verdict なしの継続サンプラー + financing/swap 日次収集 — explore pre-reg の stressed-net (swap 算入) への入力として稼働継続**。 **→ explore pre-reg 敵対的検証完了 2026-08-05 (5-lens workflow): GO-WITH-CONDITIONS (21 条件、12 blocking)** — payload `raw/analysis/wave6-cc-mr-explore-candidates-2026-08-05.json` / verdict `raw/analysis/wave6-cc-mr-adversarial-verification-2026-08-05.md`。要点: primary = D1 z-score onset fade (variant B = declared non-tested、FAIL 時クローズ範囲 = slow location-anchor band fade × multi-day × 3 クロス全 anchor 着せ替え・B 含むを事前凍結) / entry = 19:00 America/New_York bar close (catalog 字面 23:00 UTC からの偏差 on-record、DST 根拠) / pooled primary m=1 + gate G binding (§4.1 Bonferroni 行は on-record 修正) / 固定 2-ISO-週 block / swap = base−quote 3 式 + markup 実測 ~1.1%/yr + 0/0=MISSING / despike 規則 (AUD_NZD に 1,150p fake print 実測) / honest MDE 12-17p 公表義務。被覆修復: MASSIVE --days 4950 probe 成功 (AUD_NZD first bar 2013-01-13 ≤ 要求 2013-03-01、残 2 ペア取得中)、gap 窓 51/72 日穴の修復 or 凍結 gap-tolerance が LOCK 前必須。**LOCK (rule:R1) は 21 条件充足後のみ — 測定未着手、OOS 未接触** |
