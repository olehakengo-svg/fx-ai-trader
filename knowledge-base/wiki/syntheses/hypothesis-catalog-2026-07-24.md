...
| 21 | commodity_cross_range_mr (wave-6 EA-a) | **queued 2026-07-31 → G0 計測着手 2026-08-03** — [[ea-landscape-sweep-2026-07-31]] §4.1 (31-agent EA 大規模調査の敵対的検証 GO、score 63/56)。**G0 = OANDA 3 クロス (AUD_NZD/AUD_CAD/NZD_CAD) RT 実測 ≥1 週間 (rollover 込み) がシグナル計算前の必須ゲート** — RT 未収載ペアで最尤 kill 点。G0 通過時のみ explore pre-reg 起案。**計測器 = `tools/commodity_cross_g0_rt.py` (Render cron `fx-ai-cc-g0-rt`、日次 06:00 UTC)** — BA M5 candles 遡及 14d spread 分布 + financing 日次 snapshot、計測のみで verdict なし (判定基準は pre-reg 起案時に凍結)。シグナル計算・fwd return 接触ゼロ | — (未測定)。per-position 単発のみ (grid/averaging 評価対象外)、v1 純価格アンカー限定 (金利差調整変種は E20 隣接で禁止)、M5/15m 形態禁止。session-mr-cross-wave1 (05-11 BLOCKED_DATA) を supersede。負の prior: 「方向は合うが弱い」死型 3 例の家系 + RBA/RBNZ デシンク窓 (2014-15/2022-24) を OOS 敵対窓に必須組込 |
| 22 | equity_curve_shadow_gating (wave-6 EA-b) | **🔒 LOCKED-FORWARD 2026-08-03** — [[equity-curve-shadow-gating-forward-prereg-2026-08-03]] (self-LOCK、敵対的検証 SURVIVES-W-AMENDMENTS #1-#8 反映)。**在庫調査で §4.2 の暗黙仮定が否定された** (shadow book 全長 4.04mo、eligible N≥300×12mo = 0 セル) → 分析パラメータ無変更のまま forward-completing 化。first look cutoff **2027-05-06** (= epoch+28d+365d 機械導出) / verdict 05-13、registry 2 本で機械監視。P-10 型 gate 条件付き計算 ban 発効中 (運用モニタは名指し whitelist) | — (未測定、登録アクションのみ = スロット非消費)。検証は内部 shadow closed P&L のみで完結 (新規データ/摩擦ゼロ、LOCKED 非接触)。学術 prior mixed〜negative の低 prior・低コスト枠。eligible 予測 3-7 セル (R2 demote 執行形態依存)。PASS 時 OOS = cutoff 後 90d shadow forward。live gate 化は R1 + user 承認 |
