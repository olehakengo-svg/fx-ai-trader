...
| 23 | e21_human_signal_stream (帰属分解、scan 第3次 §2.2) | **S2 標本量実測 完了 2026-08-17** — [[e21-attribution-s2-2026-08-17]] (診断枠、供給ラインとして数えない。WR/N 統計ではなく会計恒等分解) | ⏸️ **user 決裁待ち** — bot 口座の手動 = 40 往復・全決済・全て分単位保有・financing 接触 0 日・実現 −6,821 円 = **長期キャリー母集団はこの口座に不在** (30000u×7 も 07-10/07-13 の分単位スキャルと判明 — 「07-29」は確認日)。トークンは個人口座 (CFD) を視認可能だが読み取りは permission classifier がブロック → **選択肢 (a) 読み取り許可 (b) CSV export 提供 (c) carry 成分 data-blocked クローズ**。registry `e21-personal-account-decision` (deadline 2026-08-31) |
