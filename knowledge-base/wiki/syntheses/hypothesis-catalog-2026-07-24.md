...
| 25 | e23_cb_statement_text (中銀声明テキスト差分、scan 第3次 §E23) | **採用 — S2 診断枠 2026-08-18** — [[e23-cb-text-adjudication-s2-2026-08-18]] (ゲート = E7 verdict、08-17 前倒し確定で解除。改訂 WIP 原則発動: 能動ライン 0 本)。claim = PR #188 + queue ticket。**testable form DRAFT + 敵対的検証 (自己) 完了 2026-09-10** — [[../decisions/e23-cb-text-explore-prereg-2026-09-10]] (scan#4 前倒しに同乗、queue item 3)。**🔒 explore pre-reg LOCK 2026-09-10 (別 commit、explore 枠 1/3 消費)** | 未測定 — 測定 (pass-0 census → two-pass) は LOCK 後の別タスク、registry `e23-explore-verdict-deadline` (2026-09-20)。**凍結設計**: Apel–Blix Grimaldi 2012 辞書 (WP 261 逐語転記、`tools/e23_lexicon_apel_grimaldi.py`、sha256 は LOCK commit で pin) の ΔNH sign-follow × G4 中銀 (Fed/ECB/BOE/BoJ、1 中銀 1 文書種) × D1+5 pooled (bp)、explore 2014-2023 / OOS 2024-2026H1、census gates (per-CB 被覆、生存 <2 CB = DATA-BLOCKED)。TDW/WCB は CC BY-NC につき E22 §2.1 型事前コミット節で secondary 保留 (user 決裁点)。**負の prior 不変**: E15+E7 二重 FAIL 家系、生存 0/17。**#27 family A との text 多重性は endpoint 直交 (price-facing vs label-facing) で BH 非合流 — 両 doc 相互参照で会計** | **🆕 pass-0 執行完了 2026-09-15**: コーパス 398 文書取得 (Fed/ECB/BOE/BoJ 公式サイト、`data/external/cb_statements/` + sha256 manifest)、census gates **全 4 CB PASS → pass-1 解錠** (生存 CB 4 ≥ 2)。⚠️ **正直な power 警告**: 凍結辞書の当たり密度 0.036-0.424 bigram/文書 = matched 文書 50 件 → **イベント数の機械的上界 100 = Gate B 閾値ちょうど** (実 N は void/隣接重複で必ず下回る) ⇒ pass-1 で UNDERPOWERED の公算大。救済的な語彙拡張は §0-3 で恒久禁止。BoJ 文書同定は表題条件付け (= 政策変更回だけ落ちる選択バイアス) を避け主文書スロット規則へ (観測前・価格非接触、pre-reg §10.3)。詳細: [[../decisions/e23-cb-text-explore-prereg-2026-09-10]] §10 / [[../../raw/analysis/e23-pass0-census-2026-09-15]]
...
| 27 | family_a_statement_ladder_intervention_prob (MoF 発言ラダー→介入確率、label-facing・価格不使用) | **採用 2026-09-10 (scan 第4次 A/B/C 統合裁定)** — [[../research/external-hypothesis-scan-round4-2026-09-10]] §2.2 / pre-reg DRAFT = [[../decisions/family-a-statement-ladder-prereg-2026-08-19]] (起草時仮番 #26 は family C が消費 → #27 確定)。**explore 枠は未消費** — 消費は敵対的検証 → 凍結コミット時 (registry `family-a-adversarial-freeze-deadline` 2026-09-24)。**凍結まで発言×介入ラベル joint 計算は全面禁止** | 未測定。estimand = ladder 検出器 (L≥4 遷移) の hit/FA 較正 — **有効 N = 4 episode blocks につき explore はいかなる結果も記述級 (edge 主張恒久禁止、DRAFT §5)**。クリーン判定は forward OOS のみ (最初の判定イベント = Q3 日次開示 ~2026-11-06。月次開示 08-28 で 07-30..08-26 窓 15.4 兆円が確定済み = forward イベント供給は実在)。価値 = family B 回避設計 + T5 型運用判断への較正済み入力。**#25 E23 との text 多重性は endpoint 直交で BH 非合流** (scan#4 §2.2 で会計) |
...
| 28 | range_boundary_limit_fade (user 発案 2026-09-11「レンジ端指値」、L-d 条項準拠の同型差分節付き) | **explore 完了 2026-09-14 (同日、単独スキャン、Bonferroni α=0.05/32)** — [[../learning/range-fade-limit-scan-2026-09-14\|range-fade-limit-scan-2026-09-14]] (ban 監査 17 系統 → 同型回避差分節 §1 / 敵対的コードレビュー 2 レンズが計測前に look-ahead 2 件検出・修正 / positive control 10× friction が決定論的予測と厳密一致 = harness 両方向 pin) | ❌ **FAIL_NO_SURVIVORS クローズ** — 32 セル (USD_JPY/EUR_USD/GBP_USD/EUR_JPY × lb{96,192} × vol{any,low} × exit{midasym,sym1r})、N=4,648、事前宣言ゲート (recentered block bootstrap p_block < 0.0015625) 通過 **0/32**、min p_block=0.1133、25/32 セル負 EV。EUR_JPY lb96 lowvol の正 EV (+1.85/+3.33p、wf 3/3) は非有意 + **非同時窓 (2025-07-21..2026-07-21、凍結 parquet stale)** + max-t 選択下で候補認定不能 — 推奨クローズ。slow-MR/band-fade「方向は合うが弱い or 負」死型 5 例目相当 (ppp/qs/rn/cc-mr 系列)。**同型再試行禁止 = rolling 時間窓レンジ極値 × 指値 penetration fade × 15m × 固定 TP/SL 全変種。再挑戦は L-d 条項 (IC-first + 本レポート §1 差分節を超える明示差分) のみ**。引用時は live 乖離 caveat 3 点 (§5.2: 両側同時貫通 no-entry の live 再現不能 / stressed RT / EUR_JPY 窓) 必読。asia_range_fade_v1 (OPEN) は別 estimand で影響なし。2013-2025 の過去データは非接触保存。raw: `raw/bt-results/range-fade-scan-2026-09-14/` |

## scan 第4次 triage (2026-09-10 — [[../research/external-hypothesis-scan-round4-2026-09-10]])

- **family B (MoF 介入イベント→回避/執行)**: ❌ **不採用 (park、番号非付与)** — (a) 介入認定は外部一次情報のみの拘束下で、公式日次ラベルは四半期ラグ = イベント時点の執行条件付けが構造的に不能。E-A 検知器 (価格シグネチャ) をラベル代用すると `mof-next-episode-reverdict` の 2026+ OOS を burn するため禁止。(b) episode blocks ≤4 + 2026-05 outcome (+188.1p) は #4 verdict で既公表 = peek 済み — explore の砂場が無い。(c) 方向 prior 不安定 (E-C 符号逆 N=1)。**再裁定条件 = family A (#27) explore verdict + #4 reverdict (~11-06) の両完了後、「A 検出器 → B 回避設計」を新 family として**。dossier 資産 (初撃/追撃区別・48h 集中) は凍結保存
- **E26 介入情報リリース・マイクロ構造** (BoJ 当預残高予想/短資会社経路、ScienceDirect 2026-04): ❌ C1 棄却 (歴史系列の無料経路なし)。機構知見は family B 再裁定の設計参照
- **E27 CFTC TFF positioning**: ❌ C2 棄却 — #16 ban「COT Δ/flow×週次固定ホライズン全変種 (**母集団問わず**)」に正面衝突、再挑戦経路なし
- **E28 ML/深層学習 FX 予測 (2025-26 文献群)**: ❌ C2/C3 棄却 — in-sample R² 家系 + OHLCV 内部モダリティ閉鎖済み。無料 OHLCV×intraday 同型は起案しない (base rate 4% 教訓)
- **U4 決裁材料** (有償/長ホライズン/非価格 feasibility) は scan#4 §4 に凍結 — 現時点の購買推奨ゼロ、再上程条件 = U1「継続」+ ロック済みライン verdict 到達
