...
| 23 | e21_human_signal_stream (帰属分解、scan 第3次 §2.2) | **S2 標本量実測 完了 2026-08-17** — [[e21-attribution-s2-2026-08-17]] (診断枠、供給ラインとして数えない。WR/N 統計ではなく会計恒等分解) | ⏸️ **決裁 (a) 消化 → 外部口座 CSV 待ち (data-blocked 同型)** — bot 口座の手動 = 40 往復・分単位・−6,821 円、**個人口座 (CFD、user 決裁 (a) で 08-17 監査済み) = 04-02 の 1 日 45 往復・−5,891 円・全額出金 NAV 0** → **トークン視認 2 口座のどちらにもキャリー母集団は不在** (手動短期 計 85 往復 −12,712 円・swap ゼロ)。キャリー玉は外部業者/口座 — 残る選択肢 = (b) 明細 CSV 提供 (data/external/e21/) / (c) data-blocked クローズ。registry `e21-personal-account-decision` (deadline 2026-08-31、未提供なら (c) で起案) |
