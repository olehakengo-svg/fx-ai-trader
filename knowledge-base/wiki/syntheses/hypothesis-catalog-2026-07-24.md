...
| 16 | cot_commercial_flow_weekly (W3-1) | **凍結 explore 走行 2026-07-29** — [[cot-commercial-flow-explore-prereg-2026-07-29]] (単独 wave m=1、#5 の明示 carve-out。敵対的検証 6 条件解決済み: `raw/analysis/wave3-adversarial-verification-2026-07-29.md`) | 走行中 |
| 17 | fx_quote_spread_state (W3-2) | **queued (単独 wave、#16 verdict 後)** — 新モダリティ = MASSIVE tick BBO 2010+ (当日実在確認、`raw/analysis/wave3-candidates-2026-07-29.json`)。LOCK 前 6 条件 (イベント条件付き摩擦ゲート先行 / デスゾーン配備ブロッカー解消 / feed QA / DST 補正 / coverage assert / primary 1 本) は検証レポート [W3-2] 節 | 未着手 — fetch は #16 走行中に background 開始可、coverage assert 不通過は data-blocked クローズ (事後緩和禁止) |
