...
旧 BT (v8.x pre-gate-chain) と 新 BT (M5 2026-04-27 v9.3 post-gate-chain, raw/bt-results/bt-365d-2026-04-27.json) を両併記。
v9.3 deploy (2026-04-24) より前の旧 BT は spread_sl_gate / Q4 gate / Phase0 gate を経由しないため Live と乖離する。
post-gate-chain EV は BT 内に gate chain を適用した後の値で Live と整合性が高い。

| 戦略 | ペア | 旧 EV (v8.x pre-gate) | M5 N | M5 WR | **M5 EV (post-gate v9.3)** |
|---|---|---:|---:|---:|---:|
| session_time_bias | USD_JPY | +0.580 | 344 | 65.7% | **+0.177** |
| session_time_bias | EUR_USD | +0.215 | 442 | 62.7% | **+0.131** |
| session_time_bias | GBP_USD | +0.113 | 443 | 59.6% | **+0.017** |
| trendline_sweep | EUR_USD | +0.927 | 56 | 76.8% | **+0.722** |
| trendline_sweep | GBP_USD | +0.599 | 108 | 66.7% | **+0.270** |
| gbp_deep_pullback | GBP_USD | +1.064 | 98 | 65.3% | **+0.721** |

旧合計PnL想定: **+433pip/年** (v8.x pre-gate)
新 EV ベース推定: post-gate-chain で全戦略 EV ~30-70% 縮小、月利寄与は同比率で再評価が必要。

**注**: 旧数字は ELITE_LIVE 昇格判断時の BT であり依然有効な参照点。
新数字は v9.3 gate chain 反映で「Live と数学的に整合する」EV。月利目標達成度を試算する際は新 EV を採用。
