...
## 4. GO 候補 (台帳 queued — [[hypothesis-catalog-2026-07-24]] #21/#22)

### 4.1 #21 `commodity_cross_range_mr` — コモディティ三角クロスの per-position レンジ MR (score 63)
...
### 4.2 #22 `equity_curve_shadow_gating` — shadow 損益条件付き live 転送のメタ配分層 (score 55)
...
- [[hypothesis-catalog-2026-07-24]] に **#21 commodity_cross_range_mr (queued)** / **#22 equity_curve_shadow_gating (queued)** を追記 (本コミット)。並列アクティブ上限 3 本ルール: wave-3/4 全クローズ済み + #20 composite は PARK (枠非消費) でアクティブ 0 → 2 本着手可。番号 #20 は並行セッションの composite_weak_signal_portfolio が先取したため本調査分は #21/#22 (旧記載 #20/#21 から renumber、2026-07-31 merge 時)。
- **#21 の最初のアクション = G0 (OANDA 3 クロス RT 実測、≥1 週間、rollover 込み)** — シグナル計算ゼロの摩擦測定なので即着手可、pre-reg スロットも消費しない。G0 通過時のみ explore pre-reg 起案。
- **#22 の最初のアクション = explore pre-reg 起案** (K/閾値グリッド凍結、交絡遮断規定、Bonferroni 分母確定) → self-LOCK → 単一実行。
