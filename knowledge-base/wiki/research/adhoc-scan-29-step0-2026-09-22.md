...
- **step 1 での扱い (第6次で実行と裁定された場合)**: primary 候補として **逐語転記 → sha256 pin → P1 監査後コーパスに as-is 適用 → NH≠0 文書割合と |ΔNH| 分布 (価格非接触)** を数える。**クローズ条件 (§7-3 で事前固定) = ABGH-2019 の NH≠0 文書割合が、同一コーパス・同一 run で再計算した ABG-2012 の NH≠0 文書割合の 2 倍未満なら「license-free dense primary 不在」で #29 S1 候補なしクローズ** (secondary だけでは pre-reg の primary が立たない)。参照値 (監査前コーパス、pass-1): ABG-2012 は NH=0 279/327 → **NH≠0 = 48/327 = 14.7%** (pre-reg §11.2)。⚠️ pass-0 の「matched bigram ≥1 = 50/327 (15.3%)」は NH≠0 ではない (bigram を持っても #hawk = #dove なら NH=0 — 差の 2 文書)。P1 監査でコーパスが動けば参照値も動くので、閾値は**比 (2 倍)** で凍結しベースラインは census 時に再計算する。
...
| 制約 | 判定 | 根拠 |
|---|---|---|
| **C1 データ入手性** | **△ 条件付き** | コーパス: in-repo 398 文書 + sha256 manifest (再取得不要、ただし P1 監査で manifest 変更予定)。モデル: primary 候補 (ABGH-2019) は公刊 Appendix で $0・即時 / 文分類器候補 (WCB pooled) は gated manual + ライセンス不整合。**無条件に入手可能な候補は 0、dense と実測された候補も 0** (密度は全候補で step 1 まで未測定) |
...
**裁定**: **step 0 の結論 = 無条件候補 0 / 条件付き 2 (いずれも dense 未確認)。step 1 census は提案として置き、実行可否は第6次 (10-18) の裁定に委ねる。family 採用・explore LOCK は起案しない。** 再評価 09-22 §3 Rank 8 (iii) が #29 S1 を step 0 に限定している (候補ゼロでも可) ため、本稿は step 1 を承認せず、着手可能本数にも数えない (§6)。第6次が「step 1 実行」と裁定した場合のクローズ条件は §7-3 の文言 (ABG-2012 同一コーパス再計算比 2 倍) で事前固定済み。time-to-M2 への寄与評価は memo Rank 5 (refuted、+0.4〜1.6pp) から**動かない** — 本稿は供給ライン (WIP) の充足手段としてのみ意味を持つ。
...
| **提案 (裁定待ち)** | 1 | **#29 S1 step 1 census** — 第6次 (10-18) で実行可否を裁定。実行なら前提 = **E23 P1 監査 (10-03) 後のコーパス確定** + ABGH-2019 転記 + sha256 pin。価格非接触・$0。⚠️ **台帳 #29 は未登録のまま** (登録は第6次 or 新 pre-reg 起案時の裁定) — 提案は S1 intake の作業単位であって採用 family ではない |
...
2. **[第6次 2026-10-18 / R3]** 本稿 (step 0) を持ち込み、(i) **#29 S1 step 1 census の実行可否** — 再評価 09-22 §3 Rank 8 (iii) の「step 0 のみ」を解く**新しい裁定**であり、本稿は承認しない、(ii) #29 の台帳登録可否、(iii) explore 枠の現在値確定 (0/3〜2/3)、(iv) Picault–Renault 再取得、(v) 下記 C5 規律の KB 反映、を裁定。
3. **[第6次で (i) = 実行と裁定された場合のみ / 10-18 以降 / R3 / 別 PR、価格非接触]** #29 S1 step 1 census: ABGH-2019 辞書の逐語転記 (`tools/` 新モジュール、WP 381 Appendix 出典明示、sha256 pin、test で語彙編集を封鎖) → P1 監査後コーパスに as-is 適用 → NH≠0 文書割合 / |ΔNH| 分布 / CB 別 (件数のみ)。**同じ run で ABG-2012 (in-repo 凍結モジュール) を同一コーパスに再適用し、ベースライン NH≠0 割合を再計算する** (E23 pass-1 の 279/327 は監査前コーパスの値で、監査後は動く)。secondary 併載 = WCB pooled (研究利用、§0-4 — 文書密度の実測を兼ねる)、Gardner–Scotti–Vega (Fed 景況 sentiment、stance 非該当を明示)。**閾値・ホライズン・ペア写像の探索は禁止** (それは pre-reg の領分)。クローズ条件を census 実行前に本節の文言で固定: 「**ABGH-2019 の NH≠0 文書割合 < 2 × ABG-2012 の NH≠0 文書割合 (同一コーパス・同一 run で再計算) なら license-free dense primary 不在 → #29 S1 候補なしクローズ**」。参照値 (監査前、pass-1、pre-reg §11.2): ABG-2012 NH≠0 = 327 − 279 = **48/327 = 14.7%** → 2 倍 = 29.4%。⚠️ pass-0 の「matched bigram ≥1 = 50/327 (15.3%)」は NH≠0 ではない (bigram を持っても #hawk = #dove なら NH=0) — 初版はこれをベースラインに誤用しており、境界近傍 (29.4–30.6%) の候補を誤分類しえた (レビュー P2 で訂正)。
...
**registry 更新案 (S7 が編集)**:
- `edge-supply-scan-monthly` (deadline **2026-10-18 は据え置き**): message に「**臨時スキャン 2026-09-22 実行済 (#29 S1 step 0、[[adhoc-scan-29-step0-2026-09-22]])**: 無条件候補 0 / 条件付き 2 (WCB pooled = ライセンス不整合で secondary、ABGH-2019 辞書 = license-free primary 候補・密度未測定)。WIP 会計: 着手可能 (能動) 0 / 提案 1 (S1 step 1 census — 第6次で実行可否を裁定、承認は再評価 09-22 Rank 8 (iii) の step 0 限定を解く新裁定が必要)。第6次で step 1 実行可否・#29 台帳登録可否・explore 枠現在値・C5 text 規律を裁定」を追記。
...
- 新規 (**第6次で step 1 実行と裁定された場合のみ**、`deadline_info`): `adhoc-scan-29-step1-census-gate` — deadline は第6次が置く (第7次 ≈ 2026-11-18 の前)。「E23 P1 監査後コーパスで ABGH-2019 と ABG-2012 を同一 run で census し、ABGH-2019 の NH≠0 文書割合 < 2 × ABG-2012 の NH≠0 文書割合なら #29 S1 候補なしクローズ」。**本稿時点では起票しない** (初版の 10-17 期日案は step 1 を第6次前に走らせる前提で、step 0 限定の承認範囲を超えていた)。
...
- **本稿は step 1 を承認しない**。再評価 09-22 §3 Rank 8 (iii) の承認範囲は step 0 のみ。step 1 の実行・ベースライン再計算・registry gate 起票は全て第6次 (10-18) の裁定後。
...
| #29 S1 = step 0 のみ (候補ゼロでも可)、ABG 辞書 census 禁止、結論は 10-18 を待たず臨時スキャン記録に | [[path-to-win-reassessment-2026-09-22]] §3 Rank 8 (iii) |
