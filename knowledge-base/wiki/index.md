...
- 現在: **DD防御0.2x** (DD=**90.12%** flat, 5連騰後初の non-NEW-HIGH, defensive mode — 2026-06-25) → 月利47%（BT推定、クリーンデータ蓄積中）
...
- Defensive mode: **0.2x** (DD=**90.12%** / 901.2pip — **flat vs 06-19, first non-NEW-HIGH in 5 snapshots** ✅, eq_current=−$884.3 — Render API 2026-06-25 wiki-daily-update; 30d Kelly=0.0% edge=**-28.82%** ⚠️⚠️⚠️ deteriorating from -24.75%, 6th straight window; prev snapshot 06-19 DD 90.12% / edge -24.75%)
...
- FX-only post-cutoff (2026-04-08〜) — **2026-06-25 wiki-daily-update** (demo/stats live only, is_shadow=false):
  - Live only (is_shadow=0): **N=516**, WR=42.8% (decided 45.7%), EV=-0.82, PnL=**-423.5pip** (**+6.0pip vs 2026-06-19 ✅ first improvement in recent log**, ~6-day window — 06-20/21 weekend, no log 06-22/23/24); +3 fills (**3W/0L/0BE — all winners, breaks 5-loss run**); Wilson lower=41.3%, BF lower=38.4%
  - 🟢 **1 LIVE filled 2026-06-24 17:27 UTC** (trendline_sweep → daytrade_gbpusd GBP_USD SELL 5000u, **oanda#541666** — first *confirmed* fill (non-empty trade id) since 06-16/17/19 awaiting-fill sends; ELITE_LIVE pipeline executed)
  - 30d rolling (risk API): Kelly=**0.0%** ⚠️⚠️ (edge=**-28.82%** NEGATIVE, WR=47.66%, odds 0.493 — ⚠️⚠️⚠️ deteriorating from -24.75%, 6th straight window); 30d attribution: **gross=net=-211.5pip** (n=107, friction 397.7pip/3.72 per-trade) — ⚠️⚠️ gross-edge deterioration into a 6th window: -42.7→-70.1→-96.1→-113.8→-179.2→-211.5, directional edge dominant, friction flat-to-down. **Every 30d instrument negative — USD_JPY blew out -1.4→-39.3 (#2 drag); GBP_USD eased -113.4→-108.7 but #1 drag (mean -3.40)**
...
- Last updated: 2026-06-25 (wiki-daily-update auto; ~6-day window vs 06-19, 06-20/21 weekend + no log 06-22/23/24); prev: 2026-06-19
