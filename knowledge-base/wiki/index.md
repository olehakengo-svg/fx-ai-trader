...
<!-- KB_PORTFOLIO_START -->
## Current Portfolio (auto-synced, 2026-08-20)
...
- Defensive mode: **0.2x** (DD=🔴🔴🔴**1209.1pip = 120.9%** on the legacy $1000 pip basis — **NEW HIGH +201.1pip vs 07-27's 1008.0pip/100.8%**, eq_current=−$1192.2 vs peak +$16.9 — Render API 2026-08-20. 30d Kelly=0.0%, overall-edge (risk dash)=**−32.01%** (⚠️ WORSENED −6.32pp vs 07-27's −25.69% — real deterioration, not window-roll); prev snapshot 07-27 DD 1008.0pip / edge −25.69%)
  - ⚠️ **`dd_pct` basis changed (v2.3 実 NAV(JPY) 移行)**: the API field now returns **0.097 = 9.7% against the JPY NAV ledger** (`jpy_ledger.active=true`: base ¥359,109 / peak ¥359,288.47 / current ¥324,461.58 / DD ¥34,826.89), **not** the legacy pip/$1000 basis. The historical wiki series (98.2 → 99.33 → 100.01 → 100.8%) is the pip basis; its continuation is **120.9%**. **100.8% → 9.7% is a unit change, NOT a recovery.**
...
- Ruin probability: **0.0%** ✅ (MC sims — Render API 2026-08-20; ⚠️ init rebased 1000→**5801.44**: worst-case DD99=**1037.72 (17.9% of init)**, median max DD=**664.33 (11.45%)**, median final eq=**5159.73 (−11.1%)**; normalized vs 07-27 the tail is mildly wider / median max DD mildly narrower) — 実現 ruin が 0% なのは 0.2× lot cap のみによる (原エッジは負のまま)
- Aggregate Kelly: **0.0%** ⚠️⚠️⚠️ (overall edge (risk dash)=**−32.01%** ⚠️ WORSENED −6.32pp vs 07-27's −25.69% — **real deterioration, not window-roll**: risk-window WR 57.89%→40.0%, odds 0.2835→0.6998; DSR=0.0/haircut 100%/Sharpe −0.2259/n_trials 3; net **−107.6 (n=10)**, per-trade **−10.76** (5.4× worse than 07-27's −1.99), friction 4.82 per-trade; **AUD_JPY #1 30d drag −122.6 on n=2 (mean −61.3)**, USD_JPY +15.0 n=8; VaR95/CVaR95 blew out 13.58/19.0 → **81.31/123.2**; TRUE_LIVE SSOT: N=371 raw=-0.69. 🟢 agg-kelly gate is genuinely load-bearing this window — **4 live fills passed / 26 blocked** (`agg_kelly` −0.349…−0.371<0))
...
- ⚠️ Portfolio warnings (cumulative post-cutoff, 2026-08-20): 🔴 **price_shock_rev_aud_jpy_h1_long — worst per-trade cell in the book (N=2, −122.6pip, mean −61.3)** vs a BT EV of **+32.25pip**; session_time_bias #1 cumulative drag (N=30, WR=40%, −67.8pip); vwap_mean_reversion (N=11, WR=36.4%, −63.1pip); wick_imbalance_reversion (N=14, WR=35.7%, −63.0pip); trendline_sweep (N=32, WR=62.5%, −49.7pip — high WR, negative PnL = payoff asymmetry); vix_carry_unwind (N=26, −46.9pip, PAIR_DEMOTED 08-03); bb_rsi_reversion (N=108, WR=41.7%, −34.5pip). All DSR 0.0 (haircut 100%). 🟢 Only positive live cells of size: usdjpy_carry_dip_accumulator (N=7, +45.1), orb_trap (N=4, +23.2), post_news_vol (N=2, +19.0), ema_pullback (N=2, +17.8), vol_momentum_scalp (N=17, +11.1).
...
- Last updated: 2026-08-20 (wiki-daily-update auto; ⚠️⚠️ **24-day cadence gap 07-28→08-19 — longest in the log, and the book was NOT flat across it**; N 563→**573 (+10 closed fills, 4W/6L)**, flat run broken; PnL −552.7→**−660.3 (−107.6)**; EV −0.98→**−1.15**; DD **1209.1pip = 120.9%** pip basis — **NEW HIGH +201.1** (⚠️ API `dd_pct` now 0.097 = **9.7% JPY-NAV basis**, a unit change not a recovery — see Defensive mode above); overall edge (risk dash) **−32.01%** worsened −6.32pp (**real**, not window-roll); 🔴 **the whole −107.6 is `price_shock_rev_aud_jpy_h1_long` n=2 −122.6 (mean −61.3)** — ex-AUD_JPY the 30d book is **+15.0 on n=8**; 🟢 **4 confirmed live fills, all `usdjpy_carry_dip_accumulator` USD_JPY BUY 1000u** (oanda #677402/#677910/#677917/#677924, 08-14→08-19) — its 07-02 zero-fire dormancy ended when USD_JPY fell back under the 159.5 ceiling; **26 agg-kelly blocks / 0 false-sent**; shadow_count 10,456→**12,241 (+1,785)**; ruin 0.0%; 🆕 learner adj **id=93** (08-19, 4th identical `sr_channel_reversal` re-affirm — blacklist still `[]` = no-op); see [[2026-08-20]]); prev: 2026-07-27. ⚠️ **The System State body below remains partly stale at 2026-07-08 (N=558/−540.7)** for the deep narrative lines — the current-state figures are the ones in this line + Defensive mode / Ruin / Aggregate Kelly / Portfolio-warnings lines above + Session History top entry.
