...
- 現在: **DD防御0.2x** (DD=🔴🔴🔴**100.8%** — **100%バリア超え、held flat**（07-13 で barrier 突破後、07-14/07-16 は NEW HIGH なし・deepening せず。07-16 に +3 fills 2W/0L/1BE=+3.7pip で book 微増したが DD 動かず）, defensive mode — 2026-07-16) → 月利47%（BT推定、クリーンデータ蓄積中）
...
- Defensive mode: **0.2x** (DD=🔴🔴🔴**100.8%** / 1008.0pip — **100% barrier EXCEEDED, held flat** (no new high vs 07-14 — **+3 fills 2W/0L/1BE=+3.7pip 07-16**, book edged up but the gain was too small to move DD at 1-dp so it neither deepened nor made a new high), eq_current=−$991.1 vs peak +$16.9 — Render API 2026-07-16 (⚠️ 07-15 cadence gap, 2-day window). 30d Kelly=0.0% overall-edge (risk dash)=**-40.5%** (WORSE −0.5pp vs 07-14; per-trade net −3.52→−3.64, window rolled n 64→60); prev snapshot 07-14 DD 100.8% / edge -40.0%)
...
- Ruin probability: **0.0%** ✅ (MC sims — Render API 2026-07-16; worst-case DD99=**272.36** (widened vs 07-14's 264.3); median final eq=**781.25**/init 1000, vs 788.65; median max DD 219.75) — 実現 ruin が 0% なのは 0.2× lot cap のみによる (原エッジは負のまま。07-16 は realized book 微増だが MC forward tail は左シフト)
- Aggregate Kelly: **0.0%** ⚠️⚠️⚠️ (overall edge (risk dash)=**-40.5%** WORSE −0.5pp vs 07-14's -40.0% — per-trade net −3.52→−3.64; 30d window rolled n 64→60 (effective_date_from → 2026-06-16), absolute net eased -225.6→**-218.5** as a denominator artifact NOT edge gain; DSR=0.0/haircut 100%/Sharpe -0.562; friction 4.15 per-trade (n=60); GBP_USD #1 30d drag -96.7 n=21 (mean -4.60). per-strategy: only bb_rsi_reversion +edge this window (edge +0.158/half-Kelly 0.134/n=11/WR 72.7%), all else 0.0/insufficient. 🟢 agg-kelly gate remains the active safety net — 0 live fills / **0 blocked** this window (no signal reached the gate; all shadow_tracking))
...
- Last updated: 2026-07-16 (wiki-daily-update auto; ⚠️ **07-15 cadence gap → 2-day window**; N=**563** **+3 fills 2W/0L/1BE = +3.7pip** (245W/284L/34BE); PnL −556.4→**−552.7** (first net-positive window recently, marginal)/DD 🔴🔴🔴**100.8%** — **barrier stays exceeded, NO new high** (book gain too small to move DD); overall edge **-40.5%** WORSE −0.5pp (per-trade −3.64, window rolled n 64→60); ⚪ **0 live fills / 0 blocked** (all 30 shadow_tracking); 0 false-sent; MC tail widened (worst DD99 272.36, median final eq 781.25); shadow_count 9,656 +215; audit total 12,496 +224); prev: 2026-07-14
