...
- 現在: **DD防御0.2x** (DD=🔴🔴🔴**100.8%** — **100%バリア超え、held flat**（07-13 で barrier 突破後、07-14/07-16/07-21/07-23 とも NEW HIGH なし・deepening せず。07-16→07-21→07-23 は **3窓連続 0 closed fills=flat**で book 不動）, defensive mode — 2026-07-23) → 月利47%（BT推定、クリーンデータ蓄積中）
...
- Defensive mode: **0.2x** (DD=🔴🔴🔴**100.8%** / 1008.0pip — **100% barrier EXCEEDED, held flat** (no new high vs 07-21 — **0 closed fills = flat realized window, 3rd consecutive** (07-16=07-21=07-23), book unchanged so DD neither deepened nor made a new high), eq_current=−$991.1 vs peak +$16.9 — Render API 2026-07-23 (⚠️ 1-day cadence gap: no run 07-22). 30d Kelly=0.0% overall-edge (risk dash)=**-31.7%** (⚠️ worsened -2.1pp on window-roll — 2 slightly-positive GBP_USD trades aged out with 0 new fills, net −123.2→−126.8, mirror of 07-21's easing; **NOT a new-edge loss**); prev snapshot 07-21 DD 100.8% / edge -29.6%)
...
- Ruin probability: **0.0%** ✅ (MC sims — Render API 2026-07-23; worst-case DD99=**209.06** (widened back vs 07-21's 200.24); median final eq=**841.18**/init 1000, vs 852.25; median max DD **160.26**, vs 149.17) — 実現 ruin が 0% なのは 0.2× lot cap のみによる (原エッジは負のまま。07-23 も realized book flat=0 fills だが MC forward tail は僅かに悪化 — 30d 窓が 2件の slightly-positive GBP_USD を排出して左シフト、07-21 の改善の mirror、新エッジ変化ではない)
- Aggregate Kelly: **0.0%** ⚠️⚠️⚠️ (overall edge (risk dash)=**-31.7%** ⚠️ worsened -2.1pp vs 07-21's -29.6% — kelly.edge -0.3174; 30d window rolled n 50→48 (effective_date_from → 2026-06-23) with **0 new fills** → 2 slightly-positive GBP_USD trades aged out (dropped cohort ≈ +3.6), net worsened -123.2→**-126.8**. **NOT an edge loss — realized book is flat, mirror of 07-21's roll**; underlying edge unchanged/still deeply negative; DSR=0.0/haircut 100%/Sharpe -0.423 (worse on roll); friction 4.2 per-trade (n=48); GBP_USD #1 abs 30d drag -47.1 n=14 (mean -3.36; EUR_USD -2.96/EUR_JPY -2.87 worse per-trade). per-strategy: only bb_rsi_reversion +edge this window (edge +0.158/half-Kelly 0.134/WR 72.7%), all else 0.0/insufficient. 🟢 agg-kelly gate = active safety net — 0 live fills / **2 blocked** this window (agg_kelly -0.343<0 @ 12:01–13:01 UTC — gate evaluated + rejected, contrast prior 3 windows' 0 blocks))
...
- Last updated: 2026-07-23 (wiki-daily-update auto; ⚠️ **1-day cadence gap: no run 07-22**; N=**563** **FLAT — 0 closed fills, 3rd consecutive** (07-16=07-21=07-23; 245W/284L/34BE, all metrics bit-for-bit = 07-16); PnL **−552.7** (flat)/DD 🔴🔴🔴**100.8%** — **barrier stays exceeded, NO new high, no deepening** (0 fills); overall edge **-31.7%** ⚠️ worsened -2.1pp (window rolled n 50→48, 2 slightly-positive GBP_USD aged out — **NOT a new-edge loss, book flat, mirror of 07-21's roll**); ⚪ **0 live fills / 2 blocked** (agg_kelly -0.343<0 — gate fired, contrast prior 3 windows' 0 blocks); 0 false-sent; MC tail widened back (worst DD99 209.06, median final eq 841.18, Sharpe −0.423); shadow_count 10,226 +294; audit total ~13,146 +347); prev: 2026-07-21
