...
- 🛑 **ゲートは本 PR 自身を 12 巡止め、計 32 件 (P1 5 / P2 27) の妥当な欠陥を検出した (反証 dismiss ゼロ)**。5 巡目 P1 が最重要 — **新設した故障 banner が Discord 1900 字カットの外 (watch 節の最後尾) に落ちており「本文には出ているが読み手に届かない」= 本 PR が直している 2 日間 blind と同型**だった (M1 直後へ引き上げ)。**指摘の 18/23 は『検査器が自分の名乗る保証を満たしていない』型**。9 巡目の学び = **lint は評価器の型強制を手で写したものなので写し間違いが必ず起きる — 評価器と同じ関数を lint でも呼べ** (`float` 経由をやめ `int(value)` へ / `strptime` が通る形でなく辞書順が壊れない正準形を要求 / `active` フィルタ**前**の raw を lint する)。10 巡目 = **免除は最小スコープで宣言せよ** — 一般化の過程で入れた免除が広すぎ、`shadow_count_*` の `entry_type: ""` (= 全 shadow トレード計上で判定を極端に早める) と `threshold_date: "no-deadline"` を lint が緑のまま通していた。8 巡目は評価器の制御 field (`closed_only: "false"` が `bool()` で true、`dedup_violation: "0"` が `== 0` に不一致、`match` の綴り違いが黙って無効化) = **監視母集団が黙って変わり判定期日が前後する** sr-anti-hunt 偽発火と同クラス。7 巡目で方針転換 — 「必須/任意」「top-level/要素」の軸ごとに穴を塞いで**同じ欠陥クラスを 3 巡繰り返した**ため、`KNOWN_VALUE_FIELDS` を単一集合に畳んで 4 象限すべてを統一検査に変更。**検査器の穴は列挙した軸に沿って空く — 軸ごとに塞ぐと軸の数だけ再発する**。6 巡目は `deadline: "soon"` が文字列比較で**永久 watching** になる型 (ZN 教訓と同型) を検出 — うち 2 件は「本 PR が直そうとした欠陥クラスを修正自身が再発させていた」型 (exit 2 で stdout 破棄 → 壊れた 1 件が他の TRIGGERED を隠す / 評価器の添字の写し漏れ)。3 巡目は P0 が `P?` に落ちて BLOCKING から外れる (最重要 finding があるのに PASS)、`reviewThreads` 無ページングで 100 超のスレッドを見落とす、`id` 欠落、`prefix:""` の truthiness — 全て修正。費用は 3 巡で ~45 分。1 巡目の内訳 — うち P1 は「exit 2 で stdout を捨てると壊れた 1 件が他エントリの TRIGGERED を隠す」= **本 PR が直そうとした盲点を別の層で作り直していた**。他 2 件は lint がネスト要素を見ていない / レビュアー照合が部分一致。3 件とも修正し counterfactual 3/3 確認
- **13 巡目 (2026-09-10、autopilot)** — ゲートを**自分自身に**回して未解決 P1/P2 3 件を消化。(a) **P1: 前方枠は行ごとに縛ったが合計を縛っていなかった** — `extract_watch_triggered` の 220 字 clip は 8 件以上で 1900 字枠を食い潰し、後続 TRIGGERED と **M1 節**を Discord カットの外へ押し出す = 5 巡目 P1 (「本文には出ているが読み手に届かない」) の**同型再発を修正自身が別パラメータで作り直していた** (`TRIGGERED_SECTION_CHARS=700` の総量予算 + 溢れ件数の明示告知、黙って捨てない)。(b) **P2: 要素キーが allow-by-default のまま残っていた** — 12 巡目で top-level は reject-by-default にしたが、コレクション要素は素通りで `{"column","value","opp"}` が lint を通り `_csv_row_predicate` が op 既定値 `==` で**別の条件**を毎日評価し続ける (`COLLECTION_ELEMENT_FIELDS` に任意キー欄を追加し要素も reject-by-default へ)。(c) **P2: `max_age_hours` に下限が無かった** — 有限数検査だけでは負値/0 が通り `age > max_h` が常に true = **1 秒前の記録まで stale** と報告して毎日 false TRIGGERED を出す (`FLOAT_MIN_EXCLUSIVE`/`FLOAT_MIN_INCLUSIVE` を新設、`threshold<=0` の永久 watching も同時に閉じた)。**学び: 「免除は最小スコープで」(10 巡目) と対になる — 型強制を足すときは int 経路と float 経路の両方に境界を置け。片方だけだと NUMERIC ∖ INT の field に穴が残る**
- テスト追加: trigger watch **+3 本** (要素未知キー / 任意キー許容 / 非 int 数値の意味的下限)、quant gate **+2 本** (多数 TRIGGERED でも M1 が枠内に残る性質 pin / 節の総量上限と溢れ件数告知)。**counterfactual 2/2** が所望のテストだけを落とすことを確認 (総量予算を無効化 → 2 本のみ FAIL)。全 3052 tests green / `scripts/check.py` 10/10
- **14 巡目 (2026-09-10)** — P2 2 件。(a) **allowlist に入れた selector が評価器まで配線されていなかった** — `shadow_count_info` は `instrument`/`direction` を許可していたのに `fetch_shadow_count()` へ渡しておらず、綴りが正しい entry でも**黙って全ペア/全方向**を計上していた (`shadow_count_decision` 側は 2026-08-18 の sr-anti-hunt 偽発火で同じ穴を塞いでいた = **同一欠陥の 2 例目**)。(b) **reject-by-default の 3 層目 (dict spec の中身) が素通り** — `source.date_colum` は lint を通り `fetch_data_coverage_max` が既定値 `""` に落ちて日付列なしのまま毎日 DATA_UNAVAILABLE を返し続ける。🛑 **修正の初回実装で自分が同じクラスの欠陥を作った** — 許可キー集合を新定数として足したところ 10 巡目の `NESTED_SPEC_FIELDS` と**名前衝突**し、既存の走査 (`source.label_columns: 1` 検出) を静かに壊していた。既存テストが落ちて発覚 → 「走査対象の一覧」と「許可キー」を `NESTED_SPEC_ALLOWED_KEYS` 1 本の SSOT に統合。**学び: 検査器に層を足すときは既存の層の定数名を先に grep せよ。同じ概念に 2 つの定数を置くと片方が他方を黙って上書きする**
- テスト追加: **+2 本** (ネスト spec の未知キー拒否 / allowlist selector の配線を**性質**で pin — instrument 指定 entry の実測 N が絞り込み後の値になること)。**counterfactual 2/2** (配線を外す / reject ループを外す) が所望のテストのみを落とすことを確認。全 3054 tests green / `scripts/check.py` 10/10
- **15 巡目 (2026-09-10)** — P2 1 件: **`direction` が str 型検査だけで通っていた**。`direction: "BYU"` は `count_matching()` / `count_live_matching()` の完全一致で全行 false = **母集団が黙って空**になり、N ベースの判定が永久 WATCHING に留まるか低 N の deadline 分岐 (retire) を誤って踏む。`ENUM_FIELDS_IF_NONEMPTY` を新設 (空文字 = ワイルドカードは正当なので `ENUM_FIELDS` には入れられない — 「非空なら評価器が解釈できる値」という別の箱が必要だった)。同クラスの `instrument` も併せて塞いだ: 新ペア追加が常時ありうるので閉じた enum にはできないが**形** (`^[A-Z]{3}_[A-Z]{3}$`) は固定なので `SHAPE_IF_NONEMPTY` で `USDJPY` / `USD_JPYY` を落とす (値そのものの妥当性は落とせない旨をコードに明記)。テスト +2 本 (lint 側と評価器側の両方で「綴り違い = 母集団ゼロ」を実証)、counterfactual で両 loop を無効化すると当該 2 本のみ FAIL。全 3056 tests green
- **16 巡目 (2026-09-10)** — P1 1 件 + P2 1 件。(a) 🛑 **P1: `load_registry_raw` の `.get("triggers", [])` が root キーの綴り違い/欠落を「空の台帳」に畳んでいた** — `trigger` と書き間違えるだけで **51 エントリ全部が黙って消え**、`check.py` も監視器も exit 0 のまま「active な trigger は無い」と報告する = **本 PR が直している 2 日間 blind と同じ帰結の、もっと静かな版**。root が dict でない / `triggers` 欠落 (綴り違い候補を列挙) / list でない / **空** の 4 形すべてで RuntimeError。空も異常扱いにしたのは「意図的に空」と「台帳が消えた」を監視器が区別できないため (本当に空を運用するなら guard を明示的に外す旨をコードに明記)。(b) **P2: `mode` が非空文字列検査だけ** — `mode: "daytrade_eurgpp"` は `/api/demo/trades` へ素通しされ open/closed 両経路の完全一致で **API 成功 + 0 行**、判定は永久 WATCHING か低 N retire を誤って踏む (direction/instrument と**同クラスの 3 例目**)。許可集合は**ハンドコピーせず** `modules/demo_trader.py` の `MODE_CONFIG` を AST で読んで導出 (9 巡目の学び「lint は評価器の写しなので写し間違いが起きる」の適用)。導出不能時は空集合でなく**例外** + 退役 mode 用の明示的逃げ道 `HISTORICAL_MODES` (現在空) を併設
- テスト追加: **+4 本** (root 台帳 5 形の拒否 / mode 許可集合が MODE_CONFIG から導出されること + registry 実使用 mode が含まれること / mode 綴り違いの拒否 / 導出不能時に例外)。**counterfactual 3/3**。全 3060 tests green / `scripts/check.py` 10/10
- **17 巡目 (2026-09-10)** — P2 3 件、いずれも「lint は通るが評価器が黙って別の答えを出す」型。(a) **辞書順比較される日付 field に時刻が付くと期日当日に成立しない** — `deadline: "2026-09-10T23:00:00"` は `"2026-09-10"` より辞書順で後ろなので、`today >= deadline` が期日当日 false = **予定されたレビューが黙って遅れる**。`LEXICAL_DATE_FIELDS` (`deadline`/`threshold_date`) を新設して日付のみを要求 (`since` は `fromisoformat` でパースされるので datetime 可 — **同じ DATE_FIELDS でも比較のしかたが違う**のが要点)。(b) **`endpoint` の先頭スラッシュ** — `fetch_ingest_health` は `f"{app_base}{endpoint}"` と素の連結なので `"api/..."` は `https://host.comapi/...` になり毎日 DATA_UNAVAILABLE。`SHAPE_IF_NONEMPTY` に絶対パス要求を追加。(c) **CSV 述語の `value` がスカラー限定でなかった** — dict/list を渡すと `str()` で潰されるので `op: "!="` はほぼ全ての通常値に一致し**偽 TRIGGERED** を出す。`_scalar_predicate_reason()` を新設 (非有限数も同時に拒否)
- テスト: **+3 本** (いずれも lint 側だけでなく**害の実演**を同梱 — 辞書順比較の非成立 / URL 連結の破損 / dict value × `!=` の一致)。**counterfactual 3/3**。全 3063 tests green / `scripts/check.py` 10/10
- 📋 **レビュー巡回の到達点 (17 巡・累計 P1 9 / P2 38)**: 13〜17 巡目の 11 件は**すべて `prereg_trigger_watch.py` の authoring lint の厳格化**であり、本 PR の主目的 (マージゲート新設 + 監視器の隔離/exit code 修復) とは別レイヤー。マージゲート自体は 3 巡目で完成しており、以降は「lint が名乗る保証を満たしているか」の反復。**この family は収束するまで巡回を続ける方針を維持**する — 理由: 各 finding が示す failure mode は例外なく「registry の綴り違いが監視を黙って無効化する」= 本 PR が直している 2 日間 blind の同型であり、authoring lint はその唯一の予防層だから
- **18 巡目 (2026-09-10)** — P2 3 件。(a) 🛑 **`load_registry` の `t.get("active")` が隔離ラッパより手前で走る** — `{"triggers": [null, {...}]}` は list 形検査を通るが `null.get()` でここが落ち、`evaluate_trigger` の隔離に届く前に**後続の正常な trigger すべてが未評価**になる = 隔離ラッパが防ぐはずの盲点を、その手前の層で作り直していた (1 巡目・16 巡目と**同じ「盲点の再生産」で 3 例目**)。要素の dict 検査を active フィルタの手前へ。(b) **lint 側で strip して判定していたが評価器は元の文字列を消費する** — `" 2026-09-10"` は先頭空白が辞書順で数字より前に来るため `today > deadline` が常に真 = **前日から期限切れ扱い**、`since` では `fromisoformat` が落ちる。**書かれたまま**の正準性を要求。(c) **`key`/`prefix` は「少なくとも 1 つ」でなく「排他」でなければならない** — 両方あると評価器は `if prefix:` で分岐して `key` を黙って無視し、明示したキーが完全に未監視になる。`EXCLUSIVE_ELEMENT_GROUPS` を新設
- テスト: **+3 本** (いずれも害の実演を同梱 — `"2026-09-09" > " 2026-09-10"` 等)。**counterfactual 3/3**。全 3066 tests green
