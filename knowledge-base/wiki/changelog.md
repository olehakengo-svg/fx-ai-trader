...
## 2026-09-26 — feat(engine): entry 時 SL/TP 構築の分岐を fill 行に永続 (`[SLTP_CONSTRUCT]` / `[BROKER_TP]`) — 宣言 SL/TP との乖離を fill 毎に復元可能に (rule:R3 record-only)

- **動機**: `_tick_entry` の共有 SL/TP 経路 (C0a SR/ATR 選択 + MIN/MAX clamp、C0c 低流動性 / fast-SL / カウンタートレンド / ラウンドナンバー、C0d MTF TP ×1.3、C0b quick-harvest ×0.85) は分岐を row に残さず、`kalman_d7_po_dn_flip` (PR #299 review 11 巡目: 「C0 の live 実測率は導出不能」) と `usdjpy_carry_dip_accumulator` (宣言 150p SL が **16/16** で 9.8–28.5p に置換、[[2026-09-25]] 発見 1、機構「order construction 上流」で止まっていた) の 2 セルで **live が何を発注したかの provenance が無かった**。09-25 の carry_dip 2 敗 (#893195 −16.1p / #893201 −17.1p、SL 距離 16–17p) も同型
- **実装** (`modules/demo_trader.py`): `_sltp_trace` を SL/TP 構築の各分岐で更新し、row insert 直前に `[SLTP_CONSTRUCT] sl=<preserve|sr|atr_nosr|atr_rrlow> clamp=<none|min|max> lowliq fastsl ct rn mtf_tp range_tp decl_sl_p sl_p decl_tp_p tp_p` を reasons に永続。promoted 送信経路では `[BROKER_TP] basis=<qh|exempt|range_mr|wg_none|passthrough> mult tp_p` を `append_trade_reason` で追記 (demo 行 `tp` 列は宣言のまま、broker TP だけが別値 — [[carry-dip-broker-reconcile-2026-09-22]] §5 の 0.85 を row で読めるようにする)。**record-only**: is_shadow / gate / lot / SL・TP 値 / 送信可否に触れない (pin `test_marker_is_record_only`)
- **読み手を同一コミットで併設** ([[c1-candidate-readout-hull-funnel-2026-08-24]] の write-only 教訓): `tools/sltp_construct_readout.py` — entry_type × (live/shadow) で SL 選択枝比率 / clamp / C0c 発動率 / MTF ×1.3 率 / 宣言 vs 実発注 距離中央値。**EV / WR / outcome は読まない** (凍結 pre-reg の outcome 量に触れない)
- **pin**: `tests/test_sltp_construct_marker_r3.py` 15 本 (SR 採用 / RR 床退避 / ATR fallback / preserve / min・max clamp / lowliq 21Z vs 12Z / fast-SL 同ペア vs 別ペア / rn .000 vs .300 / MTF strong 一致 / broker TP 送信・拒否 / record-only)。counterfactual: rn trace と `basis="qh"` 代入を外すと 3 本落ちる
- **読み方の注意**: marker は **デプロイ後の row にのみ**付く (09-26 以前の fill には無い — 過去分の SL 選択枝は依然復元不能)。`sl=unset` は構築経路を通らずに insert された行 (shadow_emit 経路 L1693 は SL/TP を構築しないので marker 自体が付かない)。kalman 制約付き BT (Codex queue `20260925-0300`) の C0 感度差し替えは live N が溜まってから
- **レビュー 1 巡目 (Codex P2 4110716471)**: `decl_sl_p` / `decl_tp_p` を current_price (bid/ask = 実約定基準) から測っていたため、closed-bar signal の rebase (`rebase_tp_to_current_price`、距離保存) や slippage で current_price が `sig["entry"]` からずれると宣言側が偽の拡大/縮小に見える (例: 60p 宣言 TP を +20p 後に執行 → decl 40 / tp 60 = 「1.5×」)。宣言側の基準を **signal entry** に分離し、ずれ自体は `entry_drift_p` で別出し。pin 2 本追加 (formatter 直叩き + bid/ask +3p の実経路)
- **レビュー 2 巡目 (Codex P2 4110734587)**: `[BROKER_TP] tp_p` を quick-harvest の計算基準 `_signal_price` (sig.entry) から測っていた → quote がずれると `[SLTP_CONSTRUCT] tp_p` (current_price 基準) と基準が違い、readout の中央値比較が壊れる (rebase +20p の例で 68p vs 実 48p)。基準を current_price に統一 (`basis` / `mult` は構築 provenance として維持)。pin 1 本 (bid/ask +3p → tp_p 57 / broker 48 / drift 3)
- **carry_dip の機構 (code reading、marker で確認予定)**: mode `daytrade_1h` は `_get_base_mode` で接尾辞が剥がれないため ATR fallback 倍率は `.get("daytrade_1h", 0.8)` = **0.8**、MIN_SL_DIST `.get(…, 0.030)` = **3p**、MAX_SL_DIST `"daytrade_1h": 0.500` = **50p**。宣言 SL 150p は `_1H_PRESERVE_SLTP` 不在で捨てられ、SR (nearest_support − 0.3×ATR、RR≥1.0) か ATR×0.8 に置換 → 観測帯 9.8–28.5p と整合。**SL 復元は user 決裁事項 (08-07) で autopilot 禁止 — 本 PR は計測のみ**
...
- **レビュー 4 巡目 (P2 4090163186)**: 読み手走査の除外集合に `research` が入っていたが、`modules/demo_trader.py` が live 経路で `research.edge_discovery.{strategy_family_map,regime_labeler,mtf_regime_engine}` を import している (L6365 / L11218 / L11343) — 対称チェック (除外 top-dir × 走査対象からの import) で該当は `research` のみ。`research` を除外から外し、除外集合の自己検査 pin `test_non_web_top_dirs_are_not_imported_by_web` (「除外 top-dir は走査対象から import されない」) を新設。counterfactual: `research` を戻す → 2 テスト失敗 / `research/edge_discovery/regime_labeler.py` に `Path("data") / "monitoring"` を足す → 読み手 guard が名指しで失敗。**5 巡目 (P2 4100241463)**: 分割リテラルの区切りが `,` / `/` のみで `Path("data").joinpath("monitoring", …)` 形を取り逃していた → 区切りに `).joinpath(` を追加。counterfactual 3 形 (double / single quote / 空白入り) で落ち、control `joinpath("cache")` は通る。**6 巡目 (P2 4100278149)**: writer (`tools/nav_floor_projection.py`) の import 検知 regex が `from tools import (` + 改行 の複数行 import を取り逃していた → `ast` の Import/ImportFrom 走査に置き換え (`_imports_monitoring_writer`、構文エラーは保守側)。counterfactual 3 形 (複数行 / from-module / import-as) で落ち、control `from tools import (pr_review_gate)` は通る。**巡目上限 (≤6) 到達**。7 巡目 (P2 4100330235 / 4100330240) は 1 行級だったので修正で消化 (相対 import `from .nav_floor_projection` は `ImportFrom.level` で判定 / 自己検査のカンマ import `import os, services.foo` は `ast` で全 alias 走査)、以降は再レビュー依頼を出さない
