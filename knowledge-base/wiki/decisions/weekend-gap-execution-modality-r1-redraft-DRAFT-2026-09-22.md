...
| **同一週末に複数 pair-event が qualify し結果が混在** (例: fill + abandon) | pair-event 単位、**時系列順に逐次適用** | 各 pair-event を上の 4 行の規則に**時系列順**で 1 件ずつ当てる。**順序キー = そのペアの `[WEEKEND_GAP] {instrument} qualifying event:` ログ行の発火時刻** (demo_trader.py:4246–4250 — HOLD を抜けて SEND / ABANDONED_* が決まった tick に 1 回だけ出る = pair-event の成立時刻。shadow row の entry 時刻と同一 tick)。**終端結果 (filled / abandon latch / cancel tx) の時刻で並べない** — SEND 後の halt-race 再送 (`modules/oanda_bridge.py:669–719`、+30s × 1 回) は fill を最大 ~30s 遅らせるので、先に qualify した fill が後に qualify した即時放棄より**後の終端時刻**を持ちうる。終端時刻で並べると [fill(先) , abandon(後)] が [abandon, fill] に反転し、09-13 と合わせて偽の 2 連続 (または逆順で偽の抑止) を生む (PR #281 review P2、3 巡目)。終端結果は当該 qualifying event に**後から付与**する (fill 確定は oanda_audit sent→filled、cancel は tx)。同秒なら Render ログの出力順。「2 連続不成立」= **隣接する 2 pair-event が共に不成立** (`SKIPPED_SPREAD` は数えず連鎖も切らない — 列から除外、NO-QUALIFY は列に現れない)。**fill は F2 resolve (registry 明示編集) と連鎖リセットを同時に起こすが、週末単位で「fill があれば発動しない」とは読まない** — F2 resolve と §6 発動は独立で、同一週末に両方起きうる。worked example (列の順序 = qualifying event 発火順、09-13 = 不成立 1 件目を先頭に置く): (a) `fill → ABANDONED_DRIFT → ABANDONED_HALT` = 列 [DRIFT(09-13), fill, DRIFT, HALT] → fill でリセット後に DRIFT・HALT が隣接 → **発動** (かつ F2 resolve); (b) `ABANDONED_DRIFT → fill → ABANDONED_HALT` = [DRIFT(09-13), DRIFT, …] → 最初の pair-event で **発動** (後続 fill で F2 resolve、HALT は新連鎖の 1 件目); (c) `fill → fill → ABANDONED_DRIFT` = 発動せず、DRIFT が 1 件目; (d) 順序キーの反例: pair A が 21:05:02 に qualify → SEND → halt-race 再送で 21:05:35 に filled、pair B が 21:05:10 に qualify → 即時 ABANDONED_DRIFT。qualifying 順 = [DRIFT(09-13), A-fill, B-DRIFT] → リセット後 1 件目 = **発動せず**。終端時刻順なら [DRIFT(09-13), B-DRIFT, A-fill] = 偽の 2 連続。⚠️ packet §6 / registry g0prime は「イベント」の単位 (pair-event / 週末) を明示していない — **本稿の読みは pair-event 逐次。週末単位の別規則を運用に使うには packet §6 文言の user 承認が先** (未承認、本稿は使わない)。昇格時に文言を確定する (凍結閾値には触れない文言整備) | 上記の逐次判定に従う (fill の有無だけでは決めない) |
...
- **転記するもの (価格系・執行系のみ)**: ペア別 gap (p) と閾値、**qualifying event ログ行の発火時刻 (pair-event 列の順序キー、§2)**、decision (SEND / HOLD 回数 / ABANDONED_* / SKIPPED_SPREAD)、tradeable・quote_age、drift (p)、send_mid、latch 値、oanda_audit id / tx id と終端結果の時刻 (fill / cancel — 順序キーには使わない)、fill 時は `oanda_trade_id` と当該 1 event の persisted slippage 値 (配管確認、集計しない)。
...
- pair-event 列を終端結果 (filled / abandon latch / cancel tx) の時刻で並べること — 順序キーは qualifying event ログ行の発火時刻のみ (halt-race 再送 +30s で反転する、§2 (d)、PR #281 review P2 3 巡目)。
