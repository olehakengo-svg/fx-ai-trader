...
| 8 | 実装チェックリスト + pre-reg 突合 (stage-2 §8 様式) | — | 凍結値 pin テストの更新範囲、counterfactual pin を同一 commit。**専用 first-qualification ts の system_kv 永続化** (qualifying event 判定時に 1 回、guard early-return の再発火で上書きしない — R3、凍結値不変更、PR #281 review 4 巡目) は発動と無関係に単独 R3 PR で実施可 — **09-27 21:00Z 前を推奨**。無ければ混在週末の event #2 は W5 上程 (§2 (β)) |
