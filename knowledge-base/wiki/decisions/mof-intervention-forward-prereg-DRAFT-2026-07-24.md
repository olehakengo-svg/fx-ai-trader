...
- 識別 rule 校正 (2022/2024 のみ、事前宣言 grid Y∈{0.25..2.00 step 0.25}% × X∈{1.5..4.0 step 0.5}、機械的基準 hits≥6→FP最小→hits最大→最緩): **凍結 (X, Y) = (2.0, 0.25%)**。hit 6/7 (2022-10-24 は close/open +0.86% で構造的捕捉不能 — covert 追撃日)、FP 39/775 = **5.03%** (event±10d 除外版 5.06%)。binding は 2024-07-12 (co_ret −0.258% / ratio 2.18) — ±20% 摂動 (校正データのみ) で同日が脱落し 5/7、他 5 日は margin 大 (co_ret ≤ −1.17% / ratio ≥ 4.69)。2026 窓側の摂動チェックは §8.3 どおり verdict 時。
...
- データ注記: 2026-05-07 (木) は parquet バー 0 本 (ソース欠損、休場ではない) → 母集団外で M=21。開示 D が 05-07 を含む場合 S と overlap し得ない = PASS に不利な方向の欠損として verdict で記録 (gate 定義不変)。
- 超幾何閾値 (M=21, |S|=2, α=0.10): k=1 → overlap≥1 (p=0.0952) / k=2..5 → **overlap≥2 必須** (p = 0.0048 / 0.0143 / 0.0286 / 0.0476)。overlap=1 は k≥2 で不合格 (p 0.186–0.429)。**E-D 予測 k∈[2,5] の下では E-A PASS ≒ 「04-30 と 05-06 の両方が介入日」** — §8.3 のナイフエッジ明記対象。
