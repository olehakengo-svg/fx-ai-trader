...
- **tool の挙動 (pin 済み)**: marker が既存なら exit 3 で拒否 (API へ問い合わせもしない)。**marker が無くても attempt 台帳に試行があれば exit 3** (前回は本番へ問い合わせ済み — 失敗の理由は台帳 `reason`)。現在時刻 < cutoff なら exit 2 で拒否 (早期凍結が marker を占有するのを防ぐ)。`--slice-ohlcv` で parquet が欠けている / 末尾が cutoff 直前の完結 bar (06:15Z) に届かない / 開始が t0 より後なら **API 要求前に** exit 2 (preflight、試行に数えない — `bt_data_cache.py refresh 15m` をやり直す。lag を許容するなら `--ohlcv-max-lag-bars N` を明示し verdict に併記)。0 行の instrument があれば exit 2 で台帳に `failed/instruments_missing` (再実行は `--force --allow-missing-instruments` — ingest 障害を先に切り分ける)。roundtrip 不一致は exit 2、staging 残置、marker 不書込。lock 保持中の二重起動は exit 3 (API 要求なし)。stdout は件数・秒精度時刻・sha256・パスのみ。
