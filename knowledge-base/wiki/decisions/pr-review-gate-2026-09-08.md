...
- **止めた finding: 計 14 件** (P1 3 / P2 11)。全て妥当で、**反証して dismiss したものはゼロ**
- うち 2 件は「本 PR が直そうとした欠陥クラスを修正自身が再発させていた」型 (exit 2 で stdout 破棄 / 評価器の添字の写し漏れ)
- **費用**: 1 巡あたり pre-commit ~10 分 + レビュー ~5 分。6 巡で ~90 分。潜伏中央値 124 日の欠陥を作成時に止める対価としては安い
...
- **観察**: 指摘の 9/14 は「lint/ゲート/検知器が自分の名乗る保証を満たしていない」型 (5 巡目 P1 は極端で、**故障を名乗る行そのものが読み手に届かない位置に置かれていた**) (presence しか見ない / 一部の severity を落とす / 一部のページしか見ない)。**検査器を書くときは『何を保証すると名乗ったか』を毎回自分に適用せよ** — これが本 PR 最大の教訓
