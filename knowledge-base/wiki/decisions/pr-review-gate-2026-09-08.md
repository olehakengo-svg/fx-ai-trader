...
> **経緯追記 (2026-09-10, PR #227 救済)**: R2 マージゲートは 2 セッションが独立に実装し、
> **main へ先着したのは PR #231 版** (`tools/pr_review_gate.py` = evaluate(pr)→(code,msg)、
> exit 0/2/3/4、REST/comment ベース) — 現行の正はそちら。本書は PR #227 (座礁・未マージ) から
> 救済した**インシデント記録 + 定量根拠 + 補完修正の決裁文書**である: (a) §2 の 40 PR 実測
> (write-only の定量確認)、(b) §3 の実害 (未読 P1 が prereg 監視器を 51 エントリ 2 日間停止)、
> (c) 監視器側の恒久修復 (`prereg_trigger_watch` の隔離ラッパ / EVAL_ERROR 分離 /
> registry authoring lint / `quant_gate_status` の returncode 検査と前方 banner) は
> 本書とともに救済ブランチで main へ移植した。§4(a) と §5b 巡回記録に現れるゲート実装
> (GraphQL reviewThreads / is_designated_reviewer 完全一致 / P0 fail-closed / ページング /
> HEAD_UNREVIEWED) は **#227 版の記述**であり、#231 版 (REST、部分一致 REVIEWER_PAT、
> レビュー後 commit/review-ack で消化扱い) には未実装 — 採用可否は別途判断
> (本書は記録として原文保存)。
...
### 12 巡目 — 家系を「1 件ずつ」から「reject-by-default」で閉じた

| # | 指摘 | 判定 | 修正 |
|---|---|---|---|
| P1 | 故障 banner だけを前方へ上げても、**行動を要する TRIGGERED** が 1900 字カットの外に残る (「見えているのに動けない」) | **妥当・実害の再現** — 本セッションの 2 日間、T5 第1要件 TRIGGERED は誰にも届いていなかった | `extract_watch_triggered()` を新設し TRIGGERED 節も M1 より前へ。registry の message は数百字あるので 1 行 220 字にクリップ (全文は下の watch 節) |
| P2 | 綴り違いの selector (`instrumnt: "USD_JPY"`) は評価器に**黙って無視され全ペアを計上**する | **妥当・危険** | **allow-by-default をやめ reject-by-default へ** — `OPTIONAL_FIELDS_BY_TYPE` + `META_FIELDS` を定義し未知キーを違反にした |
| P2 | `dedup_violation` は評価器が `== 0` でしか dedup を有効にしないが、型検査は 1/2 も通す (黙って無視され重複行が母集団に入る) | **妥当** | `ENUM_FIELDS` で `{0}` に固定 |

counterfactual 3/3 確認。

**reject-by-default が「打ち切り判断」の実質的な解になった**: 11 巡目までは
「既知フィールドを 1 つずつ足す」= allow-by-default で、未知キーの家系は
原理的に終わらなかった。12 巡目の `OPTIONAL_FIELDS_BY_TYPE` で
**未知キーは全て違反**になり、この家系は構造的に閉じた。
残るのは「既知キーの値意味論」の粒度だけで、これは有限かつ registry の実利用に
限定される。
...
- **止めた finding: 計 32 件** (P1 5 / P2 27)。全て妥当で、**反証して dismiss したものはゼロ**
- うち 2 件は「本 PR が直そうとした欠陥クラスを修正自身が再発させていた」型 (exit 2 で stdout 破棄 / 評価器の添字の写し漏れ)
- **費用**: 1 巡あたり pre-commit ~10 分 + レビュー ~5 分。12 巡で ~180 分。潜伏中央値 124 日の欠陥を作成時に止める対価としては安い
...
- **観察**: 指摘の 27/32 は「lint/ゲート/検知器が自分の名乗る保証を満たしていない」型 (5 巡目 P1 は極端で、**故障を名乗る行そのものが読み手に届かない位置に置かれていた**) (presence しか見ない / 一部の severity を落とす / 一部のページしか見ない)。**検査器を書くときは『何を保証すると名乗ったか』を毎回自分に適用せよ** — これが本 PR 最大の教訓
