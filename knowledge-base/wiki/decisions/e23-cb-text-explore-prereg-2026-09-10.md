...
待たず臨時スキャン、R3)** の発動条件に該当する。

> 🔴 **2026-09-17 訂正 (scan 第5次 §2、rule:R3)**: 上の「能動測定ライン = 0 本」は **台帳 #27
> (family A statement_ladder) を数え落としている**。#27 は 2026-09-10 (scan 第4次) に採用済みで、
> pre-reg §8 の次ステップ (敵対的検証 → 凍結コミット、期日 09-24) は本 verdict 時点で
> **着手可能・ブロッカー無し**だった。統治規則は「S1-S4 の本数」ではなく
> **「今日着手できる本数 ≥1」** (パイプライン §5 追補、2026-08-14 user 承認) なので、
> **臨時スキャンの発動条件は成立していなかった**。実害はゼロ (期日 09-18 が翌々日で
> スキャンはいずれにせよ走る / 焼いた窓・消費した枠なし) だが、この会計自体が発動ゲートであり、
> 「供給ライン全滅」という読み方は U1/F1 の入力を歪めるため訂正する。
> **正しい会計 = 着手可能 1 本 (#27) / 時限ロック 3 / 条件付き 2 / park 2。**
> 詳細: [[../research/external-hypothesis-scan-round5-2026-09-17]] §2外部仮説 explore→OOS 生存の base rate 4%
