...
| docs/KB のみ (`knowledge-base/wiki/{sessions,lessons,analyses,syntheses,strategies,learning}/**` + `decisions/*.md` = `render.yaml:39–77` ignoredPaths でデプロイなし) | CI green で即マージ可 (CLAUDE.md コードレビュー節どおり)。ただし **`knowledge-base/wiki/research/**` は ignoredPaths に無くデプロイあり** (`render.yaml:39–77` 実読、2026-09-22) — research 文書 PR は code PR のマージ窓を適用。`decisions/prereg-trigger-registry.json` は cron が読む load-bearing 状態で意図的に ignore 対象外 (`render.yaml:59–63` コメント) なので code PR 扱い | — |
...
| ignoredPaths 現行 | `render.yaml:39–77` (decisions/*.md = L64–65、`wiki/research/**` は不在、`data/external/**` は cb_statements のみ L77) |
