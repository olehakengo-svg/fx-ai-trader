...
post-hoc の検出力曲線 (実 armed 系列 + 合成ラベル、600 反復、**実ラベルの
...
⇒ **観測された効果量での検出力は 0.158** (84.2% 取り逃す)。
**実用域 (0.9) に届くのは全 4 block が hit する場合のみ**、3/4 でも 0.710。

**本 FAIL が意味するのは「凍結 α 規則を満たさなかった」ことだけ**。
全 block hit 級 (J≈0.70) に対しては感度 0.968 と高いので**弱い反証にはなる**が、
**その効果が真でも 3.2% は非有意になるため「無い」とは言えない** — 不在の主張には
同等性/非劣性検定か信頼区間が要り、本 pre-reg はどちらも規定していない (Codex P2 第7波)。
「分離が無い」「検出器に情報が無い」は当然言えない (3/4 block ですら検出力 0.710)。
