...
⇒ **観測された効果量での検出力は 0.145**。その効果が実在しても約 85% の確率で
閾値を割る設計だった。**完全な検出器 (4/4) でも 0.863 で慣例的な 0.9 に届かない** —
つまり**この estimand をこの窓で検定すること自体に無理があった**。

**本 FAIL が意味するのは「凍結された α 規則を満たさなかった」ことだけ**であり、
「分離が無い」「検出器に情報が無い」ではない。

> 凍結時に power analysis を規定しなかったことが設計の不足。
> 「記述級」と宣言することと、**どの効果量なら検出できるかを事前に数値化すること**は別物。
> 事前に測っていれば「完全検出器でも power 0.86」が分かり凍結前に作り直せた。
