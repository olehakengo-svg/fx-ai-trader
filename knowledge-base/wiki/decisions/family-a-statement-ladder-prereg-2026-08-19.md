...
⇒ **観測された効果量での検出力は 0.152**。その効果が実在しても約 85% の確率で
閾値を割る設計だった。**実用域 (0.9) に届くのは全 4 block が hit する場合のみ**。

**本 FAIL が意味するのは「凍結 α 規則を満たさなかった」ことと
「全 block hit 級 (J≈0.71) の効果は無い」ことだけ**であり、
「分離が無い」「検出器に情報が無い」ではない (3/4 block ですら検出力 0.62)。
