...
| gate | 実測 | 判定 |
|---|---|---|
| Gate A | armed 営業日率 **0.1327** (147 / 1,108 bd) | ✅ (設計期待 9–15% の中に着地) |
...
### 11.1 検出力不足ではない (positive control)

**凍結検出器の実 armed 系列** (147/1,108 bd) に**合成ラベル**を置いて測定:
各 event 窓に陽性 1 つ (k=7) で **J=0.8728 / p=0.0190**、armed 全日が陽性なら **p=0.0009**。
⇒ 観測された陽性数でも強い効果なら α=0.05 を通せた。**測って分離が無かった**のであり、
E23 の UNDERPOWERED (測れなかった) とは性質が異なる。

⚠️ 初版の **p=0.0027 は誤り** (Codex P2 / PR #270) — 合成の等間隔 armed マスク上の値で、
実 armed 系列ではなかった。circular-shift の有意性は armed 窓の位置と間隔に依存する。
回帰 pin が凍結 events から armed を再構成して 0.0190 を固定している。
...
§6 の forward OOS は本 FAIL で実質無意味化 (explore で分離が無い以上 forward で確認する仮説がない)。
ただし **forward 期 (2026-07-30〜) の gate×outcome 計算の禁止は解除しない** —
将来の新 pre-reg が同じ窓を genuine OOS として使える状態を保つため、**未接触のまま凍結**する。
