...
post-hoc の検出力曲線 (実 armed 系列 + 合成ラベル、陽性 7、600 反復):

| armed に入る陽性 | 想定 J | 検出力 |
|---:|---:|---:|
| 2/7 | +0.154 | 0.000 |
| **3/7 (観測値)** | **+0.298** | **0.338** |
| 4/7 | +0.442 | 0.927 |
...
⇒ **観測された効果量での検出力は 0.34**。他の条件が同じでも
約 2/3 の確率で閾値を割る設計だった。実用域 (>0.9) は 4/7 以上。

**本 FAIL が意味するのは「凍結された α 規則を満たさなかった」ことだけ**であり、
「分離が無い」ではない。強い効果 (≥4/7) は無いと言えるが、中程度の効果は
本設計では判別できない。

> 凍結時に power analysis を規定しなかったこと自体が設計の不足。
> 「記述級」と宣言することと、**どの効果量なら検出できるかを事前に数値化すること**は別物。
> 本曲線は post-hoc で、**verdict の判定には使っていない** (FAIL は凍結規則のまま)。
> 次の pre-reg では**凍結時に検出力曲線を計算し実用域の効果量を宣言する**こと。
