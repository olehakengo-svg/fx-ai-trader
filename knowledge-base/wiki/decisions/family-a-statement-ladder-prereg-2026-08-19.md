...
実 armed 系列 + **合成ラベル**で測った最良ケース (実ラベルは再使用していない):
「陽性が全て armed 窓内」なら **k=7 で p=0.0027 / k=10 で p=0.0027**。
⇒ 観測された陽性数でも強い効果なら α を通せた。**測って分離が無かった**のであり、
...
⚠️ **event 数と日数の混同に注意** (Codex P2 / PR #270 で修正)。2022-09-29 の 1 event が
10-21 と 10-24 の両方を覆うため hit 日は 2 だが、**hit した event は 1/2** (2022-05-19 は空振り)。

pass-1 で「2022 の 2 本だけで結論が反転しないか点検せよ」と書いた点検結果は、
**全 7 event のうち何かを当てたのは 2 本だけ** (2022 が 1 / 2026 が 1)、残り 5 本は false alarm。
**全体 J のほぼ全てを 2022 の 1 event が担っている**。検出器は「介入前の梯子」ではなく
せいぜい 2022 の 1 回のエスカレーションの記述。リードは 15 営業日 (2022-09-29→10-21) と
3 営業日 (2026-04-24→04-30)。
