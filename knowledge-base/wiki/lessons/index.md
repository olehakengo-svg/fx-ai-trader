...
- 問題: `REGIME_ADAPTIVE_FAMILY` 拡張タスク実装中に前提が 3箇所で破壊 (instrument×regime 交絡, BT-Live 乖離は市場遷移原因, post-Cutoff 全 MR/TF 崩壊)
- 症状: 当初タスクは意思決定に使えない分析と判明. 既存 mapping は bug なし, 真の損失源は range_tight × MR (mapping override 対象外)
- 原因: タスクを受けた時点で「この問題が正しい問題か」を Live 現況から検証しなかった
- 修正: REGIME_ADAPTIVE 拡張は凍結. P0 = range_tight 止血 + Gate leak 調査に再編
- 教訓: **新タスク着手前に「問題定義 / 前提検証 / 逆命題」を自問する. ユーザー challenge は depth-dive 指示として扱う** ([[lesson-user-challenge-as-signal]] 再確認事例)
