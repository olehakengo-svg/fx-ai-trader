---
id: 20260505-1225-w4-shadow-redesign-ema_cross
title: "[W4-Shadow Phase 1] ema_cross (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:25:00+0900
...
W4-EDA audit (`audits/edge_design/ema_cross.md`) で **THESIS_VALID_TIMING_BROKEN** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/ema_cross-shadow-redesign-2026-05-05.md`:
...
`tests/test_ema_cross_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`EMA_CROSS_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['ema_cross'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['ema_cross'], variant=True)
"
```

`knowledge-base/raw/bt-results/ema_cross-shadow-bt-2026-05-05.json` に保存。
