---
id: 20260505-1229-w4-shadow-redesign-orb_trap
title: "[W4-Shadow Phase 1] orb_trap (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:29:00+0900
...
W4-EDA audit (`audits/edge_design/orb_trap.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/orb_trap-shadow-redesign-2026-05-05.md`:
...
`tests/test_orb_trap_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`ORB_TRAP_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['orb_trap'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['orb_trap'], variant=True)
"
```

`knowledge-base/raw/bt-results/orb_trap-shadow-bt-2026-05-05.json` に保存。
