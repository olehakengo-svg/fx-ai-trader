---
id: 20260505-1231-w4-shadow-redesign-trend_rebound
title: "[W4-Shadow Phase 1] trend_rebound (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:31:00+0900
...
W4-EDA audit (`audits/edge_design/trend_rebound.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **A** と判定。
...
破綻点は `momentum_limit` の符号設計である。現在の BUY 条件 `_momentum < +8` は「急落しすぎていない」ではなく「強い上昇ではない」だけを見ており、下降トレンド中にさらに強く落ちている足を許す。SELL 条件 `_momentum > -8` も同様に、上昇トレンド中にさらに強く上がっている足を許す。再設計案は、BUY を `-momentum_limit <= _momentum <= 0` または `_momentum > -momentum_limit` かつ `macdh > 
...
`knowledge-base/wiki/decisions/trend_rebound-shadow-redesign-2026-05-05.md`:
...
`tests/test_trend_rebound_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`TREND_REBOUND_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['trend_rebound'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['trend_rebound'], variant=True)
"
```

`knowledge-base/raw/bt-results/trend_rebound-shadow-bt-2026-05-05.json` に保存。
