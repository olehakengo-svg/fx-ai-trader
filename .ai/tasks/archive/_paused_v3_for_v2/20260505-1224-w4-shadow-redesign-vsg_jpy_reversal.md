---
id: 20260505-1224-w4-shadow-redesign-vsg_jpy_reversal
title: "[W4-Shadow Phase 1] vsg_jpy_reversal (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:24:00+0900
...
W4-EDA audit (`audits/edge_design/vsg_jpy_reversal.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/vsg_jpy_reversal-shadow-redesign-2026-05-05.md`:
...
`tests/test_vsg_jpy_reversal_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`VSG_JPY_REVERSAL_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['vsg_jpy_reversal'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['vsg_jpy_reversal'], variant=True)
"
```

`knowledge-base/raw/bt-results/vsg_jpy_reversal-shadow-bt-2026-05-05.json` に保存。
