---
id: 20260505-1259-w4-shadow-redesign-sr_break_retest
title: "[W4-Shadow Phase 1] sr_break_retest (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:59:00+0900
...
W4-EDA audit (`audits/edge_design/sr_break_retest.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
再設計案は、trigger の思想を維持しつつ timing と exit を変える。まず retest 反転確認を current tick から確定済み signal bar へ移し、`signal_bar = ctx.df.iloc[-2]` の `Close > Open`, `Close > SR`, `Close > EMA9` を BUY
...
`knowledge-base/wiki/decisions/sr_break_retest-shadow-redesign-2026-05-05.md`:
...
`tests/test_sr_break_retest_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`SR_BREAK_RETEST_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['sr_break_retest'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['sr_break_retest'], variant=True)
"
```

`knowledge-base/raw/bt-results/sr_break_retest-shadow-bt-2026-05-05.json` に保存。
