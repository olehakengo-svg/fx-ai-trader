---
id: 20260505-1228-w4-shadow-redesign-lin_reg_channel
title: "[W4-Shadow Phase 1] lin_reg_channel (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:28:00+0900
roadmap_gate: "W4 Shadow-first redesign — quant 推奨 (BT は軽量 filter、shadow が真の estimator)"
rule: R1
prereq_artifacts:
  - audits/edge_design/lin_reg_channel.md
...
W4-EDA audit (`audits/edge_design/lin_reg_channel.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **A** と判定。
...
再設計案は、signal bar を確定足に固定し、entry は次 bar execution として扱うこと。BUY なら確定足 `Close[signal] <= lower + zone` かつ `Close[signal] > Open[signal]` かつ `Close[signal] > lower`、SELL は対称条件にし、Candidate には `(entry_type, symbol, signal_bar_time, direction)` 相当の dedup key を載せる。Stop/TP は
...
`knowledge-base/wiki/decisions/lin_reg_channel-shadow-redesign-2026-05-05.md`:
...
`tests/test_lin_reg_channel_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`LIN_REG_CHANNEL_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['lin_reg_channel'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['lin_reg_channel'], variant=True)
"
```

`knowledge-base/raw/bt-results/lin_reg_channel-shadow-bt-2026-05-05.json` に保存。
