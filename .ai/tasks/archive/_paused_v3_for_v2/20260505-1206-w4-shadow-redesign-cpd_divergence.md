---
id: 20260505-1206-w4-shadow-redesign-cpd_divergence
title: "[W4-Shadow Phase 1] cpd_divergence (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:06:00+0900
...
W4-EDA audit (`audits/edge_design/cpd_divergence.md`) で **THESIS_VALID_TIMING_BROKEN** / 推奨度 **A** と判定。
...
再設計案は timing/data alignment を先に直す。`ctx.df.iloc[-2]` と leader_df の同一 timestamp 以前だけで signal を作り、execution は次 bar の `ctx.en
...
`knowledge-base/wiki/decisions/cpd_divergence-shadow-redesign-2026-05-05.md`:
...
`tests/test_cpd_divergence_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`CPD_DIVERGENCE_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['cpd_divergence'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['cpd_divergence'], variant=True)
"
```

`knowledge-base/raw/bt-results/cpd_divergence-shadow-bt-2026-05-05.json` に保存。
