---
id: 20260505-1248-w4-shadow-redesign-gold_pips
title: "[W4-Shadow Phase 1] gold_pips (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:48:00+0900
...
W4-EDA audit (`audits/edge_design/gold_pips.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
再設計案は、まず signal bar を確定足に固定し、`df.iloc[-2]` で body/engulfing/high/low を計算して次 bar の `ctx.entry` で約定する variant に切ること。加えて `(ctx.symbol, signal, bar_id)` の per-bar dedup を入れ、同一 1m bar では一度しか emit しない。Stop/TP は `risk = abs(entry - sl)` を計算して `tp_distance >= 1.5*risk` を満たさない場合は skip、または TP を `max(1.8ATR, 1.5*
...
`knowledge-base/wiki/decisions/gold_pips-shadow-redesign-2026-05-05.md`:
...
`tests/test_gold_pips_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`GOLD_PIPS_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['gold_pips'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['gold_pips'], variant=True)
"
```

`knowledge-base/raw/bt-results/gold_pips-shadow-bt-2026-05-05.json` に保存。
