---
id: 20260504T2329-w4-eda-sr_fib_confluence
title: "[W4-EDA] Edge Design Audit — sr_fib_confluence"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:29:00+0900
roadmap_gate: "W4 Edge Design Audit — sr_fib_confluence の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`sr_fib_confluence` (`Tier 3 (FORCE_DEMOTED)` / `force_demoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/sr_fib_confluence.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "force_demoted", "strategy": "sr_fib_confluence", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/sr_fib_confluence.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/sr_fib_confluence.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/sr_fib_confluence.py:N`>
...
- `audits/edge_design/sr_fib_confluence.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T15:20:58Z)

exit=0 · 120s

```
作成しました: [audits/edge_design/sr_fib_confluence.md](/data/repo/fx-ai-trader/audits/edge_design/sr_fib_confluence.md)

要点:
- Verdict: `THESIS_VALID_DESIGN_BROKEN`
- Redesign Recommendation: `B`
- 主破綻軸: Axis 2 / 3 / 4
- empirical evidence は既存 tier-master / audit artifacts のみ使用し、BT は実行していません。
- 戦略コード本体は変更していません。
```
