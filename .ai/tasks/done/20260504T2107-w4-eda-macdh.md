---
id: 20260504T2107-w4-eda-macdh
title: "[W4-EDA] Edge Design Audit — macdh"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T21:07:00+0900
roadmap_gate: "W4 Edge Design Audit — macdh の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/scalp/macdh.py
related:
...
`macdh` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/macdh.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "macdh", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/macdh.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/macdh.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/macdh.py:N`>
...
- `audits/edge_design/macdh.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T12:42:33Z)

exit=0 · 135s

```
作成しました: [audits/edge_design/macdh.md](/data/repo/fx-ai-trader/audits/edge_design/macdh.md)

要点:
- Verdict: `THESIS_VALID_DESIGN_BROKEN`
- Redesign Recommendation: `B`
- 破綻軸: Axis 3 timing / Axis 5 stop-TP geometry
- Axis 7 は tier-master `—` のため `INSUFFICIENT_EVIDENCE` を明記しつつ、既存 decision/audit log の N=62, WR=32.26%, Wilson lo=21.95%, PF=0.468, Kelly=0.0000 などを表に入れました。
