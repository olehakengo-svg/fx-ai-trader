---
id: 20260504T1957-w4-eda-dt_sr_channel
title: "[W4-EDA] Edge Design Audit — dt_sr_channel"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T19:57:00+0900
roadmap_gate: "W4 Edge Design Audit — dt_sr_channel の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/dt_sr_channel.py
related:
...
`dt_sr_channel` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/dt_sr_channel.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "dt_sr_channel", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/dt_sr_channel.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/dt_sr_channel.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/dt_sr_channel.py:N`>
...
- `audits/edge_design/dt_sr_channel.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T10:28:44Z)

exit=0 · 143s

```
作成しました: [audits/edge_design/dt_sr_channel.md](/data/repo/fx-ai-trader/audits/edge_design/dt_sr_channel.md)
...
- Axis 7: `INSUFFICIENT_EVIDENCE`。Wilson lo / PF / Kelly は既存 audit から埋め、WF folds は ALL/phase0_shadow として不足を明記
- BT は実行していません
- 戦略コードの引用は Axis 1-5 で `strategies/daytrade/dt_sr_channel.py:LINE` 形式にしています

`dt_sr_channel.py` の実装上の entry_type が `dt_sr_channel_reversal` なので、監査本文ではその対応も明記しています。
```
