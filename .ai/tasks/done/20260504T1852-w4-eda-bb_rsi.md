---
id: 20260504T1852-w4-eda-bb_rsi
title: "[W4-EDA] Edge Design Audit — bb_rsi"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T18:52:00+0900
roadmap_gate: "W4 Edge Design Audit — bb_rsi の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`bb_rsi` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/bb_rsi.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "bb_rsi", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/bb_rsi.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/bb_rsi.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/bb_rsi.py:N`>
...
- `audits/edge_design/bb_rsi.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T10:01:16Z)

exit=0 · 123s

```
作成しました: [audits/edge_design/bb_rsi.md](/data/repo/fx-ai-trader/audits/edge_design/bb_rsi.md)
...
- Empirical evidence: Wilson / PF / Bonferroni / Kelly は既存 audit DB 系の数値を記載し、ALL cell の WF folds と decision-grade Kelly 不足は `INSUFFICIENT_EVIDENCE` として明記

コード変更はしていません。BT も実行していません。
```
