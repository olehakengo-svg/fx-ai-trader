---
id: 20260504T2057-w4-eda-london_ny_swing
title: "[W4-EDA] Edge Design Audit — london_ny_swing"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T20:57:00+0900
roadmap_gate: "W4 Edge Design Audit — london_ny_swing の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/london_ny_swing.py
related:
...
`london_ny_swing` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/london_ny_swing.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "london_ny_swing", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/london_ny_swing.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/london_ny_swing.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/london_ny_swing.py:N`>
...
- `audits/edge_design/london_ny_swing.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T12:16:22Z)

exit=0 · 162s

```
作成しました: [audits/edge_design/london_ny_swing.md](/data/repo/fx-ai-trader/audits/edge_design/london_ny_swing.md)

要点:
- Verdict: `THESIS_VALID_TIMING_BROKEN`
- Redesign Recommendation: `A`
- 主破綻軸: Axis 3 timing window
- 統計根拠: tier-master は EV `—`、`demo_trades.db` 側も対象行 0 件のため `INSUFFICIENT_EVIDENCE` として正当化
- BT は実行していません。既存コード、tier-master、audit DB のみ確認しました。
```
