---
id: 20260504T2226-w4-eda-rsk_gbpjpy_reversion
title: "[W4-EDA] Edge Design Audit — rsk_gbpjpy_reversion"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T22:26:00+0900
roadmap_gate: "W4 Edge Design Audit — rsk_gbpjpy_reversion の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/rsk_gbpjpy_reversion.py
related:
...
`rsk_gbpjpy_reversion` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/rsk_gbpjpy_reversion.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "rsk_gbpjpy_reversion", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/rsk_gbpjpy_reversion.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/rsk_gbpjpy_reversion.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/rsk_gbpjpy_reversion.py:N`>
...
- `audits/edge_design/rsk_gbpjpy_reversion.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T13:34:41Z)

exit=0 · 147s

```
作成済みです: [audits/edge_design/rsk_gbpjpy_reversion.md](/data/repo/fx-ai-trader/audits/edge_design/rsk_gbpjpy_reversion.md)

要点:
- Verdict: `THESIS_VALID_DESIGN_BROKEN`
- Redesign Recommendation: `A`
- 主破綻軸: Axis 5 stop/TP geometry
- Axis 7 は Wilson / Bonferroni は既存 `raw/rsk_audit` から記載、PF / WF folds / Kelly は既存 tier-master / audit DB から復元不可として `INSUFFICIENT_EVIDENCE`

新規 BT は実行していません。`git diff --check` は通っています。
```
