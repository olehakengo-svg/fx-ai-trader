---
id: 20260504T2337-w4-eda-mtf_counter_trend_scalp
title: "[W4-EDA] Edge Design Audit — mtf_counter_trend_scalp"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:37:00+0900
roadmap_gate: "W4 Edge Design Audit — mtf_counter_trend_scalp の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`mtf_counter_trend_scalp` (`Tier 4 (SCALP_SENTINEL)` / `scalp_sentinel`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/mtf_counter_trend_scalp.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "scalp_sentinel", "strategy": "mtf_counter_trend_scalp", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/mtf_counter_trend_scalp.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/mtf_counter_trend_scalp.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/mtf_counter_trend_scalp.py:N`>
...
- `audits/edge_design/mtf_counter_trend_scalp.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
