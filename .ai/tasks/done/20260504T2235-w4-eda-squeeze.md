---
id: 20260504T2235-w4-eda-squeeze
title: "[W4-EDA] Edge Design Audit — squeeze"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T22:35:00+0900
roadmap_gate: "W4 Edge Design Audit — squeeze の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/scalp/squeeze.py
related:
...
`squeeze` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/squeeze.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "squeeze", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/squeeze.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/squeeze.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/squeeze.py:N`>
...
- `audits/edge_design/squeeze.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T13:58:53Z)

exit=0 · 154s

```
作成しました: [audits/edge_design/squeeze.md](/data/repo/fx-ai-trader/audits/edge_design/squeeze.md)

内容は指定テンプレートに沿って、8軸診断、Verdict、Redesign Recommendation、Empirical Evidence Table を埋めています。結論は以下です。

- Verdict: `THESIS_VALID_DESIGN_BROKEN`
- Redesign Recommendation: `A`
- 主な破綻軸: Axis 2 trigger mismatch、Axis 3 LOOKAHEAD risk、Axis 5 stop/TP geometry
- BT新規実行: なし
- 戦略コード変更: なし

`git status` 上は新規ファイル `?? audits/edge_design/squeeze.md` のみです。
```
