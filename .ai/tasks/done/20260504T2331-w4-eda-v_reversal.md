---
id: 20260504T2331-w4-eda-v_reversal
title: "[W4-EDA] Edge Design Audit — v_reversal"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:31:00+0900
roadmap_gate: "W4 Edge Design Audit — v_reversal の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`v_reversal` (`Tier 3 (FORCE_DEMOTED)` / `force_demoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/v_reversal.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "force_demoted", "strategy": "v_reversal", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/v_reversal.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/v_reversal.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/v_reversal.py:N`>
...
- `audits/edge_design/v_reversal.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
