---
id: 20260504T2324-w4-eda-lin_reg_channel
title: "[W4-EDA] Edge Design Audit — lin_reg_channel"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:24:00+0900
roadmap_gate: "W4 Edge Design Audit — lin_reg_channel の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/lin_reg_channel.py
related:
...
`lin_reg_channel` (`Tier 3 (FORCE_DEMOTED)` / `force_demoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/lin_reg_channel.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "force_demoted", "strategy": "lin_reg_channel", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/lin_reg_channel.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/lin_reg_channel.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/lin_reg_channel.py:N`>
...
- `audits/edge_design/lin_reg_channel.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T15:07:57Z)

exit=0 · 139s

```
作成しました: [audits/edge_design/lin_reg_channel.md](/data/repo/fx-ai-trader/audits/edge_design/lin_reg_channel.md)

結論は `THESIS_VALID_DESIGN_BROKEN`、Redesign Recommendation は `A` です。Axis 1 はコードから導出可能だったため停止せず、主な破綻軸は Axis 3 の timing window と Axis 5 の stop/TP geometry としました。

BT は実行していません。数値は既存の gate-progression audit、tier-master、既存 BT artifact の read-only 集計だけを使っています。
```
