---
id: 20260504T2312-w4-eda-vbp
title: "[W4-EDA] Edge Design Audit — vbp"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:12:00+0900
roadmap_gate: "W4 Edge Design Audit — vbp の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/micro_scalp/vbp.py
related:
...
`vbp` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/micro_scalp/vbp.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "vbp", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/micro_scalp/vbp.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/vbp.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/micro_scalp/vbp.py:N`>
...
- `audits/edge_design/vbp.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T14:33:24Z)

exit=0 · 134s

```
作成しました: [audits/edge_design/vbp.md](/data/repo/fx-ai-trader/audits/edge_design/vbp.md)

主な判定は `THESIS_VALID_DESIGN_BROKEN`、Redesign Recommendation は `A` です。中核所見は Axis 2/3 で、`H_prev/L_prev` が候補 break bar 自身を含む窓から計算されるため、通常の OHLC では `close > H_prev` / `close < L_prev` がほぼ成立しない構造になっています。

BT は制約通り実行していません。既存 tier-master / audit DB に `vbp` の Wilson/PF/Kelly 等が見つからなかったため、Axis 7 は `INSUFFICIENT_EVIDENCE` として正当化しています。
```
