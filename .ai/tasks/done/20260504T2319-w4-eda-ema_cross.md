---
id: 20260504T2319-w4-eda-ema_cross
title: "[W4-EDA] Edge Design Audit — ema_cross"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:19:00+0900
roadmap_gate: "W4 Edge Design Audit — ema_cross の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/ema_cross.py
related:
...
`ema_cross` (`Tier 3 (FORCE_DEMOTED)` / `force_demoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/ema_cross.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "force_demoted", "strategy": "ema_cross", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/ema_cross.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/ema_cross.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/ema_cross.py:N`>
...
- `audits/edge_design/ema_cross.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T14:53:20Z)

exit=0 · 157s

```
作成しました: [audits/edge_design/ema_cross.md](/data/repo/fx-ai-trader/audits/edge_design/ema_cross.md)
...
- Axis 2/4/5 はコード上は概ね整合
- Empirical evidence は既存 audit DB/tier-master 由来のみ使用し、BT は実行していません
- Wilson lo / PF / Kelly は埋め、WF folds は `INSUFFICIENT_EVIDENCE` として正当化しました

`ema_cross` は thesis 自体は trend-retest として読めるため棄却せず、closed-bar 化 + per-bar dedup + `USD_JPY × NY × SELL` tail への scope 分離を再設計案にしています。
```
