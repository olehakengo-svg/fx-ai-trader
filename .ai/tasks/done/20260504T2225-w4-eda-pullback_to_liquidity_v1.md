---
id: 20260504T2225-w4-eda-pullback_to_liquidity_v1
title: "[W4-EDA] Edge Design Audit — pullback_to_liquidity_v1"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T22:25:00+0900
roadmap_gate: "W4 Edge Design Audit — pullback_to_liquidity_v1 の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`pullback_to_liquidity_v1` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/pullback_to_liquidity_v1.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "pullback_to_liquidity_v1", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/pullback_to_liquidity_v1.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/pullback_to_liquidity_v1.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/pullback_to_liquidity_v1.py:N`>
...
- `audits/edge_design/pullback_to_liquidity_v1.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
