---
id: 20260504T2334-w4-eda-ma_mr_hybrid
title: "[W4-EDA] Edge Design Audit — ma_mr_hybrid"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:34:00+0900
roadmap_gate: "W4 Edge Design Audit — ma_mr_hybrid の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/scalp/ma_mr_hybrid.py
related:
...
`ma_mr_hybrid` (`Tier 4 (SCALP_SENTINEL)` / `scalp_sentinel`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/ma_mr_hybrid.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "scalp_sentinel", "strategy": "ma_mr_hybrid", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/ma_mr_hybrid.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/ma_mr_hybrid.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/ma_mr_hybrid.py:N`>
...
- `audits/edge_design/ma_mr_hybrid.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T15:35:29Z)

exit=0 · 132s

```
監査ファイルを新規作成しました: [audits/edge_design/ma_mr_hybrid.md](/data/repo/fx-ai-trader/audits/edge_design/ma_mr_hybrid.md)

結果は以下です。

- Verdict: `THESIS_VALID_DESIGN_BROKEN`
- Redesign Recommendation: `A`
- 主破綻軸: Axis 4 filter coherence
- 副破綻軸: Axis 3 timing window、Axis 5 stop/TP geometry
- Empirical evidence: 既存 audit CSV の N=1 / PF=99.0 / Kelly=0.0 を記載し、N不足と Bonferroni-adjusted p 欠落により `INSUFFICIENT_EVIDENCE` と明記

BT は実行していません。既存の tier-master / audit CSV / strategy code のみで作成しています。
```
