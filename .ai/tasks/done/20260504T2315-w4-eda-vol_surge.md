---
id: 20260504T2315-w4-eda-vol_surge
title: "[W4-EDA] Edge Design Audit — vol_surge"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:15:00+0900
roadmap_gate: "W4 Edge Design Audit — vol_surge の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`vol_surge` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/vol_surge.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "vol_surge", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/vol_surge.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/vol_surge.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/vol_surge.py:N`>
...
- `audits/edge_design/vol_surge.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T14:41:46Z)

exit=0 · 160s

```
作成しました: [audits/edge_design/vol_surge.md](/data/repo/fx-ai-trader/audits/edge_design/vol_surge.md)
...
- Empirical evidence: ALL phase0_shadow としては `INSUFFICIENT_EVIDENCE`。Archived cell metrics は併記済み。
- BT は実行していません。既存 tier-master / audit DB / archived audit artifacts のみ参照しました。

検証として、ファイル存在、Verdict、Recommendation、Empirical Evidence Table、Wilson/PF/Kelly 行、`strategies/scalp/vol_surge.py:LINE` 形式の引用を確認済みです。
```
