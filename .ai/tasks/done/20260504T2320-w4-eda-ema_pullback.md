---
id: 20260504T2320-w4-eda-ema_pullback
title: "[W4-EDA] Edge Design Audit — ema_pullback"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:20:00+0900
roadmap_gate: "W4 Edge Design Audit — ema_pullback の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/scalp/ema_pullback.py
related:
...
`ema_pullback` (`Tier 3 (FORCE_DEMOTED)` / `force_demoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/ema_pullback.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "force_demoted", "strategy": "ema_pullback", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/ema_pullback.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/ema_pullback.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/ema_pullback.py:N`>
...
- `audits/edge_design/ema_pullback.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T14:56:25Z)

exit=0 · 151s

```
作成しました: [ema_pullback.md](/data/repo/fx-ai-trader/audits/edge_design/ema_pullback.md)

要点:
- Verdict: `THESIS_VALID_DESIGN_BROKEN`
- Redesign Recommendation: `A`
- 主な破綻軸: Axis 3 `LOOKAHEAD`、Axis 5 `MISALIGNED`
- BT は実行していません。既存 tier-master / audit DB 相当の文書と local DB 読み取りだけで埋め、不足分は `INSUFFICIENT_EVIDENCE` として明記しました。
- Wilson lo / PF / Kelly は最新 gate-progression aggregate 由来で埋めています。
```
