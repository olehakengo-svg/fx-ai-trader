---
id: 20260505-1950-w4-shadow-redesign-v2-sr_break_retest
title: "[W4-Shadow-Redesign v2.1] sr_break_retest (Tier 3 (FORCE_DEMOTED)) — relaxed spec"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T19:50:00+0900
...
W4-EDA audit (`audits/edge_design/sr_break_retest.md`):
...
## Step 1: 既存 audit 確認 (`audits/edge_design/sr_break_retest.md`)

audit が指摘する redesign axis を読み、**最小 1 軸** で実装。

## Step 2: 実装 (flag 配下)

`SR_BREAK_RETEST_REDESIGN_V2=1` 環境変数 / config flag で proposed variant を有効化。
default OFF で live は影響受けない。

## Step 3: 失敗テスト + 緑

`tests/test_sr_break_retest_shadow_redesign_v2.py`
...
- 失敗テスト緑
- BT report (`bt-results/sr_break_retest-shadow-redesign-v2-2026-05-05.json`)
