---
id: 20260504T2333-w4-eda-bb_rsi_ema_aligned
title: "[W4-EDA] Edge Design Audit — bb_rsi_ema_aligned"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:33:00+0900
roadmap_gate: "W4 Edge Design Audit — bb_rsi_ema_aligned の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`bb_rsi_ema_aligned` (`Tier 4 (SCALP_SENTINEL)` / `scalp_sentinel`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/bb_rsi_ema_aligned.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "scalp_sentinel", "strategy": "bb_rsi_ema_aligned", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/bb_rsi_ema_aligned.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/bb_rsi_ema_aligned.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/bb_rsi_ema_aligned.py:N`>
...
- `audits/edge_design/bb_rsi_ema_aligned.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
[bb_rsi_ema_aligned.md](/data/repo/fx-ai-trader/audits/edge_design/bb_rsi_ema_aligned.md)

内容は指定テンプレート通りで、主判定は以下です。

- Verdict: `THESIS_VALID_DESIGN_BROKEN`
- Redesign Recommendation: `A`
- 主破綻軸: Axis 5 `stop/TP geometry`
- Axis 7: Bonferroni-adjusted p は既存 tier-master / audit CSV に family size がなく `INSUFFICIENT_EVIDENCE` と明記
- BT 実行なし。既存 audit DB / tier-master のみ参照しました。
```
