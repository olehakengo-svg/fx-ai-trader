---
id: 20260504T2309-w4-eda-turtle_s2_donchian
title: "[W4-EDA] Edge Design Audit — turtle_s2_donchian"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:09:00+0900
roadmap_gate: "W4 Edge Design Audit — turtle_s2_donchian の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/turtle_s2_donchian.py
related:
...
`turtle_s2_donchian` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/turtle_s2_donchian.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "turtle_s2_donchian", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/turtle_s2_donchian.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/turtle_s2_donchian.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/turtle_s2_donchian.py:N`>
...
- `audits/edge_design/turtle_s2_donchian.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T14:24:54Z)

exit=0 · 164s

```
作成しました: [audits/edge_design/turtle_s2_donchian.md](/data/repo/fx-ai-trader/audits/edge_design/turtle_s2_donchian.md:1)

要点:
- Verdict: `THESIS_VALID_INSUFFICIENT_EVIDENCE`
- Redesign Recommendation: `C`
- Axis 2/4/5 は設計破綻なし。55D Donchian breakout、BoJ guard、2N stop + 20D low exit は thesis と整合。
- Axis 7 は live audit DB が `turtle_s2%` / `%turtle%` で N=0、WF folds>=3 も不足のため evidence 不足。
- BT は実行していません。既存 DB / KB / strategy docstring の値だけで監査しています。
```
