---
id: 20260504T2025-w4-eda-ema200_reversal
title: "[W4-EDA] Edge Design Audit — ema200_reversal"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T20:25:00+0900
roadmap_gate: "W4 Edge Design Audit — ema200_reversal の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`ema200_reversal` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
...
```json
[{"tier": "phase0_shadow", "strategy": "ema200_reversal", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```
...
ファイル `audits/edge_design/ema200_reversal.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
- `audits/edge_design/ema200_reversal.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T10:49:31Z)

exit=0 · 151s

```
作成しました: [audits/edge_design/ema200_reversal.md](/data/repo/fx-ai-trader/audits/edge_design/ema200_reversal.md)

要点:
- Verdict: `THESIS_VALID_TIMING_BROKEN`
- Redesign Recommendation: `A`
- 主な破綻軸: Axis 3 の per-bar dedup 欠落、Axis 6 の ALL 適用による USD_JPY edge の希釈、Axis 7 の WF/Bonferroni 不足
- BT は実行していません。既存の tier-master / audit artifact のみ参照しました。
```
