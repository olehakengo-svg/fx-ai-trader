---
id: 20260504T2026-w4-eda-ema_ribbon
title: "[W4-EDA] Edge Design Audit — ema_ribbon"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T20:26:00+0900
roadmap_gate: "W4 Edge Design Audit — ema_ribbon の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/scalp/ema_ribbon.py
related:
...
`ema_ribbon` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/ema_ribbon.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "ema_ribbon", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/ema_ribbon.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/ema_ribbon.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/ema_ribbon.py:N`>
...
- `audits/edge_design/ema_ribbon.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
