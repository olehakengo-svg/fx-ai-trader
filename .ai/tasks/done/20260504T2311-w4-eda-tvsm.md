---
id: 20260504T2311-w4-eda-tvsm
title: "[W4-EDA] Edge Design Audit — tvsm"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:11:00+0900
roadmap_gate: "W4 Edge Design Audit — tvsm の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`tvsm` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/micro_scalp/tvsm.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "tvsm", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/micro_scalp/tvsm.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/tvsm.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/micro_scalp/tvsm.py:N`>
...
- `audits/edge_design/tvsm.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T14:30:35Z)

exit=0 · 131s

```
作成しました: [audits/edge_design/tvsm.md](/data/repo/fx-ai-trader/audits/edge_design/tvsm.md)

内容は指定テンプレート通りで、Verdict は `THESIS_VALID_INSUFFICIENT_EVIDENCE`、Redesign Recommendation は `A` です。Axis 1-5 は `strategies/micro_scalp/tvsm.py:N` 形式でコード引用済みです。

新規 BT は実行していません。既存 tier-master 入力が `—`、local audit DB でも exact `tvsm` 行が 0 件だったため、Empirical Evidence は `INSUFFICIENT_EVIDENCE` として正当化しています。
```
