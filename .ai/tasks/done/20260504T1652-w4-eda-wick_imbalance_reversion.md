---
id: 20260504T1652-w4-eda-wick_imbalance_reversion
title: "[W4-EDA] Edge Design Audit — wick_imbalance_reversion"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T16:52:00+0900
roadmap_gate: "W4 Edge Design Audit — wick_imbalance_reversion の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/alpha_wick_imbalance.py
related:
...
`wick_imbalance_reversion` (`Tier 1 (LIVE)` / `pair_promoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/alpha_wick_imbalance.py`
- Pairs: `GBP_USD`
- Historical metrics (tier-master 由来):

```json
[{"tier": "pair_promoted", "strategy": "wick_imbalance_reversion", "pair": "GBP_USD", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/alpha_wick_imbalance.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/wick_imbalance_reversion.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/alpha_wick_imbalance.py:N`>
...
- `audits/edge_design/wick_imbalance_reversion.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
