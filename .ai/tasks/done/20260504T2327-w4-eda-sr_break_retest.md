---
id: 20260504T2327-w4-eda-sr_break_retest
title: "[W4-EDA] Edge Design Audit — sr_break_retest"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:27:00+0900
roadmap_gate: "W4 Edge Design Audit — sr_break_retest の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`sr_break_retest` (`Tier 3 (FORCE_DEMOTED)` / `force_demoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/sr_break_retest.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "force_demoted", "strategy": "sr_break_retest", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/sr_break_retest.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/sr_break_retest.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/sr_break_retest.py:N`>
...
- `audits/edge_design/sr_break_retest.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
