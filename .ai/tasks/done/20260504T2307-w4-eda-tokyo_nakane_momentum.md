---
id: 20260504T2307-w4-eda-tokyo_nakane_momentum
title: "[W4-EDA] Edge Design Audit — tokyo_nakane_momentum"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:07:00+0900
roadmap_gate: "W4 Edge Design Audit — tokyo_nakane_momentum の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/tokyo_nakane_momentum.py
related:
...
`tokyo_nakane_momentum` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/tokyo_nakane_momentum.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "tokyo_nakane_momentum", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/tokyo_nakane_momentum.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/tokyo_nakane_momentum.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/tokyo_nakane_momentum.py:N`>
...
- `audits/edge_design/tokyo_nakane_momentum.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
- 追加: [tokyo_nakane_momentum.md](/data/repo/fx-ai-trader/audits/edge_design/tokyo_nakane_momentum.md)
- Verdict: `THESIS_VALID_DESIGN_BROKEN`
- Redesign Recommendation: `A`
- 主因: Axis 4 の HTF hard block と `ctx.is_jpy` による JPY cross 拡張が、USD/JPY 仲値リバーサル thesis と衝突
- Axis 7: PF / WF / Bonferroni p / Kelly は既存 tier-master/audit DB で不足のため `INSUFFICIENT_EVIDENCE` と明記

検査済み:
- 指定テンプレートの主要セクションあり
- Axis 1-5 に `strategies/daytrade/tokyo_nakane_momentum.py:LINE` 形式の code citation あり
- Wilson lo は N=10, WR=70% から 39.68% として派生記載
- 戦略コード変更なし、監査 Markdown のみ追加しました。
```
