---
id: 20260504T2336-w4-eda-ma_trend_perfect
title: "[W4-EDA] Edge Design Audit — ma_trend_perfect"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:36:00+0900
roadmap_gate: "W4 Edge Design Audit — ma_trend_perfect の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/scalp/ma_trend_perfect.py
related:
...
`ma_trend_perfect` (`Tier 4 (SCALP_SENTINEL)` / `scalp_sentinel`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/ma_trend_perfect.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "scalp_sentinel", "strategy": "ma_trend_perfect", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/ma_trend_perfect.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/ma_trend_perfect.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/ma_trend_perfect.py:N`>
...
- `audits/edge_design/ma_trend_perfect.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T15:40:52Z)

exit=0 · 128s

```
作成しました: [audits/edge_design/ma_trend_perfect.md](/data/repo/fx-ai-trader/audits/edge_design/ma_trend_perfect.md)

内容は指定テンプレート通りで、主判定は以下です。

- Verdict: `THESIS_VALID_TIMING_BROKEN`
- Redesign Recommendation: `A`
- 主破綻軸: Axis 3 の timing window
- trigger/filter/stop は thesis と整合
- empirical evidence は 180d pre-reg BT prior は positive、ただし tier-master 365d / live audit DB / stored Bonferroni-adjusted p は `INSUFFICIENT_EVIDENCE`

BT は実行していません。検証はファイル作成と必須項目の grep 確認のみです。
```
