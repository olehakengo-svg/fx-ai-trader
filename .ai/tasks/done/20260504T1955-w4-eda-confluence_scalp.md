---
id: 20260504T1955-w4-eda-confluence_scalp
title: "[W4-EDA] Edge Design Audit — confluence_scalp"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T19:55:00+0900
roadmap_gate: "W4 Edge Design Audit — confluence_scalp の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/scalp/confluence_scalp.py
related:
...
`confluence_scalp` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/confluence_scalp.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "confluence_scalp", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/confluence_scalp.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/confluence_scalp.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/confluence_scalp.py:N`>
...
- `audits/edge_design/confluence_scalp.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T10:23:03Z)

exit=0 · 151s

```
作成しました: [audits/edge_design/confluence_scalp.md](/data/repo/fx-ai-trader/audits/edge_design/confluence_scalp.md)

Verdict は `THESIS_VALID_TIMING_BROKEN`、Redesign Recommendation は `A` です。BT は実行していません。既存 tier-master / audit DB / 既存 BT 断片のみを使い、Axis 1-5 は `strategies/scalp/confluence_scalp.py:LINE` 形式で引用済みです。
```
