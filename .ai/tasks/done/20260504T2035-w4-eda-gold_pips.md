---
id: 20260504T2035-w4-eda-gold_pips
title: "[W4-EDA] Edge Design Audit — gold_pips"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T20:35:00+0900
roadmap_gate: "W4 Edge Design Audit — gold_pips の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/scalp/gold_pips.py
related:
...
`gold_pips` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/gold_pips.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "gold_pips", "pair": "ALL", "tier_master_md_365d_BT_EV": "—", "note": "XAU/Gold strategy — user feedback memory: XAU is excluded from production scope but design audit still useful"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/gold_pips.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/gold_pips.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/gold_pips.py:N`>
...
- `audits/edge_design/gold_pips.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T11:16:44Z)

exit=0 · 133s

```
作成しました: [audits/edge_design/gold_pips.md](/data/repo/fx-ai-trader/audits/edge_design/gold_pips.md)

要点:
- Verdict: `THESIS_VALID_DESIGN_BROKEN`
- Redesign Recommendation: `B`
- 主因: Axis 3 `LOOKAHEAD`、Axis 5 `MISALIGNED`
- Empirical evidence: 既存 tier-master / audit DB に Wilson lo / PF / Kelly を算出できる closed trade evidence がなく、`INSUFFICIENT_EVIDENCE` として正当化

BT は実行していません。Audit ファイルのみ追加し、コード変更はしていません。
```
