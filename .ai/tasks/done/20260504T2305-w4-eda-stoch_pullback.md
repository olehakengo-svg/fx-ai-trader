---
id: 20260504T2305-w4-eda-stoch_pullback
title: "[W4-EDA] Edge Design Audit — stoch_pullback"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:05:00+0900
roadmap_gate: "W4 Edge Design Audit — stoch_pullback の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/scalp/stoch_pullback.py
related:
...
`stoch_pullback` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/stoch_pullback.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "stoch_pullback", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/stoch_pullback.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/stoch_pullback.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/stoch_pullback.py:N`>
...
- `audits/edge_design/stoch_pullback.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
