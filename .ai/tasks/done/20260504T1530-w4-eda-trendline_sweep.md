---
id: 20260504T1530-w4-eda-trendline_sweep
title: "[W4-EDA] Edge Design Audit — trendline_sweep"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T15:30:00+0900
roadmap_gate: "W4 Edge Design Audit — trendline_sweep の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/trendline_sweep.py
related:
...
`trendline_sweep` (`Tier 1 (LIVE)` / `elite_live`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/trendline_sweep.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "elite_live", "strategy": "trendline_sweep", "pair": "ALL", "tier_master_md": {"365d_BT_JPY_EV": "—", "EUR_EV": "+0.574", "GBP_EV": "+0.838"}, "note": "Wilson lo / PF / WF / Bonferroni / Kelly should be queried from audit DB (sqlite) by Codex"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/trendline_sweep.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/trendline_sweep.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/trendline_sweep.py:N`>
...
- `audits/edge_design/trendline_sweep.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
