---
id: 20260504T2217-w4-eda-ofi_mr
title: "[W4-EDA] Edge Design Audit — ofi_mr"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T22:17:00+0900
roadmap_gate: "W4 Edge Design Audit — ofi_mr の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/micro_scalp/ofi_mr.py
related:
...
`ofi_mr` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/micro_scalp/ofi_mr.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "ofi_mr", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/micro_scalp/ofi_mr.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/ofi_mr.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/micro_scalp/ofi_mr.py:N`>
...
- `audits/edge_design/ofi_mr.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T13:09:28Z)

exit=0 · 108s

```
作成しました: [audits/edge_design/ofi_mr.md](/data/repo/fx-ai-trader/audits/edge_design/ofi_mr.md)

要点:
- Verdict: `THESIS_VALID_DESIGN_BROKEN`
- Redesign Recommendation: `B`
- 主破綻軸: Axis 3 `LOOKAHEAD`, Axis 5 `MISALIGNED`
- Axis 7 は tier-master と audit DB に `ofi_mr` の decision-grade 統計が無いため `INSUFFICIENT_EVIDENCE` として正当化
