---
id: 20260504T2335-w4-eda-ma_regime_switch
title: "[W4-EDA] Edge Design Audit — ma_regime_switch"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:35:00+0900
roadmap_gate: "W4 Edge Design Audit — ma_regime_switch の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/scalp/ma_regime_switch.py
related:
...
`ma_regime_switch` (`Tier 4 (SCALP_SENTINEL)` / `scalp_sentinel`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/ma_regime_switch.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "scalp_sentinel", "strategy": "ma_regime_switch", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/ma_regime_switch.py:LINE` 形式で引用する**。
...
8. **failure mode 診断** — `Tier 4 (SCALP_SENTINEL)` ∈ {`Tier 3`, `Tier 4`}、または Tier 1/2 で metrics 劣化のもの。**どの軸（2/3/4/5）で破綻したか同定**し、再設計案を最低 1 案。「思想は正、設計が誤」仮説を裏付ける具体方針:
   - Trigger 修正（条件式変更）
   - Filter 削除 or 置換
...
ファイル `audits/edge_design/ma_regime_switch.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/ma_regime_switch.py:N`>
...
- `audits/edge_design/ma_regime_switch.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
...
## Result (2026-05-04T15:38:10Z)

exit=0 · 126s

```
作成しました: [audits/edge_design/ma_regime_switch.md](/data/repo/fx-ai-trader/audits/edge_design/ma_regime_switch.md)

主要判定:
- Verdict: `THESIS_VALID_DESIGN_BROKEN`
- Redesign Recommendation: `A`
- 主破綻軸: Axis 2 / 4
- 理由: thesis は明確だが、中心の regime trigger が M15 ATR percentile ではなく `ctx.bb_width_pct` proxy になっており、regime switch の設計が thesis とズレています。
- Empirical evidence は既存 audit DB から Wilson lo / PF / Kelly を埋め、Bonferroni-adjusted p は保存値がないため `INSUFFICIENT_EVIDENCE` として正当化しました。
