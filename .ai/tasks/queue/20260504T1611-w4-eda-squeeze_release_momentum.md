---
id: 20260504T1611-w4-eda-squeeze_release_momentum
title: "[W4-EDA] Edge Design Audit — squeeze_release_momentum"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T16:11:00+0900
roadmap_gate: "W4 Edge Design Audit — squeeze_release_momentum の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/squeeze_release_momentum.py
related:
...
`squeeze_release_momentum` (`Tier 1 (LIVE)` / `pair_promoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/squeeze_release_momentum.py`
- Pairs: `EUR_USD`
- Historical metrics (tier-master 由来):

```json
[{"tier": "pair_promoted", "strategy": "squeeze_release_momentum", "pair": "EUR_USD", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/squeeze_release_momentum.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/squeeze_release_momentum.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/squeeze_release_momentum.py:N`>
...
- `audits/edge_design/squeeze_release_momentum.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
