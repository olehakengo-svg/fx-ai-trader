---
id: 20260505-1212-w4-shadow-redesign-mtf_confluence
title: "[W4-Shadow Phase 1] mtf_confluence (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:12:00+0900
...
W4-EDA audit (`audits/edge_design/mtf_confluence.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/mtf_confluence-shadow-redesign-2026-05-05.md`:
...
`tests/test_mtf_confluence_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`MTF_CONFLUENCE_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['mtf_confluence'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['mtf_confluence'], variant=True)
"
```

`knowledge-base/raw/bt-results/mtf_confluence-shadow-bt-2026-05-05.json` に保存。
