---
id: 20260504T1610-w4-eda-ema200_trend_reversal
title: "[W4-EDA] Edge Design Audit — ema200_trend_reversal"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T16:10:00+0900
roadmap_gate: "W4 Edge Design Audit — ema200_trend_reversal の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/ema200_reversal.py
related:
...
`ema200_trend_reversal` (`Tier 1 (LIVE)` / `pair_promoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/ema200_reversal.py`
- Pairs: `USD_JPY`
- Historical metrics (tier-master 由来):

```json
[{"tier": "pair_promoted", "strategy": "ema200_trend_reversal", "pair": "USD_JPY", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/ema200_reversal.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/ema200_trend_reversal.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/ema200_reversal.py:N`>
...
- `audits/edge_design/ema200_trend_reversal.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
