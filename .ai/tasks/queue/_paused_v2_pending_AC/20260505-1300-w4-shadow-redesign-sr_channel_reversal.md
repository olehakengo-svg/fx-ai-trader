---
id: 20260505-1300-w4-shadow-redesign-sr_channel_reversal
title: "[W4-Shadow Phase 1] sr_channel_reversal (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T13:00:00+0900
...
W4-EDA audit (`audits/edge_design/sr_channel_reversal.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
Stop/TP は MR 型へ置換する。具体的には SL を support/resistance/c
...
`knowledge-base/wiki/decisions/sr_channel_reversal-shadow-redesign-2026-05-05.md`:
...
`tests/test_sr_channel_reversal_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`SR_CHANNEL_REVERSAL_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['sr_channel_reversal'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['sr_channel_reversal'], variant=True)
"
```

`knowledge-base/raw/bt-results/sr_channel_reversal-shadow-bt-2026-05-05.json` に保存。
