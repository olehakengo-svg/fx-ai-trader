---
id: 20260505-1205-w4-shadow-redesign-confluence_scalp
title: "[W4-Shadow Phase 1] confluence_scalp (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:05:00+0900
roadmap_gate: "W4 Shadow-first redesign — quant 推奨 (BT は軽量 filter、shadow が真の estimator)"
rule: R1
prereq_artifacts:
  - audits/edge_design/confluence_scalp.md
...
W4-EDA audit (`audits/edge_design/confluence_scalp.md`) で **THESIS_VALID_TIMING_BROKEN** / 推奨度 **A** と判定。
...
再設計案は timing 1 系統。`evaluate()` の trigger 入力を確定済み signal bar に固定し、execution は次 bar の `ct
...
`knowledge-base/wiki/decisions/confluence_scalp-shadow-redesign-2026-05-05.md`:
...
`tests/test_confluence_scalp_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`CONFLUENCE_SCALP_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['confluence_scalp'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['confluence_scalp'], variant=True)
"
```

`knowledge-base/raw/bt-results/confluence_scalp-shadow-bt-2026-05-05.json` に保存。
