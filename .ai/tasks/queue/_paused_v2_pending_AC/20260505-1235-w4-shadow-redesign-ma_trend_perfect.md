---
id: 20260505-1235-w4-shadow-redesign-ma_trend_perfect
title: "[W4-Shadow Phase 1] ma_trend_perfect (Tier 4 (SCALP_SENTINEL)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:35:00+0900
...
W4-EDA audit (`audits/edge_design/ma_trend_perfect.md`) で **THESIS_VALID_TIMING_BROKEN** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/ma_trend_perfect-shadow-redesign-2026-05-05.md`:
...
`tests/test_ma_trend_perfect_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`MA_TREND_PERFECT_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['ma_trend_perfect'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['ma_trend_perfect'], variant=True)
"
```

`knowledge-base/raw/bt-results/ma_trend_perfect-shadow-bt-2026-05-05.json` に保存。
