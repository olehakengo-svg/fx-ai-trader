---
id: 20260505-1258-w4-shadow-redesign-session_vol_expansion
title: "[W4-Shadow Phase 1] session_vol_expansion (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:58:00+0900
roadmap_gate: "W4 Shadow-first redesign — quant 推奨 (BT は軽量 filter、shadow が真の estimator)"
rule: R1
prereq_artifacts:
  - audits/edge_design/session_vol_expansion.md
...
W4-EDA audit (`audits/edge_design/session_vol_expansion.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
再設計案は、trigger を「確定済みレンジの外側に signal bar close が抜けた」形へ修正すること。具体的には `_range_slice = ctx.df.iloc[-self.lookback_range-1:-1]` で signal bar を除外し、`BUY: close > ra
...
`knowledge-base/wiki/decisions/session_vol_expansion-shadow-redesign-2026-05-05.md`:
...
`tests/test_session_vol_expansion_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`SESSION_VOL_EXPANSION_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['session_vol_expansion'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['session_vol_expansion'], variant=True)
"
```

`knowledge-base/raw/bt-results/session_vol_expansion-shadow-bt-2026-05-05.json` に保存。
