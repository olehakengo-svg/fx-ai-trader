---
id: 20260505-1305-w4-shadow-redesign-vwap_mean_reversion
title: "[W4-Shadow Phase 1] vwap_mean_reversion (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T13:05:00+0900
...
W4-EDA audit (`audits/edge_design/vwap_mean_reversion.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
再設計案は、trigger 本体は維持し、signal feature を確定済み bar のみで計算して next-bar / next-tick execution に固定すること。HTF hard direction block は削除し、代わりに ADX/slope を soft score または no-trade threshold として検証する。TP は VWAP、また
...
`knowledge-base/wiki/decisions/vwap_mean_reversion-shadow-redesign-2026-05-05.md`:
...
`tests/test_vwap_mean_reversion_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`VWAP_MEAN_REVERSION_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['vwap_mean_reversion'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['vwap_mean_reversion'], variant=True)
"
```

`knowledge-base/raw/bt-results/vwap_mean_reversion-shadow-bt-2026-05-05.json` に保存。
