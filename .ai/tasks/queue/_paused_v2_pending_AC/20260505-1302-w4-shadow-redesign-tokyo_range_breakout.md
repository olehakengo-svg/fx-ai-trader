---
id: 20260505-1302-w4-shadow-redesign-tokyo_range_breakout
title: "[W4-Shadow Phase 1] tokyo_range_breakout (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T13:02:00+0900
roadmap_gate: "W4 Shadow-first redesign — quant 推奨 (BT は軽量 filter、shadow が真の estimator)"
rule: R1
prereq_artifacts:
  - audits/edge_design/tokyo_range_breakout.md
...
W4-EDA audit (`audits/edge_design/tokyo_range_breakout.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
再設計案は、core trigger を残しつつ、filter と timing と exit を分離して v2 化すること。Trigger は `last_closed_close > tokyo_high + buffer` の bar-close only に寄せ、`buffe
...
`knowledge-base/wiki/decisions/tokyo_range_breakout-shadow-redesign-2026-05-05.md`:
...
`tests/test_tokyo_range_breakout_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`TOKYO_RANGE_BREAKOUT_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['tokyo_range_breakout'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['tokyo_range_breakout'], variant=True)
"
```

`knowledge-base/raw/bt-results/tokyo_range_breakout-shadow-bt-2026-05-05.json` に保存。
