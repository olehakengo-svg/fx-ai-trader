---
id: 20260505-1221-w4-shadow-redesign-turtle_soup
title: "[W4-Shadow Phase 1] turtle_soup (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:21:00+0900
...
W4-EDA audit (`audits/edge_design/turtle_soup.md`) で **THESIS_VALID_TIMING_BROKEN** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/turtle_soup-shadow-redesign-2026-05-05.md`:
...
`tests/test_turtle_soup_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`TURTLE_SOUP_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['turtle_soup'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['turtle_soup'], variant=True)
"
```

`knowledge-base/raw/bt-results/turtle_soup-shadow-bt-2026-05-05.json` に保存。
