---
id: 20260505-1309-w4-shadow-redesign-adx_trend_continuation
title: "[W4-Shadow Phase 1] adx_trend_continuation (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T13:09:00+0900
...
W4-EDA audit (`audits/edge_design/adx_trend_continuation.md`) で **THESIS_VALID_INSUFFICIENT_EVIDENCE** / 推奨度 **A** と判定。
...
再設計案は timing hardening と scope 明示の 2 点。まず trigger 判定を closed bar に固定し、`bar_id = ctx.bar_time or ctx.df.index[-1]` を使った `(symbol, signal, bar_id)` dedup
...
`knowledge-base/wiki/decisions/adx_trend_continuation-shadow-redesign-2026-05-05.md`:
...
`tests/test_adx_trend_continuation_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`ADX_TREND_CONTINUATION_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['adx_trend_continuation'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['adx_trend_continuation'], variant=True)
"
```

`knowledge-base/raw/bt-results/adx_trend_continuation-shadow-bt-2026-05-05.json` に保存。
