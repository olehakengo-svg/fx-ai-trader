---
id: 20260505-1311-w4-shadow-redesign-vdr_jpy
title: "[W4-Shadow Phase 1] vdr_jpy (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T13:11:00+0900
...
W4-EDA audit (`audits/edge_design/vdr_jpy.md`) で **THESIS_VALID_INSUFFICIENT_EVIDENCE** / 推奨度 **A** と判定。
...
再設計案は trigger/timing の一系統修正。まず pair-specific parameter を導入し、USDJPY は raw best に合わせて `DEV_SIGMA_THRESHOLD=2.0` と `forward_bars=2` 相当の time exit、EURJPY/GBPJPY は `DEV_SIGMA_THRESHOLD=1.5` を維持する variant を pre-register する。次に candle confirmation を hard gate から score penalty/bonus へ落とし、乖離成
...
`knowledge-base/wiki/decisions/vdr_jpy-shadow-redesign-2026-05-05.md`:
...
`tests/test_vdr_jpy_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`VDR_JPY_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['vdr_jpy'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['vdr_jpy'], variant=True)
"
```

`knowledge-base/raw/bt-results/vdr_jpy-shadow-bt-2026-05-05.json` に保存。
