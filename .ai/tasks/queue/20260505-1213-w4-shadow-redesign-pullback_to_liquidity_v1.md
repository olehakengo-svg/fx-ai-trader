---
id: 20260505-1213-w4-shadow-redesign-pullback_to_liquidity_v1
title: "[W4-Shadow Phase 1] pullback_to_liquidity_v1 (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:13:00+0900
...
W4-EDA audit (`audits/edge_design/pullback_to_liquidity_v1.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **A** と判定。
...
再設計案は trigger/timing の 2 点。Trigger は `tolerance = 5 / ctx.pip_mult` のように fixed-pip 化し、BUY は `abs(current_low - swing_low) <= tolerance` または「下抜け許容は最大 1ATR/特定 pip まで」の bounded swe
...
`knowledge-base/wiki/decisions/pullback_to_liquidity_v1-shadow-redesign-2026-05-05.md`:
...
`tests/test_pullback_to_liquidity_v1_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`PULLBACK_TO_LIQUIDITY_V1_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['pullback_to_liquidity_v1'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['pullback_to_liquidity_v1'], variant=True)
"
```

`knowledge-base/raw/bt-results/pullback_to_liquidity_v1-shadow-bt-2026-05-05.json` に保存。
