---
id: 20260505-1236-w4-shadow-redesign-mtf_counter_trend_scalp
title: "[W4-Shadow Phase 1] mtf_counter_trend_scalp (Tier 4 (SCALP_SENTINEL)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:36:00+0900
...
W4-EDA audit (`audits/edge_design/mtf_counter_trend_scalp.md`) で **THESIS_VALID_TIMING_BROKEN** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/mtf_counter_trend_scalp-shadow-redesign-2026-05-05.md`:
...
`tests/test_mtf_counter_trend_scalp_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`MTF_COUNTER_TREND_SCALP_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['mtf_counter_trend_scalp'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['mtf_counter_trend_scalp'], variant=True)
"
```

`knowledge-base/raw/bt-results/mtf_counter_trend_scalp-shadow-bt-2026-05-05.json` に保存。
