---
id: 20260505-1222-w4-shadow-redesign-vbp
title: "[W4-Shadow Phase 1] vbp (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:22:00+0900
...
W4-EDA audit (`audits/edge_design/vbp.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/vbp-shadow-redesign-2026-05-05.md`:
...
`tests/test_vbp_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`VBP_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['vbp'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['vbp'], variant=True)
"
```

`knowledge-base/raw/bt-results/vbp-shadow-bt-2026-05-05.json` に保存。
