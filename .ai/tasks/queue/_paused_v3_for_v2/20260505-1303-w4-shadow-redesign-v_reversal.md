---
id: 20260505-1303-w4-shadow-redesign-v_reversal
title: "[W4-Shadow Phase 1] v_reversal (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T13:03:00+0900
...
W4-EDA audit (`audits/edge_design/v_reversal.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
`knowledge-base/wiki/decisions/v_reversal-shadow-redesign-2026-05-05.md`:
...
`tests/test_v_reversal_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`V_REVERSAL_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['v_reversal'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['v_reversal'], variant=True)
"
```

`knowledge-base/raw/bt-results/v_reversal-shadow-bt-2026-05-05.json` に保存。
