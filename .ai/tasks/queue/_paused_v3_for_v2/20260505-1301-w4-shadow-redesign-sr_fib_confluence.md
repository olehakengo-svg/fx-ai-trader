---
id: 20260505-1301-w4-shadow-redesign-sr_fib_confluence
title: "[W4-Shadow Phase 1] sr_fib_confluence (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T13:01:00+0900
...
W4-EDA audit (`audits/edge_design/sr_fib_confluence.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
再設計案は、思想を維持して trigger と timing を作り直すこと。`dt_reasons` 文字列ではなく、上流 DT layer から `fib_level`, `sr_level`, `ob_zone_low/high`, `confluence_type`, `signal_bar_time` のような構造化 feature を `ctx.layer3` に渡し、この file では `abs(ctx.entry - fib_level) <= 0.35*ATR` または `ob_zone_low <= entry <= ob_zone_high` を hard gate にする。さらに EMA direc
...
`knowledge-base/wiki/decisions/sr_fib_confluence-shadow-redesign-2026-05-05.md`:
...
`tests/test_sr_fib_confluence_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`SR_FIB_CONFLUENCE_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['sr_fib_confluence'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['sr_fib_confluence'], variant=True)
"
```

`knowledge-base/raw/bt-results/sr_fib_confluence-shadow-bt-2026-05-05.json` に保存。
