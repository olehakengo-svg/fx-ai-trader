---
id: 20260505-1241-w4-shadow-redesign-donchian_momentum_breakout
title: "[W4-Shadow Phase 1] donchian_momentum_breakout (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:41:00+0900
roadmap_gate: "W4 Shadow-first redesign — quant 推奨 (BT は軽量 filter、shadow が真の estimator)"
rule: R1
prereq_artifacts:
  - audits/edge_design/donchian_momentum_breakout.md
...
W4-EDA audit (`audits/edge_design/donchian_momentum_breakout.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
再設計案は、trigger の骨格を残して timing と exit を v2 化すること。Trigger は `last_closed_close > prev_don_high + buffer` / `< prev_don_low - buffer` の bar-clo
...
`knowledge-base/wiki/decisions/donchian_momentum_breakout-shadow-redesign-2026-05-05.md`:
...
`tests/test_donchian_momentum_breakout_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`DONCHIAN_MOMENTUM_BREAKOUT_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['donchian_momentum_breakout'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['donchian_momentum_breakout'], variant=True)
"
```

`knowledge-base/raw/bt-results/donchian_momentum_breakout-shadow-bt-2026-05-05.json` に保存。
