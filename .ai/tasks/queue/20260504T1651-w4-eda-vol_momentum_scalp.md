---
id: 20260504T1651-w4-eda-vol_momentum_scalp
title: "[W4-EDA] Edge Design Audit — vol_momentum_scalp"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T16:51:00+0900
roadmap_gate: "W4 Edge Design Audit — vol_momentum_scalp の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`vol_momentum_scalp` (`Tier 1 (LIVE)` / `pair_promoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/scalp/vol_momentum.py`
- Pairs: `EUR_JPY`
- Historical metrics (tier-master 由来):

```json
[{"tier": "pair_promoted", "strategy": "vol_momentum_scalp", "pair": "EUR_JPY", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/scalp/vol_momentum.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/vol_momentum_scalp.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/scalp/vol_momentum.py:N`>
...
- `audits/edge_design/vol_momentum_scalp.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
