...
- 走 0〜5 の表 (**走 0′〜4 は「C0 近似」ラベル** — SR ベース SL を ATR×1.0 で代用し下流 SL 調整 (C0c) も未再現、+ bar 内順序仮定の両方向、走 5 は「C6 近似」) + **C0 感度** (live ログから SR-stop 採用率と C0c 発動率、**5–20pip clamp の発動率** (ATR×1.0 / SR 距離が範囲外だった fill の割合) を出し、SR-stop 採用 fill を除いた再集計と clamp 有/無の再集計を併記) + winner/loser 別 hold 分布 + exit 種別比率 + packet 用の所見が analyses/ に保存され、done ファイルに '## Claude Review' が付く
