---
id: 20260505-1246-w4-shadow-redesign-engulfing_bb
title: "[W4-Shadow Phase 1] engulfing_bb (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:46:00+0900
...
W4-EDA audit (`audits/edge_design/engulfing_bb.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
`knowledge-base/wiki/decisions/engulfing_bb-shadow-redesign-2026-05-05.md`:
...
`tests/test_engulfing_bb_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`ENGULFING_BB_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['engulfing_bb'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['engulfing_bb'], variant=True)
"
```

`knowledge-base/raw/bt-results/engulfing_bb-shadow-bt-2026-05-05.json` に保存。
