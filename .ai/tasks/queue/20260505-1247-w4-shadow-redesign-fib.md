---
id: 20260505-1247-w4-shadow-redesign-fib
title: "[W4-Shadow Phase 1] fib (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:47:00+0900
roadmap_gate: "W4 Shadow-first redesign — quant 推奨 (BT は軽量 filter、shadow が真の estimator)"
rule: R1
prereq_artifacts:
  - audits/edge_design/fib.md
...
W4-EDA audit (`audits/edge_design/fib.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
再設計案は timing と stop geometry の 2 点。第一に `_calc_fibonacci_levels()` と trigger 判定を `ctx.df.iloc[:-1]` または `ctx.df.iloc[-lookback-1:-1]` の確定足で行い、signal は closed bar、execution は次 bar `ctx.entry` に分離する。併せて `(fib_r
...
`knowledge-base/wiki/decisions/fib-shadow-redesign-2026-05-05.md`:
...
`tests/test_fib_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`FIB_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['fib'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['fib'], variant=True)
"
```

`knowledge-base/raw/bt-results/fib-shadow-bt-2026-05-05.json` に保存。
