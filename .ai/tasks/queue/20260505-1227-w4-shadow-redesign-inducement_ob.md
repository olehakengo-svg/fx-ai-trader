---
id: 20260505-1227-w4-shadow-redesign-inducement_ob
title: "[W4-Shadow Phase 1] inducement_ob (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:27:00+0900
...
W4-EDA audit (`audits/edge_design/inducement_ob.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/inducement_ob-shadow-redesign-2026-05-05.md`:
...
`tests/test_inducement_ob_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`INDUCEMENT_OB_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['inducement_ob'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['inducement_ob'], variant=True)
"
```

`knowledge-base/raw/bt-results/inducement_ob-shadow-bt-2026-05-05.json` に保存。
