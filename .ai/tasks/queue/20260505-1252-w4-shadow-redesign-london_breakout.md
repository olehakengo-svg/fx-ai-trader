---
id: 20260505-1252-w4-shadow-redesign-london_breakout
title: "[W4-Shadow Phase 1] london_breakout (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:52:00+0900
...
W4-EDA audit (`audits/edge_design/london_breakout.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
`knowledge-base/wiki/decisions/london_breakout-shadow-redesign-2026-05-05.md`:
...
`tests/test_london_breakout_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`LONDON_BREAKOUT_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['london_breakout'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['london_breakout'], variant=True)
"
```

`knowledge-base/raw/bt-results/london_breakout-shadow-bt-2026-05-05.json` に保存。
