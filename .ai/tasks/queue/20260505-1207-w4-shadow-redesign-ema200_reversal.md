---
id: 20260505-1207-w4-shadow-redesign-ema200_reversal
title: "[W4-Shadow Phase 1] ema200_reversal (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:07:00+0900
roadmap_gate: "W4 Shadow-first redesign — quant 推奨 (BT は軽量 filter、shadow が真の estimator)"
rule: R1
prereq_artifacts:
  - audits/edge_design/ema200_reversal.md
...
W4-EDA audit (`audits/edge_design/ema200_reversal.md`) で **THESIS_VALID_TIMING_BROKEN** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/ema200_reversal-shadow-redesign-2026-05-05.md`:
...
`tests/test_ema200_reversal_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`EMA200_REVERSAL_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['ema200_reversal'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['ema200_reversal'], variant=True)
"
```

`knowledge-base/raw/bt-results/ema200_reversal-shadow-bt-2026-05-05.json` に保存。
