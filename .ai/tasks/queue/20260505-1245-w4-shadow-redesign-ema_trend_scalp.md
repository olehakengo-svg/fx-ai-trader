---
id: 20260505-1245-w4-shadow-redesign-ema_trend_scalp
title: "[W4-Shadow Phase 1] ema_trend_scalp (Tier 3 (FORCE_DEMOTED)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:45:00+0900
...
W4-EDA audit (`audits/edge_design/ema_trend_scalp.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
再設計案は `closed-bar pullback + moderate-trend gate + pair/session scope`。Signal は直近確定足で `ema9 > ema21`、EMA21 zone、candle bounce、RSI/BBPB を評価し、entry は次 bar 以降に限定する。Candidate には少なくとも `
...
`knowledge-base/wiki/decisions/ema_trend_scalp-shadow-redesign-2026-05-05.md`:
...
`tests/test_ema_trend_scalp_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`EMA_TREND_SCALP_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['ema_trend_scalp'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['ema_trend_scalp'], variant=True)
"
```

`knowledge-base/raw/bt-results/ema_trend_scalp-shadow-bt-2026-05-05.json` に保存。
