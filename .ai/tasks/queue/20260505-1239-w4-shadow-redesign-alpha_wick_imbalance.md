---
id: 20260505-1239-w4-shadow-redesign-alpha_wick_imbalance
title: "[W4-Shadow Phase 1] alpha_wick_imbalance (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:39:00+0900
...
W4-EDA audit (`audits/edge_design/alpha_wick_imbalance.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
再設計案は、HTF Hard Block を削除または soft penalty 化し、confirmation bar を closed bar に固定することを第一候補にする。コードレベルでは confirmation を `confirm = df.iloc[-2]` に移し、WIR lookback をその直前 window 本にずらして、entry は次足 `c
...
`knowledge-base/wiki/decisions/alpha_wick_imbalance-shadow-redesign-2026-05-05.md`:
...
`tests/test_alpha_wick_imbalance_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`ALPHA_WICK_IMBALANCE_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['alpha_wick_imbalance'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['alpha_wick_imbalance'], variant=True)
"
```

`knowledge-base/raw/bt-results/alpha_wick_imbalance-shadow-bt-2026-05-05.json` に保存。
