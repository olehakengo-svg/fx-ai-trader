---
id: 20260505-1307-w4-shadow-redesign-trendline_sweep
title: "[W4-Shadow Phase 1] trendline_sweep (Tier 1 (LIVE)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T13:07:00+0900
...
W4-EDA audit (`audits/edge_design/trendline_sweep.md`) で **THESIS_VALID_INSUFFICIENT_EVIDENCE** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/trendline_sweep-shadow-redesign-2026-05-05.md`:
...
`tests/test_trendline_sweep_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`TRENDLINE_SWEEP_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['trendline_sweep'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['trendline_sweep'], variant=True)
"
```

`knowledge-base/raw/bt-results/trendline_sweep-shadow-bt-2026-05-05.json` に保存。
