---
id: 20260505-1249-w4-shadow-redesign-gold_vol_break
title: "[W4-Shadow Phase 1] gold_vol_break (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:49:00+0900
...
W4-EDA audit (`audits/edge_design/gold_vol_break.md`) で **THESIS_VALID_DESIGN_BROKEN** / 推奨度 **B** と判定。
...
`knowledge-base/wiki/decisions/gold_vol_break-shadow-redesign-2026-05-05.md`:
...
`tests/test_gold_vol_break_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`GOLD_VOL_BREAK_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['gold_vol_break'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['gold_vol_break'], variant=True)
"
```

`knowledge-base/raw/bt-results/gold_vol_break-shadow-bt-2026-05-05.json` に保存。
