---
id: 20260505-1310-w4-shadow-redesign-tvsm
title: "[W4-Shadow Phase 1] tvsm (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T13:10:00+0900
...
W4-EDA audit (`audits/edge_design/tvsm.md`) で **THESIS_VALID_INSUFFICIENT_EVIDENCE** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/tvsm-shadow-redesign-2026-05-05.md`:
...
`tests/test_tvsm_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`TVSM_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['tvsm'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['tvsm'], variant=True)
"
```

`knowledge-base/raw/bt-results/tvsm-shadow-bt-2026-05-05.json` に保存。
