---
id: 20260505-1917-w4-shadow-redesign-v2-ema_trend_scalp
title: "[W4-Shadow-Redesign v2] ema_trend_scalp (Tier 3 (FORCE_DEMOTED)) — relaxed spec"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T19:17:00+0900
...
W4-EDA audit (`audits/edge_design/ema_trend_scalp.md`):
...
## Step 1: 既存 audit 確認 (`audits/edge_design/ema_trend_scalp.md`)

audit が指摘する redesign axis を読み、**最小 1 軸** で実装。

## Step 2: 実装 (flag 配下)

`EMA_TREND_SCALP_REDESIGN_V2=1` 環境変数 / config flag で proposed variant を有効化。
default OFF で live は影響受けない。

## Step 3: 失敗テスト + 緑

`tests/test_ema_trend_scalp_shadow_redesign_v2.py`
...
- 失敗テスト緑
- BT report (`bt-results/ema_trend_scalp-shadow-redesign-v2-2026-05-05.json`)
