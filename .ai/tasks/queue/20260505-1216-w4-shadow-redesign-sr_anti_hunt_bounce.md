---
id: 20260505-1216-w4-shadow-redesign-sr_anti_hunt_bounce
title: "[W4-Shadow Phase 1] sr_anti_hunt_bounce (Tier 2 (Shadow)) — implement + shadow promote (BT light filter)"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T12:16:00+0900
...
W4-EDA audit (`audits/edge_design/sr_anti_hunt_bounce.md`) で **THESIS_VALID_TIMING_BROKEN** / 推奨度 **A** と判定。
...
`knowledge-base/wiki/decisions/sr_anti_hunt_bounce-shadow-redesign-2026-05-05.md`:
...
`tests/test_sr_anti_hunt_bounce_shadow_redesign.py`

## Step 3: 実装

audit Axis 推奨に従い最小差分。**flag 配下** で実装 (`SR_ANTI_HUNT_BOUNCE_REDESIGN_V2=1` 等)、デフォルトは現行維持。
...
# baseline
r0 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['sr_anti_hunt_bounce'], variant=False)
# proposed
r1 = run_daytrade_backtest(pair=..., tf=..., days=365, strategies=['sr_anti_hunt_bounce'], variant=True)
"
```

`knowledge-base/raw/bt-results/sr_anti_hunt_bounce-shadow-bt-2026-05-05.json` に保存。
