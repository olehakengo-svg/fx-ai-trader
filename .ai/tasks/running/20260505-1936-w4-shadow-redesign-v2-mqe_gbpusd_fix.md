---
id: 20260505-1936-w4-shadow-redesign-v2-mqe_gbpusd_fix
title: "[W4-Shadow-Redesign v2.1] mqe_gbpusd_fix (Tier 2 (Shadow)) — relaxed spec"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T19:36:00+0900
...
W4-EDA audit (`audits/edge_design/mqe_gbpusd_fix.md`):
...
再設計案は、trigger 中核は維持しつつ、entry/timing と exit を audit 設計に合わせること。具体的には window を code comment と audit note に合わせて `15:30 <= ts.time < 16:00` に狭め、同一 `(symbol, month_end_date, fix_window)` の 1 trade/day dedup を追加する。Exit は `MAX_HOLD_BARS=6` を実行層に渡せる設計へ変更し、TP/SL 到達がなくても 6 bar で time close する。ATR bracket は保護 stop として残すなら、TP 
...
## Step 1: 既存 audit 確認 (`audits/edge_design/mqe_gbpusd_fix.md`)

audit が指摘する redesign axis を読み、**最小 1 軸** で実装。

## Step 2: 実装 (flag 配下)

`MQE_GBPUSD_FIX_REDESIGN_V2=1` 環境変数 / config flag で proposed variant を有効化。
default OFF で live は影響受けない。

## Step 3: 失敗テスト + 緑

`tests/test_mqe_gbpusd_fix_shadow_redesign_v2.py`
...
- 失敗テスト緑
- BT report (`bt-results/mqe_gbpusd_fix-shadow-redesign-v2-2026-05-05.json`)
