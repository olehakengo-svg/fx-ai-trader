---
id: 20260504T2037-w4-eda-gold_vol_break
title: "[W4-EDA] Edge Design Audit — gold_vol_break"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T20:37:00+0900
roadmap_gate: "W4 Edge Design Audit — gold_vol_break の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/gold_vol_break.py
related:
...
`gold_vol_break` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/gold_vol_break.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "gold_vol_break", "pair": "ALL", "tier_master_md_365d_BT_EV": "—", "note": "XAU/Gold strategy — user feedback memory: XAU is excluded from production scope but design audit still useful"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/gold_vol_break.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/gold_vol_break.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/gold_vol_break.py:N`>
...
- `audits/edge_design/gold_vol_break.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
