---
id: 20260505-2000-w4-shadow-redesign-v2-turtle_soup
title: "[W4-Shadow-Redesign v2.1] turtle_soup (Tier 2 (Shadow)) — relaxed spec"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T20:00:00+0900
...
W4-EDA audit (`audits/edge_design/turtle_soup.md`):
...
## Step 1: 既存 audit 確認 (`audits/edge_design/turtle_soup.md`)

audit が指摘する redesign axis を読み、**最小 1 軸** で実装。

## Step 2: 実装 (flag 配下)

`TURTLE_SOUP_REDESIGN_V2=1` 環境変数 / config flag で proposed variant を有効化。
default OFF で live は影響受けない。

## Step 3: 失敗テスト + 緑

`tests/test_turtle_soup_shadow_redesign_v2.py`
...
- 失敗テスト緑
- BT report (`bt-results/turtle_soup-shadow-redesign-v2-2026-05-05.json`)
