...
W4-EDA audit (`audits/edge_design/vsg_jpy_reversal.md`):
...
## Step 1: 既存 audit 確認 (`audits/edge_design/vsg_jpy_reversal.md`)

audit が指摘する redesign axis を読み、**最小 1 軸** で実装。

## Step 2: 実装 (flag 配下)

`VSG_JPY_REVERSAL_REDESIGN_V2=1` 環境変数 / config flag で proposed variant を有効化。
default OFF で live は影響受けない。

## Step 3: 失敗テスト + 緑

`tests/test_vsg_jpy_reversal_shadow_redesign_v2.py`
...
- 失敗テスト緑
- BT report (`bt-results/vsg_jpy_reversal-shadow-redesign-v2-2026-05-05.json`)
