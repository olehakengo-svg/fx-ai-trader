---
id: 20260504T1820-w4-eda-xs_momentum
title: "[W4-EDA] Edge Design Audit — xs_momentum"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T18:20:00+0900
roadmap_gate: "W4 Edge Design Audit — xs_momentum の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - strategies/daytrade/xs_momentum.py
related:
...
`xs_momentum` (`Tier 1 (LIVE)` / `pair_promoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/xs_momentum.py`
- Pairs: `EUR_USD, GBP_USD`
- Historical metrics (tier-master 由来):

```json
[{"tier": "pair_promoted", "strategy": "xs_momentum", "pair": "EUR_USD", "tier_master_md_365d_BT_EV": "+0.126"}, {"tier": "pair_promoted", "strategy": "xs_momentum", "pair": "GBP_USD", "tier_master_md_365d_BT_EV": "-0.013"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/xs_momentum.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/xs_momentum.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/xs_momentum.py:N`>
...
- `audits/edge_design/xs_momentum.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
