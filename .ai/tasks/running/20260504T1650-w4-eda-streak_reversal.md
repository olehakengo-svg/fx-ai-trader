---
id: 20260504T1650-w4-eda-streak_reversal
title: "[W4-EDA] Edge Design Audit — streak_reversal"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T16:50:00+0900
roadmap_gate: "W4 Edge Design Audit — streak_reversal の 8 軸設計監査"
rule: R1
prereq_artifacts:
  - docs/superpowers/specs/2026-05-04-edge-design-audit-design.md
  - audits/edge_design/_INVENTORY.md
  - app.py:3201-3260,8707-8745 (INLINE)
related:
...
`streak_reversal` (`Tier 1 (LIVE)` / `pair_promoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `app.py:3201-3260,8707-8745 (INLINE)`
- Pairs: `USD_JPY`
- Historical metrics (tier-master 由来):

```json
[{"tier": "pair_promoted", "strategy": "streak_reversal", "pair": "USD_JPY", "implementation_note": "INLINE strategy in app.py — daytrade variant at lines 3201-3260, scalp variant at lines 8707-8745. No separate file in strategies/. Audit must read those line ranges directly."}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `app.py:3201-3260,8707-8745 (INLINE):LINE` 形式で引用する**。
...
8. **failure mode 診断** — `Tier 1 (LIVE)` ∈ {`Tier 3`, `Tier 4`}、または Tier 1/2 で metrics 劣化のもの。**どの軸（2/3/4/5）で破綻したか同定**し、再設計案を最低 1 案。「思想は正、設計が誤」仮説を裏付ける具体方針:
   - Trigger 修正（条件式変更）
   - Filter 削除 or 置換
...
ファイル `audits/edge_design/streak_reversal.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `app.py:3201-3260,8707-8745 (INLINE):N`>
...
- `audits/edge_design/streak_reversal.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
