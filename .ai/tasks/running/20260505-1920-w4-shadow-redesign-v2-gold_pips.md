---
id: 20260505-1920-w4-shadow-redesign-v2-gold_pips
title: "[W4-Shadow-Redesign v2] gold_pips (Tier 2 (Shadow)) — relaxed spec"
owner: codex
status: queued
priority: P1
created_at: 2026-05-05T19:20:00+0900
...
W4-EDA audit (`audits/edge_design/gold_pips.md`):
...
## Step 1: 既存 audit 確認 (`audits/edge_design/gold_pips.md`)

audit が指摘する redesign axis を読み、**最小 1 軸** で実装。

## Step 2: 実装 (flag 配下)

`GOLD_PIPS_REDESIGN_V2=1` 環境変数 / config flag で proposed variant を有効化。
default OFF で live は影響受けない。

## Step 3: 失敗テスト + 緑

`tests/test_gold_pips_shadow_redesign_v2.py`
...
- 失敗テスト緑
- BT report (`bt-results/gold_pips-shadow-redesign-v2-2026-05-05.json`)
