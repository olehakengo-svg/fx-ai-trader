---
id: 20260504T2317-w4-eda-donchian_momentum_breakout
title: "[W4-EDA] Edge Design Audit — donchian_momentum_breakout"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T23:17:00+0900
roadmap_gate: "W4 Edge Design Audit — donchian_momentum_breakout の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`donchian_momentum_breakout` (`Tier 3 (FORCE_DEMOTED)` / `force_demoted`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/hourly/donchian_momentum_breakout.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "force_demoted", "strategy": "donchian_momentum_breakout", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/hourly/donchian_momentum_breakout.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/donchian_momentum_breakout.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/hourly/donchian_momentum_breakout.py:N`>
...
- `audits/edge_design/donchian_momentum_breakout.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
