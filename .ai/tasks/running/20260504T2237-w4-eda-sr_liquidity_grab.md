---
id: 20260504T2237-w4-eda-sr_liquidity_grab
title: "[W4-EDA] Edge Design Audit — sr_liquidity_grab"
owner: codex
status: queued
priority: P1
created_at: 2026-05-04T22:37:00+0900
roadmap_gate: "W4 Edge Design Audit — sr_liquidity_grab の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`sr_liquidity_grab` (`Tier 2 (Shadow)` / `phase0_shadow`) について 8 軸で診断し、
verdict と再設計推奨度を発行する。

# 1. Inputs

- Strategy file: `strategies/daytrade/sr_liquidity_grab.py`
- Pairs: `ALL`
- Historical metrics (tier-master 由来):

```json
[{"tier": "phase0_shadow", "strategy": "sr_liquidity_grab", "pair": "ALL", "tier_master_md_365d_BT_EV": "—"}]
```

# 2. Method (8 axes — spec §3)

各軸を順に処理。**コード参照は必ず `strategies/daytrade/sr_liquidity_grab.py:LINE` 形式で引用する**。
...
ファイル `audits/edge_design/sr_liquidity_grab.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `strategies/daytrade/sr_liquidity_grab.py:N`>
...
- `audits/edge_design/sr_liquidity_grab.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
