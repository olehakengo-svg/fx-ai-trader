...
20,692 distinct observations under the default 1h dedup window, as-of
2026-09-22 over the 69,576 provenance-filtered rows — the count is
window-dependent AND estimator-dependent, so never quote it without both),
so `len(rows)` is not an observation count.
