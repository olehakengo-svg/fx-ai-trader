...
| test | 値 | verdict |
|---|---|---|
| **primary: pooled short 3d mean (イベント横断), N=23** | **+46.2 p/event, p = 0.0511** | ❌ BH 閾値 0.05 (q=0.10, m=2, k=1) 不通過 |
| supporting 1d / 5d | +25.7p p=0.114 / +27.0p p=0.143 | ❌ |
...
- **凍結ルールどおり kill**: p=0.0511 は閾値を 0.001 差で外した。**単独 family なら q=0.10 で
  通過していた** — 並列 2 本の多重性コスト (Bonferroni 分母の共有) を観測前に受け入れた設計の
  帰結であり、事後に閾値を動かさない (T11 ナイフエッジ教訓の裏面)
