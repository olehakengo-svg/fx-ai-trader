...
PAIRS_S5 = ["EURUSD=X", "GBPUSD=X"]
PULLBACK_ATR_GRID = [1.5, 2.0]
DEFENSE_LINE_GRID = ["VWAP", "EMA50_HTF", "BOTH"]
N_CELLS = len(PAIRS_S5) * len(PULLBACK_ATR_GRID) * len(DEFENSE_LINE_GRID)
...
def extract_vwap_defense(symbol, df, pullback_atr, defense_line):
...
def extract_all(from_iso, to_iso, pullback_atr, defense_line):
    ...
    for symbol in PAIRS_S5:
        try:
            ...
            ents = extract_vwap_defense(symbol, df, pullback_atr, defense_line)
            print(f"[entries] {symbol} pb={pullback_atr} {defense_line}: {len(ents)}", flush=True)
            out.extend(ents)
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = "/tmp/phase5-vwap-dryrun" if args.dry_run else "raw/bt-results/phase5-vwap-2026-04-25"
    print(f"=== Phase 5 S5 VWAP Defense BT (PAIR={PAIRS_S5}, α_cell={ALPHA_CELL:.5f}) ===")
    if args.dry_run:
        baseline_pnls = [s["pnl_pips"] for s in [simulate_pnl(t,0) for t in synth_null_trades(300, seed=42)]]
        per_extraction = {}
        for pa in PULLBACK_ATR_GRID:
            for dl in DEFENSE_LINE_GRID:
                per_extraction[(pa, dl)] = synth_null_trades(20, seed=hash((pa,dl))&0xff)
    else:
        baseline_pnls = [s["pnl_pips"] for s in [simulate_pnl(t,0) for t in synth_null_trades(300, seed=42)]]
        per_extraction = {}
        for pa in PULLBACK_ATR_GRID:
            for dl in DEFENSE_LINE_GRID:
                per_extraction[(pa, dl)] = extract_all(args.from_iso, args.to_iso, pa, dl)

    print(f"\n--- Cell results ---")
    print(f"{'cell':<6}{'pair':<11}{'pbATR':>6}{'def':<12}{'N':>5}{'WR%':>6}{'EV':>7}{'PF':>6}{'p':>9}{'WF':>5}{'verdict':>10}")
    grid = []
    cell_id = 0
    for pair in PAIRS_S5:
        for pa in PULLBACK_ATR_GRID:
            for dl in DEFENSE_LINE_GRID:
                cell_id += 1
                cell = {"id": f"C{cell_id:02d}", "pair": pair, "pullback_atr": pa, "defense_line": dl}
                if args.dry_run:
                    trades = per_extraction.get((pa, dl), [])[:15]
                    for t in trades: t["pair"]=pair; t["symbol"]=pair
                else:
                    all_ents = per_extraction.get((pa, dl), [])
                ...
                print(f"{r['cell_id']:<6}{pair[:10]:<11}{pa:>6.1f}{dl[:11]:<12}{r['n']:>5}{r['rate_pct']:>6.1f}{r['ev']:>+7.2f}{pf_s:>6}{r['p_welch']:>9.4f}{r['wf_agree']:>3}/{len(r['wf_signs'])}{r['verdict']:>10}")
    survivors = [r for r in grid if r["verdict"]=="SURVIVOR"]
    meta = "SURVIVOR_PRESENT" if survivors else "ALL_REJECT"
    print(f"\nMeta: {meta}")
    out_dir = Path(args.output_dir); out_dir.mkdir(parents=True, exist_ok=True)
    summary = {"pre_reg": "phase5-9d-edge-matrix-2026-04-25.md (S5)",
