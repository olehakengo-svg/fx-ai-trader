...
def preprocess_trades_for_audit(data: dict) -> str:
    """監査用: 戦略×ペアの完全マトリクスを事前集計。"""
    ...
    # 戦略×ペア集計
    matrix = defaultdict(lambda: {"n": 0, "wins": 0, "pnl": 0.0,
                                   "pnl_list": [], "spread_sum": 0.0})
    for t in fx_trades:
        ...
        m["pnl"] += pnl
        m["pnl_list"].append(pnl)
        m["spread_sum"] += t.get("spread_at_entry", 0)

    rows = []
    for (strat, pair), m in sorted(matrix.items(), key=lambda x: -x[1]["n"]):
        wr = (m["wins"] / m["n"] * 100) if m["n"] > 0 else 0
        ev = m["pnl"] / m["n"] if m["n"] > 0 else 0
        avg_spread = m["spread_sum"] / m["n"] if m["n"] > 0 else 0
        # payoff ratio
        wins_pnl = [p for p in m["pnl_list"] if p > 0]
        losses_pnl = [abs(p) for p in m["pnl_list"] if p < 0]
        avg_win = sum(wins_pnl) / len(wins_pnl) if wins_pnl else 0
        avg_loss = sum(losses_pnl) / len(losses_pnl) if losses_pnl else 1
        payoff = avg_win / avg_loss if avg_loss > 0 else 0
        # Kelly
        if payoff > 0 and m["n"] >= 10:
            wr_dec = m["wins"] / m["n"]
            kelly = (wr_dec * payoff - (1 - wr_dec)) / payoff
            kelly_str = f"{kelly * 100:.1f}%"
        else:
            kelly_str = "N/A"
        reliability = "判断可能" if m["n"] >= 30 else ("傾向" if m["n"] >= 10 else "不足")
        rows.append(
            f"| {strat} | {pair} | {m['n']} | {wr:.1f}% | {ev:+.2f} "
            f"| {m['pnl']:+.1f} | {payoff:.2f} | {kelly_str} | {avg_spread:.1f} | {reliability} |"
        )

    total_n = len(fx_trades)
    total_pnl = sum(t.get("pnl_pips", 0) for t in fx_trades)
    total_wins = sum(1 for t in fx_trades if t.get("outcome") == "WIN")
    total_wr = (total_wins / total_n * 100) if total_n > 0 else 0

    return (
        f"### 全体（FX, Shadow除外, N={total_n}, WR={total_wr:.1f}%, PnL={total_pnl:+.1f}pips）\n\n"
        "### 戦略×ペア マトリクス\n"
    ...
    if regime_data and regime_data.get("pairs"):
        ...
        for pair, info in sorted(regime_data.get("pairs", {}).items()):
            if "error" in info:
                continue
            regime_rows.append(
                f"| {pair} | {info['regime']} | {info.get('atr_pctile_20d', 0):.0f}% "
                f"| {info.get('sma20_slope', 0):+.5f} |"
            )
        regime_section = (
            "\n\n### MARKET REGIME\n"
            "| Pair | Regime | ATR%ile(20d) | SMA20 Slope |\n"
            "|---|---|---|---|\n" + "\n".join(regime_rows)
        )

    trades_table = preprocess_trades_for_audit(data)
...
---
上記の集計済みデータに基づき、{audit_type}監査レポートを生成してください。
戦略×ペアマトリクスは既に計算済みなので、そのまま引用して分析・判断に集中してください。
学術文献の引用は不要。実データに基づく判断のみ。"""
