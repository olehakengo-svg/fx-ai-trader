...
PAIRS_D1 = ["USDJPY=X", "GBPJPY=X"]   # D3 高ボラ JPY 系
TF_GRID = ["15m", "30m", "1h", "4h"]   # 2h 除外
N_CELLS = len(PAIRS_D1) * len(TF_GRID)  # 8
ALPHA_CELL = 0.05 / N_CELLS  # 0.00625

TF_PARAMS = {
    "15m": {"max_hold_bars": 36, "mae_breaker": 20, "bar_min": 15},   # 9h, 順張り長め
    "30m": {"max_hold_bars": 36, "mae_breaker": 30, "bar_min": 30},
    "1h":  {"max_hold_bars": 36, "mae_breaker": 50, "bar_min": 60},
    "4h":  {"max_hold_bars": 24, "mae_breaker": 100, "bar_min": 240},
}
...
SURVIVOR_EV_MIN = 1.5
SURVIVOR_PF_MIN = 1.5
SURVIVOR_WR_MIN = 30.0  # D3 順張り低 WR 高 RR 設計
SURVIVOR_N_MIN = 20
SURVIVOR_WF_AGREE = 4
WF_WINDOWS = 4
...
def extract_d1(symbol, df, tf):
    """D3: Z-Score >3σ で **順張り** (S3 逆方向)"""
    pip = PIP_MULT.get(symbol, 100.0)
    fric_half = FRICTION_RT.get(symbol, 2.0) / 2.0 / pip
    out = []
    last_entry_bar = -Z_LOOKBACK
    if len(df) < Z_LOOKBACK + 50: return out
    sma = df["Close"].rolling(Z_LOOKBACK).mean()
    std = df["Close"].rolling(Z_LOOKBACK).std()
    atr = (df["High"] - df["Low"]).rolling(7).mean()

    max_hold = TF_PARAMS[tf]["max_hold_bars"]
    for i in range(Z_LOOKBACK + 10, len(df) - max_hold - 1):
        if i - last_entry_bar < Z_LOOKBACK//2: continue
        m = sma.iloc[i]; s = std.iloc[i]
        if math.isnan(m) or math.isnan(s) or s <= 0: continue
        cur_close = float(df["Close"].iloc[i])
        z = (cur_close - m) / s
        cur_atr = float(atr.iloc[i]) if not math.isnan(atr.iloc[i]) else 0.001
        sig_dir = ep = sl = tp = None
        # **順張り**: z>3.0 → BUY (上方継続), z<-3.0 → SELL (下方継続)
        if z >= Z_THRESHOLD:
            sig_dir = "BUY"
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = "/tmp/phase5-d3-tfgrid-dryrun" if args.dry_run else "raw/bt-results/phase5-d3-tfgrid-2026-04-26"
