...
def label_all(trades: List[dict]) -> Tuple[List[dict], Dict[str, int]]:
    ...
    for inst in sorted(pending["instrument"].unique()):
        ...
        lab = lab[["time", "regime_mtf", "d1_label", "h4_label",
                   "vol_state"]].copy()
        lab["time"] = pd.to_datetime(lab["time"], utc=True)
...
def emit_sql(rows: List[dict], output_path: str) -> None:
    ...
    with open(output_path, "w") as f:
        ...
        f.write("-- Apply via: sqlite3 /render-disk/demo_trades.db < backfill_mtf_regime.sql\n")
...
def main() -> int:
    ...
    if args.write == "sql":
        ...
        print(f"  sqlite3 /render-disk/demo_trades.db < {args.output}")
