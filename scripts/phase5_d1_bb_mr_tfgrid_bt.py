...
PAIRS_D1 = ["EURUSD=X", "USDJPY=X"]
TF_GRID = ["15m", "30m", "1h", "2h", "4h"]
N_CELLS = len(PAIRS_D1) * len(TF_GRID)  # 10
ALPHA_CELL = 0.05 / N_CELLS  # 0.005

# TF 別パラメータ (LOCKED, 文献値ベース)
TF_PARAMS = {
    "15m": {"max_hold_bars": 24, "mae_breaker": 15, "bar_min": 15},   # 6h
    "30m": {"max_hold_bars": 24, "mae_breaker": 20, "bar_min": 30},   # 12h
    "1h":  {"max_hold_bars": 24, "mae_breaker": 30, "bar_min": 60},   # 1d
    "2h":  {"max_hold_bars": 24, "mae_breaker": 50, "bar_min": 120},  # 2d
    "4h":  {"max_hold_bars": 24, "mae_breaker": 80, "bar_min": 240},  # 4d
}
...
def extract_d1(symbol, df, tf):
    ...
    out = []
    last_entry_bar = -BB_PERIOD
    if len(df) < BB_PERIOD + 50: return out
    # Compute BB
    sma = df["Close"].rolling(BB_PERIOD).mean()
    std = df["Close"].rolling(BB_PERIOD).std()
    bb_upper = sma + BB_STD * std
    bb_lower = sma - BB_STD * std
    bb_pct = (df["Close"] - bb_lower) / (bb_upper - bb_lower)
    atr = (df["High"] - df["Low"]).rolling(7).mean()

    max_hold = TF_PARAMS[tf]["max_hold_bars"]
    for i in range(BB_PERIOD + 10, len(df) - max_hold - 1):
        if i - last_entry_bar < BB_PERIOD: continue
        bbpb = bb_pct.iloc[i]
        if math.isnan(bbpb): continue
        ...
        if bbpb >= 0.95:
            ...
            sl = cur_close + SL_ATR_MULT * cur_atr
            tp = float(sma.iloc[i])  # BB middle
            sl_dist = abs(sl - ep); tp_dist = abs(ep - tp)
            ...
            sl = cur_close - SL_ATR_MULT * cur_atr
            tp = float(sma.iloc[i])
            sl_dist = abs(ep - sl); tp_dist = abs(tp - ep)
            if sl_dist <= 0 or tp_dist / sl_dist < MIN_RR_HARD: continue
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = "/tmp/phase5-d1-tfgrid-dryrun" if args.dry_run else "raw/bt-results/phase5-d1-tfgrid-2026-04-26"
