...
# - PAIR: 本日 Top2 + EUR/JPY (London cross 追加)
# - vol_spike: 本日 SURVIVOR 兆候値 2.5 で固定 (HARKing 回避)
# - session: London/NY + 両 session 統合 (新検定軸)
PAIRS_S7 = ["GBPUSD=X", "GBPJPY=X", "EURJPY=X"]
VOLUME_SPIKE_GRID = [2.5]
SESSION_GRID = {
...
ASIA_RANGE_HOURS = 6
N_CELLS = len(PAIRS_S7) * len(VOLUME_SPIKE_GRID) * len(SESSION_GRID)  # 9
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = ("/tmp/phase5-orb-2y-dryrun" if args.dry_run
                           else "raw/bt-results/phase5-orb-2y-2026-04-26")
