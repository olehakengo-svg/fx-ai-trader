...
2. デスゾーン = スプレッド異常（動的検出）のみ — 動的なスプレッド/SL gate で防御
3. **Shadow ≠ LIVE で対称ではない** — Shadow データ蓄積は静的時間ブロックで削らない (Bonferroni edge discovery の statistical power 優先)。LIVE OANDA 転送は「勝てる場所で勝つ条件だけ転送」が正しい設計で、session_pair / gbp_asia_flash_crash / alpha_scan の winning-location フィルタは LIVE 側で意図的に維持
4. 攻撃は最大の防御 — 防御フィルターの積み上げよりデータ蓄積を優先 (Shadow 側)
5. カーブフィッティング禁止 — パラメータ調整は完了、本番データ蓄積フェーズ
