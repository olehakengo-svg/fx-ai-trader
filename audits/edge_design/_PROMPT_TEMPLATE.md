...
roadmap_gate: "W4 Edge Design Audit — {{STRATEGY}} の 8 軸設計監査"
rule: R1
prereq_artifacts:
...
`{{STRATEGY}}` (`{{TIER}}` / `{{SOURCE_TIER}}`) について 8 軸で診断し、
...
各軸を順に処理。**コード参照は必ず `{{STRATEGY_PATH}}:LINE` 形式で引用する**。
...
8. **failure mode 診断** — `{{TIER}}` ∈ {`Tier 3`, `Tier 4`}、または Tier 1/2 で metrics 劣化のもの。**どの軸（2/3/4/5）で破綻したか同定**し、再設計案を最低 1 案。「思想は正、設計が誤」仮説を裏付ける具体方針:
   - Trigger 修正（条件式変更）
   - Filter 削除 or 置換
...
ファイル `audits/edge_design/{{STRATEGY}}.md` に以下フォーマットで **新規** 出力（既存があれば追記ではなく上書き）。
...
<thesis 文 (1-2 行) + コード引用 `{{STRATEGY_PATH}}:N`>
...
- `audits/edge_design/{{STRATEGY}}.md` が §3 のテンプレ通りに存在する
- Verdict が 5 分類のいずれか
