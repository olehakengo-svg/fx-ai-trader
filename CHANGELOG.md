...
- `modules/demo_trader.py`: `engine_process_role()` (`import`/`forked`) + 両 `open_trade` call site で reasons に `[EMIT_PROC] <role>` / `[MainLoop]`・tick・`[StatusHeal]` ログに `pid=`・`role=` / `get_status()` に `engine_pid`・`engine_process_role`。`app.py`: `[AutoStart] Starting` に pid/role。record-only (取引挙動・gate・dedup 不変)
- pin: `tests/test_dual_engine_process_attribution.py` (12)、counterfactual で 3 本落ちを確認。registry: 親 entry 追記 + `dual-engine-emit-proc-attribution-readout` (10-01) 新設。統合パケット D16 (record)
...
- `tools/nav_floor_projection.py`: `transfers_in_window` / `fetch_transfers` / `nav_ts_from_status`、`edge_burn_per_day(..., transfers, nav_ts)` (None = `unavailable:transfers_unavailable`、ゼロと偽らない)、新列 `nav_ts_utc` (窓端 = NAV 採取時刻 = `heartbeat.nav_at` [成功 heartbeat で nav と同時にだけ更新、PR #295 P2]、legacy 端の同日 tx は fail-closed)、basis に `transfers_jpy,n_transfers`
- `modules/oanda_bridge.py`: `heartbeat.nav_at` 追加 (telemetry のみ、失敗 heartbeat では動かない)
...
- 判定・戻り値・is_shadow 最終値・行数・凍結値は不変 (record-only)。pin: `tests/test_pre_send_guard_observability_r3.py` 26 本 (counterfactual 10 / 対称側 10 / shadow_only 母集団不変 2 / review P2 消化 4)
...
- review P2 消化: `session_filter` の audit は legacy `shadow_tracking(session_filter_out)` を維持 (P-V4 契約)、消費側正規化 `promo_block_cause_from_audit()` + session_filter 行 pin を追加 / 2 巡目: (d) 帰属条件を不変 snapshot `_live_intent_at_open` へ (live 復活後の FD 最終ゲート経路の取り逃し修正) / 3 巡目: first-block 記録を終端 `_block` に限定 (非終端 `session_filter_live_downgrade` を除外)
...
- registry 80 → 90 entry (新規 6 / 既存 9 追記、g0prime 期日は 09-28 のまま (10-05 への繰越は Codex P2 で撤回、09-27 NO-QUALIFY 時のみ手動繰越)) — 詳細: knowledge-base/wiki/changelog.md
