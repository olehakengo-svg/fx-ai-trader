...
## 最重要目標（全施策の判断基準）
**正式ミッション = M3 系列 (+2〜3%/月 複利) — user 決裁 U1=(b) 2026-09-17。経路 = M1 (clean live 月次符号転換) → M2 (+0.5%/月) → M3 (2026-07-10 段階化)**
- 旧 anchor「月利21.6%接近」「20%/月」は U1=(b) (2026-09-17) で記録から除去 — 現行ミッションの宣言・施策スコアリングに使用禁止 (分母は time-to-M2 短縮に統一)。決裁記録: `wiki/decisions/u1-mission-redecision-2026-09-17.md` / 歴史的導出 (不改変で保存): `wiki/analyses/monthly-target-rederivation-2026-07-10.md`, `wiki/decisions/shortest-path-decision-memo-2026-07-10.md`
...
## コードレビュー
- **独立レビューの実体 = GitHub Codex cloud connector** (PR #75 以降ほぼ全 PR に P1/P2 付きレビューを自動投稿)。旧記載「Codex プラグインの stop-time レビュー」は実行記録ゼロで廃止 ([[process-meta-audit-2026-09-07]] codex-1 訂正版)
- **マージ前レビュー消化ゲート (2026-09-10、R2 提言)**: レビューは届いているのに読まれずマージ (レビュー→マージ 56 秒〜2 分が常態) が真の欠陥だった。**R1 / verdict / live 経路 / monitoring に触る PR は、マージ前に `python3 tools/pr_review_gate.py <PR番号>` で connector レビュー到着 + P1/P2 消化 (対応 commit or 理由付き dismiss コメント) を確認する**。docs/KB のみの PR は従来通り即マージ可
- **修正 push 後は再レビューが自動で走らない** — `gh pr comment <N> --body "@codex review"` を先に実行してからゲートを再実行する (2026-09-08 実測、[[pr-review-gate-2026-09-08]] §5c)
...
- **PR マージは Claude が自走** — CI green 確認後 `gh pr merge N --merge --admin` を**単独コマンド形で**実行 (複合コマンドは classifier 拒否)。hot file conflict は union 解決。**コード/live 経路 PR はマージ前に `tools/pr_review_gate.py` で connector レビュー消化を確認 (上記コードレビュー節)**。詳細: MEMORY `project_pr_merge_race_parallel_sessions`
