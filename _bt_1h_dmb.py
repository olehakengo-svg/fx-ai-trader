#!/usr/bin/env python3
"""
DMB (Donchian Momentum Breakout) 1H Backtest
HourlyEngine + 実4H/1D HTFバイアス — DMB単体テスト
"""
import os, sys
...
def fetch_data(symbol, days=120, interval="1h"):
...
def compute_htf_bias(df_1h, bar_idx):
    ...
    # 1D EMA50の方向判定 (USD/JPY SELL非対称フィルター用)
...
def run_bt(symbol, label, days=120):
    print(f"\n{'='*60}")
    print(f"  {label} 1H DMB BT ({days}d)")
    print(f"{'='*60}")
    ...
    strat = DonchianMomentumBreakout()

    _is_jpy = "JPY" in symbol.upper()
    _pip_mult = 100 if _is_jpy else 10000

    MAX_HOLD = 24
    COOLDOWN = 6       # 6時間最低間隔（トレンド中の連続エントリー抑制）
    BE_TRIGGER = 0.50
    TRAIL_ATR_MULT = 1.5
    ...
    for i in range(52, len(df) - MAX_HOLD - 1):
        ...
        _spread = 0.03 if _is_jpy else 0.00015
        ...
        for j in range(1, MAX_HOLD + 1):
            ...
            if _best_progress >= tp_dist * BE_TRIGGER and not _be_activated:
                _be_activated = True
                _be_price = ep + _spread if sig == "BUY" else ep - _spread
                if sig == "BUY":
        ...
        trades.append({
            ...
            "pips": round(_pips, 1),
            "entry_type": "donchian_momentum_breakout",
