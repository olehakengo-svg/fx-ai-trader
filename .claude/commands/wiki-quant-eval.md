...
### B2. 正EV候補 N蓄積ダッシュボード（v8.9 新規）
月利100%ロードマップの最重要指標。正EV候補4戦略のN推移を追跡:
| 戦略 | ペア | 現在N | 目標N | 日次増加率 | N=30到達予測日 | PROMOTE条件 |
|------|------|-------|-------|-----------|------------|-------------|
| vol_momentum_scalp | JPY | ? | 30 | ? | ? | N≥20 WR≥60% EV≥1.0 |
| fib_reversal | JPY | ? | 50 | ? | ? | N≥30 EV≥1.0 |
| ema_pullback | JPY | ? | 30 | ? | ? | N≥30 EV≥1.0 |
| session_time_bias | EUR/GBP | ? | 30 | ? | ? | N≥20 WR≥60% EV≥1.0 |

計算方法:
- 日次増加率 = (現在N - 前回eval時のN) / 経過日数
- N=30到達予測日 = 今日 + (30 - 現在N) / 日次増加率
- 前回evalのNは `raw/trade-logs/quant-eval-*.md` の最新ファイルから取得
- N蓄積が0件/週の戦略は⚠️フラグ（発火障害の可能性）
