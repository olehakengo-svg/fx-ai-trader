...
def test_v2_shadow_worker_registration_is_opt_in(monkeypatch):
    ...
    xs = Candidate("BUY", 60, 1.0, 1.2, ["xs"], "xs_momentum", 5.0)

    monkeypatch.delenv("XS_MOMENTUM_REDESIGN_V2", raising=False)
    monkeypatch.delenv("XS_MOMENTUM_REDESIGN_V2_SHADOW_PROMOTE", raising=False)
    assert engine.split_shadow_always([best, xs], best) == []

    monkeypatch.setenv("XS_MOMENTUM_REDESIGN_V2", "1")
    monkeypatch.setenv("XS_MOMENTUM_REDESIGN_V2_SHADOW_PROMOTE", "1")
    assert engine.split_shadow_always([best, xs], best) == [xs]
