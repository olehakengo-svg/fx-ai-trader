...
Pin する契約 (P-S1(a) Option B 執行後、2026-09-11 merge 更新):
1. P-S1(a) 免除 cell (sweep_reversion_eurgbp_late × EUR_GBP) はゾーン内でも gate を
   通過し live 経路 (AMENDMENT、user 承認 2026-08-03 — packet §8)
2. 免除が将来 pin 等で無効化された場合は PR #180 の shadow rescue が backstop に
   なり shadow row として記録される (is_shadow=1、OANDA 送信なし) — rescue 経路は
   落とさない (4原則#3、分母 silent 枯渇の再発防止)
3. rescue 集合外はゾーン内で従来どおり rowless hard block (defense 縮小なし)
"""
...
def test_sweep_cell_ps1a_exempt_goes_live_in_gbp_asia_zone(tmp_path, monkeypatch):
    """P-S1(a) AMENDMENT: 免除 cell はゾーン内でも gate を通過し live 経路。"""
    ...
    assert rows, "exempt cell must write a row inside the gbp_asia zone"
    for _et, inst, is_shadow, _oanda_id in rows:
        assert inst == "EUR_GBP"
        assert is_shadow == 0, "P-S1(a) 免除 cell はゾーン内で live (AMENDMENT)"
    assert trader._oanda.calls, "exempt cell must reach the OANDA live path"
    assert not _gbp_asia_blocks(trader), \
        "exempt cell must not be counted as gbp_asia hard block"


def test_sweep_cell_shadow_rescue_backstop_when_exemption_off(tmp_path, monkeypatch):
    """P-S1(a) 免除が無効化された場合の backstop: PR #180 rescue が shadow 退避。

    rescue 経路 (_GBP_ASIA_SHADOW_RESCUE_CELLS) は免除と併存し、免除が将来
    pin 等で外れても rowless hard block へ退行しない (4原則#3、分母保存)。
    """
    from modules.demo_trader import DemoTrader

    trader, _logs = _make_trader(tmp_path, monkeypatch, hour=22)
    monkeypatch.setattr(
        DemoTrader, "_GBP_ASIA_FLASH_CRASH_EXEMPT_CELLS", frozenset())
    ...
    for _et, inst, is_shadow, oanda_id in rows:
        assert inst == "EUR_GBP"
        assert is_shadow == 1, "rescue is shadow-only — live 例外は P-S1(a) 免除のみ"
