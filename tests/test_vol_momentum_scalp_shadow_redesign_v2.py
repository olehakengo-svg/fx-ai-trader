...
def test_redesign_v2_flag_uses_closed_signal_bar_not_current_bar(monkeypatch):
    monkeypatch.setenv("VOL_MOMENTUM_SCALP_REDESIGN_V2", "1")
...
def test_redesign_v2_flag_blocks_current_bar_only_signal(monkeypatch):
    monkeypatch.setenv("VOL_MOMENTUM_SCALP_REDESIGN_V2", "1")
    monkeypatch.delenv("VOL_MOMENTUM_REDESIGN_V2", raising=False)
    VolMomentumScalp.reset_dedup_state()
...
def test_redesign_v2_flag_dedups_same_signal_bar(monkeypatch):
    monkeypatch.setenv("VOL_MOMENTUM_SCALP_REDESIGN_V2", "1")
    monkeypatch.delenv("VOL_MOMENTUM_REDESIGN_V2", raising=False)
    VolMomentumScalp.reset_dedup_state()
...
def test_redesign_v2_default_off_keeps_live_path_unchanged(monkeypatch):
...
def test_shadow_promote_worker_registration_uses_scalp_double_flag(monkeypatch):
    ...
    monkeypatch.setenv("VOL_MOMENTUM_SCALP_REDESIGN_V2", "1")
    monkeypatch.delenv("VOL_MOMENTUM_SCALP_REDESIGN_V2_SHADOW_PROMOTE", raising=False)
    assert engine.split_shadow_always([other, vol_momentum], other) == []

    monkeypatch.setenv("VOL_MOMENTUM_SCALP_REDESIGN_V2_SHADOW_PROMOTE", "1")
