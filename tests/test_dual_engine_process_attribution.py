...
def test_role_is_import_in_the_importing_process():
    ...
    assert emit_proc_marker() == f"{EMIT_PROC_REASON_TAG} import"
...
def test_role_is_forked_when_pid_differs_from_import_pid(monkeypatch):
    ...
    assert emit_proc_marker() == f"{EMIT_PROC_REASON_TAG} forked"


def test_marker_is_two_valued_not_pid():
    """row 側 marker に PID を入れると boot 毎に値が変わりラベル分析の
    cardinality が爆発する — role の 2 値だけを許す。"""
    assert not re.search(r"\d", emit_proc_marker())
...
def test_exactly_two_open_trade_call_sites_and_both_append_marker():
    ...
    for pos in sites:
        ...
        assert "emit_proc_marker()" in window, (
            "open_trade の直前 600 文字に emit_proc_marker() が無い call site がある"
        )
    assert SRC.count("+ [emit_proc_marker()]") == 2, "helper の使用箇所 (append) は call site 数と一致"
...
def test_get_status_payload_declares_engine_pid_and_role():
...
def test_shadow_emit_row_persists_emit_proc_marker(trader):
    ...
    assert f"{EMIT_PROC_REASON_TAG} import" in reasons
...
def test_shadow_emit_row_marker_reflects_forked_role(trader, monkeypatch):
    ...
    rows = [r for r in trader._db.get_open_trades() if r.get("trade_id") == trade_id]
    assert len(rows) == 1
    assert f"{EMIT_PROC_REASON_TAG} forked" in _reasons_of(rows[0])
