...
def test_v2_default_off_preserves_legacy_current_bar_signal(monkeypatch):
    monkeypatch.delenv("GOLD_TREND_MOMENTUM_REDESIGN_V2", raising=False)
    GoldTrendMomentum.reset_dedup_state()

    cand = GoldTrendMomentum().evaluate(_ctx(_df(closed_signal=False, current_signal=True)))

    assert cand is not None
    assert cand.signal == "BUY"
    assert not any("GOLD_TREND_MOMENTUM_REDESIGN_V2" in reason for reason in cand.reasons)


def test_v2_rejects_current_bar_only_signal(monkeypatch):
    monkeypatch.setenv("GOLD_TREND_MOMENTUM_REDESIGN_V2", "1")
    GoldTrendMomentum.reset_dedup_state()
...
def test_v2_uses_closed_signal_bar_with_current_entry(monkeypatch):
    ...
    cand = GoldTrendMomentum().evaluate(_ctx(_df(closed_signal=True, current_signal=False)))

    assert cand is not None
    assert cand.signal == "BUY"
    assert round(cand.sl, 2) == 1999.90
    assert round(cand.tp, 2) == 2003.70
    assert any("GOLD_TREND_MOMENTUM_REDESIGN_V2" in reason for reason in cand.reasons)


def test_v2_per_signal_bar_dedup_blocks_repeat(monkeypatch):
    monkeypatch.setenv("GOLD_TREND_MOMENTUM_REDESIGN_V2", "1")
    GoldTrendMomentum.reset_dedup_state()
    strategy = GoldTrendMomentum()
...
def test_shadow_promote_worker_registration_is_double_flagged(monkeypatch):
    ...
    gold = Candidate("BUY", 70, 1.0, 2.0, [], "gold_trend_momentum", 4.0)
    engine = DaytradeEngine()

    monkeypatch.setenv("GOLD_TREND_MOMENTUM_REDESIGN_V2", "1")
    monkeypatch.delenv("GOLD_TREND_MOMENTUM_REDESIGN_V2_SHADOW_PROMOTE", raising=False)
    assert engine.split_shadow_always([other, gold], other) == []

    monkeypatch.setenv("GOLD_TREND_MOMENTUM_REDESIGN_V2_SHADOW_PROMOTE", "1")
