...
def test_v2_shadow_worker_registration_is_opt_in(monkeypatch):
    ...
    confluence = Candidate("BUY", 60, 1.0, 1.2, ["confluence"], "confluence_scalp", 5.0)

    monkeypatch.delenv("CONFLUENCE_SCALP_REDESIGN_V2", raising=False)
    monkeypatch.delenv("CONFLUENCE_SCALP_REDESIGN_V2_SHADOW_PROMOTE", raising=False)
    assert engine.split_shadow_always([best, confluence], best) == []

    monkeypatch.setenv("CONFLUENCE_SCALP_REDESIGN_V2", "1")
    monkeypatch.setenv("CONFLUENCE_SCALP_REDESIGN_V2_SHADOW_PROMOTE", "1")
    assert engine.split_shadow_always([best, confluence], best) == [confluence]
