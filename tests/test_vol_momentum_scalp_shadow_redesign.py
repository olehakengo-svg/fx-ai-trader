...
def test_redesign_v2_uses_closed_signal_bar_not_current_bar(monkeypatch):
    ...
    assert got.tp > _ctx(df).entry


def test_redesign_v2_rejects_when_closed_bar_has_no_breakout(monkeypatch):
    monkeypatch.setenv("VOL_MOMENTUM_SCALP_REDESIGN_V2", "1")
    VolMomentumScalp.reset_dedup_state()
...
def test_redesign_v2_dedups_same_symbol_strategy_signal_bar(monkeypatch):
    monkeypatch.setenv("VOL_MOMENTUM_SCALP_REDESIGN_V2", "1")
    VolMomentumScalp.reset_dedup_state()
    ctx = _ctx(_df())

    first = VolMomentumScalp().evaluate(ctx)
    second = VolMomentumScalp().evaluate(ctx)
